...
**NOTE** the *old* way was overly simplistic (divide by 298 loci) and ignored no-calls (i.e. score = "0")
...
PSTOCK17n_ind_het <-
  apply(PSTOCK17n.gcl$scores, 1, function(ind) {
    sum(ind[, "Dose1"] != ind[, "Dose2"]) / sum(ind[, "Dose1"] != "0")
  })
PSTOCK17h_ind_het <-
  apply(PSTOCK17h.gcl$scores, 1, function(ind) {
    sum(ind[, "Dose1"] != ind[, "Dose2"]) / sum(ind[, "Dose1"] != "0")
  })
PSTOCK19_ind_het <-
  apply(PSTOCK19.gcl$scores, 1, function(ind) {
    sum(ind[, "Dose1"] != ind[, "Dose2"]) / sum(ind[, "Dose1"] != "0")
  })
