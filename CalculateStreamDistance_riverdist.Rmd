...
High tide marks were prodvided by the crew and Pete Rand. The final distance upstream will be calculated as 1) positive if above high-tide, and 2) negative if below the high-tide mark.    
The tidal marks are snapped to the streamlines using arcmap and exported as coordinates.     
...
```{r inputs}
stream = "paddy" # Enter stream name here
...
In some cases, there are two high-tide marks, one for each channel or tributary. Distances will have to be manually caclulated based on which tide mark is applicable to each fish.    
Since river network has segment routes for each segment, I tried to automate these calculations. However, it was to be easier to manually assign a tide mark based on which segment the fish are in, unfortunately. 

```{r}
tmp <- tibble(rivercleaned$segroutes) %>% 
  mutate(intertidal_seg = case_when(
    grepl(tide_df$seg[[1]], rivercleaned$segroutes) ~ tide_df$seg[[1]], # find all routes containing the first high-tide mark
    grepl(tide_df$seg[[2]], rivercleaned$segroutes) ~ tide_df$seg[[2]], # find all routes containing the second high-tide mark
    TRUE ~ as.integer(rivercleaned$mouth[1]) # 
    ))
```
...
```{r upstream_tidal}
final_df <- fish_df %>% 
  add_column(tidedist = tide_df$tidedist) %>% 
  mutate(dist2tide = mouthdist - tidedist)

sum(final_df$dist2tide > 1) / nrow(final_df) # how many waypoints are in the intertidal area??
