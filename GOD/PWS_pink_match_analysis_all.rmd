...
**0 / 298** loci failed; rates >18% (see table).
...
There are 8,627 total otos in PGOD21, some of which failed, but we need to keep as many as possible. 
...
We will retain the 15% cutoff (85% retention) from the initial rounds, giving us a minimum of 45 loci that must be genotyped. The internal duplicate rate check will remain at 99%. 
...
We have 8,243 / 8,627 otoliths remaining, with 376 failing the missingness and eight failing the duplicate rate filters.

**NOTE** Similar to the normal AHRP samples, we'll want to remove **BOTH** duplicates, if they show up, since we won't know which one identifies back to the heart genotype. The function `RemoveDups.GCL` only removes 1 of the duplicates, and we need to manually remove the other.
...
There are 8,235 otos going forward.
...
PGOD21_DWPs_and_SILLYs <- read_csv("PGOD21_DWPs_and_SILLYs_trimmed_091222.csv")
...
write_csv(new_PGOD21_attributes_and_SILLY, "new_PGOD21_attributes_and_SILLY_091222.csv")
