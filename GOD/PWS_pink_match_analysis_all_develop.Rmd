...
*Update 2023-02-13* The final 9 otoliths have been found, Jodi Estrada added the "freaks" to the end of 50960 since it was the last one from MTAL, and thus only partially filled.
...
# Final Stats 2023-02-10

10,791 otoliths with genotypes. Does not include Jodi's transfers...
...
Update 2023-02-10. Kristen remembered that Jodi had transferred these otoliths from 8 DWPs that had been sitting in her office. Going re-do this work and update again.

# Re-do 2023-02-14 

## Genotypes

Go ahead and re-import all genotypes on 2023-02-14, to make sure we have the most up to date heart genotypes available.
```{r}
load_objects(path = "~/../Desktop/Local_PWS_pinks/Genotypes/GOD_develop/", pattern = "LocusControl")
...
Save for ease of access (imported from **LOKI** 2023-02-14)
```{r}
save_sillys(
  sillyvec = c(
    "PGOD21",
...
Right off the bat, we can see that we only have transfer records for 10,822 otoliths when we should have records for 10,816...that's (6 extra otoliths based on `PGOD21.gcl` tissue inventory).
...
```{r}
save_sillys(sillyvec = "PGOD21", path = "output/")
```
...
```{r}
PoolCollections.GCL(
  collections = c(paste0("PERB", 15:17),
                  paste0("PGILMOUR", 15:18),
                  "PPADDY16"),
  loci = LocusControl$locusnames,
  newname = "heart_genotypes"
)
```

Filter heart genotypes for affected DWPs
```{r}
(
...
Make sure that all affected DWPs are included.
```{r}
heart_genotypes.gcl %>% 
...
Huh, I appear to only have 431 out of 439 DWPs, what gives? Better cross reference the outersect with all AHRP DWPs, maybe some are not GOD-related, but other otolith incidents.
```{r}
...
Oh snap, these are GOD otoliths, I just don't have heart genotypes? That's not good.

Okay, time to go look at our extraction lists to see if these got missed in our GOD genotyping prioritization, which came from `GOD_affected_DWPs_consensus`. Need to investigate that origin of that file to see how these 8 DWPs were overlooked.

Update on 2023-02-14: 

`GOD_affected_DWPs_consensus` was developed off of `otolith_transfer_111721.csv`, which did **not** include the vial otoliths `otolith_transfer_96_template_VIALS_LEFT.xlsx` or Jodi Estrada's GCL in-house otolith transfers `JLE_otolith_transfer_96.xlsx`. Thus, these 8 DWPs that do not have heart genotypes were not prioritized for genotyping.

The first 7 of these 48DWPs were on Jodi Estrada's 2 otolith trays, the last one was a vial extraction (14360 PPADDY16). Need to see which projects these are on to see if I can get them re-prioritized...

It is very weird that 14360 (PPADDY16) has not been genotyped, as that entire SILLY has been genotyped, none remaining. Going back to the data used to make eP008 (when these fish should have been extracted), this DWP is absent, they must not have been built at the time. We are not going to go back and extract these hearts, this DWP is dead.

For the first 7 DWPs (6 - PERB15, and 1 - PERB17), we'll need to see what genotyping projects they are on and see if we can re-prioritize those projects.

Get `SillySource` for the fish on those DWPs
```{r}
(
  missing_heart_SillySource <- AHRP_oceanAK %>%
    dplyr::mutate(
      "48_DWP_tray_barcode" = as.numeric(DNA_TRAY_CODE),
      sample_date = lubridate::as_date(SAMPLE_DATE)
    ) %>%
    dplyr::filter(
      `48_DWP_tray_barcode` %in% missing_heart_DWPs,
      SILLY_CODE != "PPADDY16"
    ) %>%  # drop PPDADDY16, this DWP is dead to me
    tidyr::unite(
      col = "SillySource",
...
Read in Project Plate maps
```{r}
project_files <- sapply(paste0("P0", 87:98), function(project) {
  list.files(
    path = paste0("V:/Lab/Genotyping/DNA Plate Loki Exports/Pink/", project),
    pattern = ".csv", 
    full.names = TRUE
  )
}) %>% unlist()

(project_plate_maps <- readr::read_csv(file = project_files))
```

Filter for missing hearts, which projects?
```{r}
project_plate_maps %>% 
  dplyr::filter(SILLY_FISHID %in% missing_heart_SillySource) %>% 
  dplyr::count(Project)
```

Damn, as suspected, these fish are spread over multiple projects, not going to rock the boat, will just have to wait patiently. We'll start our GOD matching without these fish.
