...
This will require careful cutoffs for dupe rate and dupe rate difference - suggest duplicate_rate >= 0.7 and diff >= 0.3? 
...
Of the otos that worked, most matched one heart cleanly, but one heart matched 2 otos and the remaining otos failed. This was not a great DWP.
...
Are we going to have to set different cutoffs for dupe rate and dupe rate difference for each DWP? Suggested duplicate_rate >= 0.7 and diff >= 0.3 for 14395, but those cutoffs could cull all but one oto here. However, maybe saving an extra two otos doesn't really matter in the grand scheme of things.
...
```{r}
# create *.gcl objects for comparison according to *._fish_ids
PoolCollections.GCL(collections = "PERB16", loci = loci, IDs = list("PERB16" = as.character(PERB16_DWP_14395_fish_ids)), newname = "PERB16_DWP_14376")
PoolCollections.GCL(collections = "PGOD21", loci = loci, IDs = list("PGOD21" = as.character(PGOD21_DWP_14395_fish_ids)), newname = "PGOD21_DWP_14376")
```
...
Definitely starting to think we going to have to set different cutoffs for dupe rate and dupe rate difference for each DWP. Originally suggested duplicate_rate >= 0.7 and diff >= 0.3 for 14395, but for this DWP, we might be able to get away with duplicate_rate >= 0.7 and diff >= 0.15. Would be nice if ggplotly worked and didn't keep throwing a "filepath too long" error.
