...
**everything below here is old and bad**
...
```{r}
dup_check_results_DWP_14395 %>% 
  count(otolith_fish_id) %>% 
  filter(n == 1) %>% 
  left_join(dup_check_results_DWP_14395, by = "otolith_fish_id")
```
...
dup_check_results_DWP_14395 %>% 
  count(Keysillyvial) %>% 
  filter(n > 1) %>% 
  left_join(dup_check_results_DWP_14395, by = "Keysillyvial") %>% 
