---
title: "PGOD21 otolith/OG SILLY heart GT-seq DWP-by-DWP matching: P066 and P067 (RGOD Genotyping Rounds 3 and 4)"
...
```{r}
# read in attributes data from last round(s) and remove processed fish_ids
...
oto_xfer_data <- read_csv("otolith_transfer_012522.csv") # mod of MTAL final spreadsheet "otolith_transfer_96_template_LEFT_all trays_17Nov21.xlsx"
...
write_csv(attributes_PGOD21, "attributes_PGOD21_031122.csv") # use SillySource from prior dates to remove already processed IDs from PGOD21.gcl
...
save_sillys(sillyvec = c("PGOD21", "PGOD21", "PGILMOUR15", "PGILMOUR16", "PGILMOUR17", "PPADDY16"), path = "../../Genotypes/GOD_21_Round3-4/")
```
...
There are 2,090 total otos in PGOD21 Rounds 3 and 4. We still need to keep as many critters as possible within reason. We're dealing with 298 loci, so we should have wiggle room, but we also have to remember that the info content of each locus is not the same. 
...
This looks better than the last time, I think. We will retain the 15% cutoff from the last round, giving us a minimum of 45 loci that must be genotyped, and keep the internal duplicate rate check at 99%. 
...
Not run this time, but... **NOTE** Similar to the normal AHRP samples, we'll want to remove **BOTH** duplicates, if they show up, since we won't know which one identifies back to the heart genotype. The function `RemoveDups.GCL` only removes 1 of the duplicates, we need to go in and manually remove the other.
...
dup_check_results_23_pass <- read_csv("dup_check_results_23_pass.csv")

# alternatively, use if all objects are available
dup_check_results_23_pass <- bind_rows(
...
write_csv(dup_check_results_23_pass, "dup_check_results_23_pass.csv")
```
...
dup_check_results_23_pass_with_wells <- attributes_and_xfer_PGOD21 %>% 
  mutate(otolith_fish_id = SillySource) %>% 
  right_join(dup_check_results_23_pass, by = "otolith_fish_id") %>% 
...
write_csv(dup_check_results_23_pass_with_wells, "dup_check_results_23_pass_with_wells.csv")
```
...
```{r}
dup_check_results_23_all <- bind_rows(
dup_check_results_DWP_14395,
dup_check_results_DWP_14449,
dup_check_results_DWP_14448,
dup_check_results_DWP_14334,
...
write_csv(dup_check_results_23_all, "dup_check_results_23_all.csv")
```

Filter for hearts that have multiple otos matching at > 0.60 duplicate rate

```{r}
(
  hearts_with_multiple_matchy_otos <-
    dup_check_results_23_all %>%
    filter(duplicate_rate > 0.7) %>%
    count(heart_fish_id) %>%
    filter(n > 1) %>%
    pull(heart_fish_id)
