...
author: "Kyle Shedd"
date: "2024-02-09 (hearts) 2024-05-21 (otoliths)"
output:
  html_notebook:
    theme: united
    toc: yes
...
Standard data QA (GT-seq, no `conScore`):
  ...
  * Remove duplicates (>95% genotype concordance, remove both duplicates)
...
Non-Standard data QA (GT-seq, no `conScore`):
  ...
  * Remove fish missing <15% genotypes
  * Remove duplicates (>99% genotype concordance, remove one duplicate)
  * Remove contamination (heterozygosity outliers +/- 3.5 modified z-score)??????
...
We started with XX hearts from XX collections. We lost XX due to missing, XX due to duplicates (both removed), and XX due to contamination. XX collection lost more than the others, presumably due to XX. Final set of hearts is XX.

We started with XX otoliths in PGOD21. We lost XX due to missing, XX due to duplicates (only 1 removed), and XX due to contamination. Final set of otoliths is XX.
