...
The next steps after this will be to *investigate otoliths with reads but no heart match* and *compare heart matches to existing AHRP data*. 
...
9,678 of 10,816 otoliths were successfully read (89.5%). Of those not read, 266 were lost and 760 were unreadable/overground. **NOTE** the otolith codes indicate that almost all are "left", but I'm not sure if that is completely true. Of the 1,648 otoliths with a hatchery mark, most were AFK (unsurpisingly), followed by Cannery Creek, Wally Norenburg, and Solomon Gulch.
...
There are 11 hearts that match > 1 otolith with different hatchery vs. wild otolith reads. While some of these have very high duplicate rates or there is a big disparity in the number of missing loci for the otolith genotype, it might just be cleanest to drop all 11 of these.

Look at `mark_id` conflicts
```{r}
...
Looks like there are 3 hearts that closely match 2 otoliths each, but clearly from different fish due to different otolith reads. Two are vial otoliths. Cleanest to drop all 3.
...
`NA` otolith position
Hearts that best match up to 2 otoliths is okay, so long as otoliths have the same read (hatchery vs. wild + origin)
Otoliths that best match > heart are a non-issue, bogus matches
Do we want to see otolith count maps?
Pull in PGOD21 otolith reads to see if the matches matter if there are close ties or multiple matches?
