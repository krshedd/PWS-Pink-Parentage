...
The purpose of this notebook is to identify which DWP wells were part of GOD and have untrustworthy otolith pairing with hearts. We already have the DWP well data for the GOD otolith transfers 48 DWP --> 96 SWP (i.e., where they were removed from at MTAL for genotyping/otolith reading in `GOD/output/PGOD21_48DWP_paired_genotype_matches.csv`), but we do not have a tidy, data friendly version of where they started. We will extract this info from the DWP maps in `GOD/data/raw_working_oto_sheets.xlsm`, which denote GOD wells with red font.

The next steps after this will be to *investigate GOD otoliths with reads but no heart match* to make additional assignments based on logic if all remaining otolith reads are the same, then *verify heart matches are GOD fish (i.e., don't have existing otolith reads)*, and ultimately *submit finalized PGOD21 matches to Eric Lardizabal*. 
...
(
  DWP_maps <- readxl::read_excel(
    ...
    sheet = "DWP maps",
    range = "A1:G8139",  # updated after fabricating DWP maps for Jodi's in-house DWP transfers
...
Total of 11,400 (original was 10,898) GOD DWP wells as of *2025-04-02*. This is close, but not the same as the 10,791 PGOD21 otoliths.
...
Read in the raw contractor data, it notes whether any otoliths were missing in the field.
```{r}
(
...
A total of **11,260** GOD wells, as of 2025-04-02.
...
# Verify PGOD21 Assignments Are GOD Wells

First we need to make sure that we haven't assigned any PGOD21 otoliths to non-GOD DWP wells. This will be a good double check to make sure we have **ALL** the GOD wells.
```{r}
# wells
PGOD21_48DWP_paired_genotype_matches %>% 
  dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
  dplyr::anti_join(y = red_cells_mod %>% 
                     dplyr::mutate(`48_DWP_tray_barcode` = as.numeric(`48_DWP_tray_barcode`)),
                   by = dplyr::join_by(`48_DWP_tray_barcode`, heart_position))

# count of wells by DWP
PGOD21_48DWP_paired_genotype_matches %>% 
  dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
  dplyr::anti_join(y = red_cells_mod %>% 
                     dplyr::mutate(`48_DWP_tray_barcode` = as.numeric(`48_DWP_tray_barcode`)),
...
Shit, as of 2025-03-27 there are 290 assignments from 76 DWPs to non-red cells.
2025-03-27 down to 170 assignments from 65 DWPs after fixing red font issue.
2025-03-28 down to 77 assignments from 54 DWPs after adding Jodi's 7 in-house transfers.

How many of these are DWP+Position vs. vial?
```{r}
PGOD21_48DWP_paired_genotype_matches %>% 
  dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
  dplyr::anti_join(y = red_cells_mod %>% 
                     dplyr::mutate(`48_DWP_tray_barcode` = as.numeric(`48_DWP_tray_barcode`)),
                   by = dplyr::join_by(`48_DWP_tray_barcode`, heart_position)) %>% 
  dplyr::count(otolith_info)
```

Interesting, 71 are DWP+Position and only 6 are vial otoliths.

This means that we have PGOD21 assignments to non-red cells. Need to go back into `data/raw_working_oto_sheets/DWP_maps_clean.xlsm` and add red formatting and re-run the VBA macro.

Use these code chunks to investigate individual DWPs.
...
PGOD21_48DWP_paired_genotype_matches %>% 
  ...
  dplyr::anti_join(y = red_cells_mod %>% 
                     dplyr::mutate(`48_DWP_tray_barcode` = as.numeric(`48_DWP_tray_barcode`)),
                   by = dplyr::join_by(`48_DWP_tray_barcode`, heart_position)) %>% 
  dplyr::filter(`48_DWP_tray_barcode` == 3088) %>% 
...
```{r}
AHRP_oceanAK %>% 
  dplyr::filter(dna_tray_code == "0000022683") %>% 
  dplyr::select(dna_tray_code, dna_tray_well_code, otolith_mark_present, otolith_mark_id, otolith_mark_status_code)
```

Some had the wrong red font, so the VBA didn't work, others did not have red font at all. Also noticed some DWPs with GOD issues, but 0 red cells and 0 otoliths transferred...7 of the 8 DWPs that Jodi Estrada transferred in-house never got DWP maps, so I made them up based on the transfer records and existing otolith reads. Those 7 DWPs were:  
...
Going to leave at 77 assignments from 54 DWPs until we finish investigating GOD samples with existing otolith reads. From there we can revisit this.

# Verify Otolith Transfers In GOD Wells

One way to check whether any entire DWPs are missing from the GOD wells is to cross-reference the DWPs in the otolith transfer records.

Any DWPs that were transferred, but not identified with red cells in "DWP maps"?
```{r}
dplyr::anti_join(
  x = PGOD21_48DWP_paired_genotype_matches %>%
...
Only 1 vial otolith transferred, no red cells in "DWP maps", discordant GOD assignment and existing otolith read. Decided to mark red cells and will ask for otolith data to be deleted.

### 2924

Only 1 vial otolith transferred, no red cells in "DWP maps". No GOD assignment. Decided to keep existing reads.

### 14360

11 vial otoliths transferred...but only 1 sample actually collected in the field. Probably an issue with the otolith transfer data.

### 14597

Only 1 vial otolith transferred, no red cells in "DWP maps". GOD assignment matches existing read. Decided to keep existing reads.

### 16127

Only 3 vial otoliths transferred, no red cells in "DWP maps". 1 GOD assignment, C5, 35, does not match existing read. Decided to mark C5 as red and will ask for otolith data to be deleted. Keep existing reads except C5.

### 22517

Only 1 vial otolith transferred, no red cells in "DWP maps". No GOD assignment. Keep existing reads.

### 22675

Only 1 vial otolith transferred, no red cells in "DWP maps". 1 GOD assignment, A4, 25, wild. All neighboring wells are wild. No existing read for A4. Decided to mark A4 red.

### 22696

Only 1 vial otolith transferred, no red cells in "DWP maps". No GOD assignment, didn't pass QA. Keep existing reads, do not mark any red cells.

### 22704

Only 1 vial otolith transferred, no red cells in "DWP maps". No GOD assignment, didn't meet thresholds. Keep existing reads, do not mark any red cells.

Otolith transfers.
```{r}
# PGOD21_48DWP_paired_genotype_matches %>% 
#   dplyr::filter(`48_DWP_tray_barcode` == 22704) %>% 
#   dplyr::arrange(heart_position)
```

Otolith transfer and assignments.
```{r}
# PGOD21_48DWP_paired_genotype_matches %>% 
#   dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
#   dplyr::anti_join(y = red_cells_mod %>% 
...
```{r}
# AHRP_oceanAK %>% 
#   dplyr::filter(dna_tray_code == "0000022704") %>% 
#   dplyr::select(dna_tray_code, dna_tray_well_code, otolith_mark_present, otolith_mark_id, otolith_mark_status_code)
```

Contractor data, check for missing otoliths in the field data.
```{r}
# stream_specimens %>% 
#   dplyr::filter(sample_tray_id == "0000022696",
#                 missing_otoliths > 0
#                 ) %>% 
#   dplyr::arrange(sample_cell) %>% 
#   dplyr::select(sample_tray_id, sample_cell, missing_otoliths)
```
...
## Join GOD Wells with Otolith Reads
...
Houston, we have a problem. There are *at least 194 GOD samples that have an existing otolith read*. None of these should have otolith reads...
...
See if we can determine from elimination by looking at the otolith transfer records `PGOD21_48DWP_paired_genotype_matches`.
...
According to the `DWP maps`, this DWP has 32 GOD wells...but somehow 10 of them do not have a `silly_code`?
```{r}
GOD_48DWP_wells_oceanak %>% 
...
There are now a total of **11,214** GOD wells, as of 2025-04-02.

## Investigate GOD Samples With Existing Otolith Reads

No GOD samples should have existing otolith reads.

Of these 194 samples, how are they spread across DWPs?
```{r}
GOD_48DWP_wells_oceanak %>% 
...
This DWP has complete otolith reads for nearly all 48 samples for PGILMOUR17 from 2017-09-06. The pattern of missing otoliths does not perfectly match "DWP maps", so I do not think that this DWP accidentally got put in the "unaffected" pile.
...
I do not have any record of GOD otoliths being transferred from the GOD DWP wells to 96-well otolith trays. Perhaps it accidently got put in the "unnafected" pile, but the pattern of missing otoliths in the read data does not match "DWP maps". Perhaps the "DWP maps" are off by a bit and some of the 0 wells actually had something in there? However, there were 5 vial otoliths from this DWP that did get transferred, 3 of which were assigned to hearts from PGILMOUR17_2900-2947.
...
I think this DWP got sorted into the *unaffected* pile and was read normally. Cross referencing with "DWP maps", it is missing data for the wells that had 0 otoliths, but otherwise has reads for most other wells.
...
This DWP was accidently put in the "unaffected" by GOD pile and read normally. Need to wipe data from GOD wells (red cells), keep existing reads otherwise.
```{r}
(
...
I think this DWP got sorted into the *unaffected* pile and was read normally. Cross referencing with "DWP maps", it is missing data for the well that had 0 otoliths (21) and wells with >2 otoliths (19, 27, 36, 37), but otherwise has reads for most other wells.

#### Conclusion

This DWP was accidentally put in the "unaffected" by GOD pile and read normally. Need to wipe data from GOD wells.
```{r}
(
  AHRP_wells_to_delete_otolith_data <- AHRP_wells_to_delete_otolith_data %>%
    dplyr::bind_rows(
...
Uhhh, these are **not** the wells that were supposed to be transferred based on "DWP maps"?
...
There are 16 missing otoliths. The 14 listed above in the transfers + A4 (0 in "DWP maps") and B4 (1 in "DWP maps"). I think we can just wipe the original, marked, GOD otoliths and they try to re-pair the otoliths that were actually transferred by mistake? I'll need to add the actual transfers to `GOD_48DWP_wells_oceanak` so we can re-pair them.
...
Remove original 14 GOD marked wells, replace with 14 mistakenly transferred wells.
```{r}
GOD_48DWP_wells_oceanak <- GOD_48DWP_wells_oceanak %>% 
  dplyr::filter(`48_DWP_tray_barcode` != 22679) %>%  # remove original, red marked GOD wells
  dplyr::bind_rows(PGOD21_48DWP_paired_genotype_matches %>% 
                     ...
                     dplyr::select(`48_DWP_tray_barcode`, `48_DWP_tray_position`) %>% 
...
See above under "Verify Otolith Transfers In GOD Wells"

    Only 1 vial otolith transferred, no red cells in "DWP maps", discordant GOD assignment and existing otolith read. Decided to mark red cells and will ask for otolith data to be deleted.

#### Conclusion

Delete otolith read data from 10 GOD marked wells.
```{r}
(
  AHRP_wells_to_delete_otolith_data <- AHRP_wells_to_delete_otolith_data %>%
    dplyr::bind_rows(
      GOD_48DWP_wells_oceanak %>%
        dplyr::filter(`48_DWP_tray_barcode` == 2839) %>%
        dplyr::select(dna_tray_code, dna_tray_well_code)
    ) %>%
    dplyr::arrange(dna_tray_code, dna_tray_well_code)
)
...
Okay, existing data matches the otolith transfer data, there are otolith reads for DWP columns 1-2, except for well 13, which was marked as 0 otoliths in "DWP maps".
...
It looks like we were expecting H6 to be missing 1 otolith (matches "DWP maps"), but A6 should have had 0 otoliths and "DWP maps" shows 2. However, the existing otolith read data shows that A6 (41) was missing both otoliths, matching the contractor data.
...
All remaining 46 DWPs have 3 or fewer GOD wells with otolith data and it is not worth the time to go through them individually. I'm making the executive decision to delete the existing otolith read data for these wells. This is the most conservative decision, better to have good data than more data.
```{r}
# Get 46 GOD DWPs with 3 or less wells that have existing otolith data
...
# Add these 57 DWP wells to the 172 to be deleted
(
  AHRP_wells_to_delete_otolith_data <- AHRP_wells_to_delete_otolith_data %>%
    dplyr::bind_rows(
      GOD_48DWP_wells_oceanak %>%
...
There are 229 samples (DWP wells) that need to have otolith data deleted.
...
(
  AHRP_wells_to_delete_otolith_data <- AHRP_wells_to_delete_otolith_data %>%
    ...
    dplyr::rename(GCL_silly_code = silly_code, GCL_fish_id = fish_id)
)
```
...
**229 wells across 53 different DWPs with existing otolith data to delete**
...
Now that we've trimmed which wells are in GOD and or not, let's return to verifying whether we have assigned any PGOD21 otoliths to non-GOD DWP wells. This will be a good double check to make sure we have correctly identified **ALL** the GOD wells. Some wells that were not marked for GOD, may have accidentally been transferred, becoming part of GOD.

The alternative is that our otolith transfer rules left some to be desired and perhaps we had some otoliths in PGOD21 that assigned to non-GOD wells that were deemed trustworthy.
```{r}
# wells
PGOD21_48DWP_paired_genotype_matches %>% 
  dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
  dplyr::anti_join(y = GOD_48DWP_wells_oceanak %>% 
                     dplyr::mutate(`48_DWP_tray_barcode` = as.numeric(`48_DWP_tray_barcode`)),
                   by = dplyr::join_by(`48_DWP_tray_barcode`, heart_position))
...
There are still 55 distinct DWPs that have a total of 68 PGOD21 genotype assignments to non-GOD wells.

Did those otoliths move? In other words, were these unmarked GOD wells from which otoliths were mistakenly transferred, but matched themselves?
```{r}
PGOD21_48DWP_paired_genotype_matches %>% 
  dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
  dplyr::anti_join(y = GOD_48DWP_wells_oceanak %>% 
                     dplyr::mutate(`48_DWP_tray_barcode` = as.numeric(`48_DWP_tray_barcode`)),
                   by = dplyr::join_by(`48_DWP_tray_barcode`, heart_position)) %>%  # matches not in GOD wells
  dplyr::count(otolith_moved)
```

## Add Unmoved PGOD21 Assignments To GOD Wells

It looks like otoliths from some unmarked wells accidentally got transferred for PGOD21. At this point, we only care if those PGOD21 otoliths were assigned to a heart genotype. If so, let's add the 41 unmoved otoliths (i.e. `otolith_moved = FALSE`) to `GOD_48DWP_wells_oceanak`. We'll filter out the remaining 23 moved + 4 NA otoliths in `07_verify-assigned-hearts-no-existing-otolith-read.Rmd`.

First, check and see if any have existing otolith reads.
```{r}
PGOD21_48DWP_paired_genotype_matches %>% 
  dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
  dplyr::anti_join(y = GOD_48DWP_wells_oceanak %>% 
                     dplyr::mutate(`48_DWP_tray_barcode` = as.numeric(`48_DWP_tray_barcode`)),
                   by = dplyr::join_by(`48_DWP_tray_barcode`, heart_position)) %>%  # matches not in GOD wells
  dplyr::filter(otolith_moved == FALSE) %>%  # only otoliths that did not move
  dplyr::select(`48_DWP_tray_barcode`, heart_position, oto_tray_and_well) %>%
    dplyr::mutate(
      dna_tray_code = stringr::str_pad(string = `48_DWP_tray_barcode`, side = "left", width = 10, pad = "0")
    ) %>%  # need 10 digit barcode, including leading 0's
    dplyr::left_join(y = pos_to_code, by = dplyr::join_by(heart_position == pos)) %>%
...
There are 4 of these 41 unmoved otoliths that have existing otolith reads. I manually checked with the GOD reads and they are the same!

Add those 4 unmoved otoliths with existing otolith reads to `AHRP_wells_to_delete_otolith_data`
```{r}
(
  AHRP_wells_to_delete_otolith_data <- AHRP_wells_to_delete_otolith_data %>%
    dplyr::select(dna_tray_code, dna_tray_well_code) %>%
    dplyr::bind_rows(
      PGOD21_48DWP_paired_genotype_matches %>%
        dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
        dplyr::anti_join(
          y = GOD_48DWP_wells_oceanak %>%
            dplyr::mutate(`48_DWP_tray_barcode` = as.numeric(`48_DWP_tray_barcode`)),
          by = dplyr::join_by(`48_DWP_tray_barcode`, heart_position)
        ) %>%  # matches not in GOD wells
        dplyr::filter(otolith_moved == FALSE) %>%  # only otoliths that did not move
        dplyr::select(`48_DWP_tray_barcode`, heart_position, oto_tray_and_well) %>%
        dplyr::mutate(
          dna_tray_code = stringr::str_pad(
...
Add to `GOD_48DWP_wells_oceanak`.
```{r}
GOD_48DWP_wells_oceanak <-
  GOD_48DWP_wells_oceanak %>%
  dplyr::bind_rows(
    PGOD21_48DWP_paired_genotype_matches %>%
      ...
      dplyr::filter(otolith_moved == FALSE) %>%  # only otoliths that did not move (i.e., self assignments)
      dplyr::select(`48_DWP_tray_barcode`, heart_position) %>%
      dplyr::mutate(
        dna_tray_code = stringr::str_pad(
          string = `48_DWP_tray_barcode`,
...
Confirm that we've dealt with all GOD wells that had otolith read data, once we delete untrustworthy samples.
```{r}
GOD_48DWP_wells_oceanak %>% 
  dplyr::anti_join(y = AHRP_wells_to_delete_otolith_data, by = dplyr::join_by(dna_tray_code, dna_tray_well_code)) %>%  # remove wells to be deleted from SALMON_BIO_FACT
  dplyr::filter(!is.na(otolith_mark_present))  # any remaining GOD wells left with otolith reads?
...
### All Assignments to GOD Wells

All PGOD21 assignments in GOD wells (except remaining 23 moved + 4 NA otoliths)
```{r}
# wells
PGOD21_48DWP_paired_genotype_matches %>% 
  dplyr::filter(!is.na(heart_position)) %>%  # filter for assignments
...
Great, that is the same 27 we expect. We'll deal with those in `07_verify-assigned-hearts-no-existing-otolith-read.Rmd`.

### No Duplicates

Remove any duplicated rows?
```{r}
nrow(GOD_48DWP_wells_oceanak) == GOD_48DWP_wells_oceanak %>% dplyr::distinct(`48_DWP_tray_barcode`, heart_position) %>% nrow()
```

## GOD Wells to Delete Otolith Data

Verify all have existing otolith reads and no duplicates.
```{r}
# what are reads for wells to delete
AHRP_wells_to_delete_otolith_data %>% 
  dplyr::distinct(dna_tray_code, dna_tray_well_code) %>%  # no duplicates
  dplyr::left_join(y = AHRP_oceanAK, by = dplyr::join_by(dna_tray_code, dna_tray_well_code)) %>%  # join in existing otolith data
  dplyr::count(otolith_mark_present)  # all have reads?
```

Yikes, 42 of our 233 wells do not have existing otolith read data. Dig into these 42 quickly.
```{r}
# filter for 42 wells
AHRP_wells_to_delete_otolith_data %>% 
  dplyr::distinct(dna_tray_code, dna_tray_well_code) %>%  # no duplicates
...
Okay, after spot checking a few of these above, I just included all GOD wells and did not filter for only those with reads. Correct here and then write out.
```{r}
# remove wells without existing otolith reads
(
  AHRP_wells_to_delete_otolith_data <- AHRP_wells_to_delete_otolith_data %>%
    dplyr::distinct() %>%  # no duplicates
    dplyr::left_join(
      y = AHRP_oceanAK,
      by = dplyr::join_by(dna_tray_code, dna_tray_well_code, sample_id)
    ) %>%  # join in existing otolith data
    dplyr::filter(!is.na(otolith_mark_present)) %>%
    dplyr::select(
      dna_tray_code,
      dna_tray_well_code,
...
How many per collection?
```{r}
AHRP_wells_to_delete_otolith_data %>% 
  dplyr::count(GCL_silly_code)
```
...
# Otolith reads to delete, untrustworthy
