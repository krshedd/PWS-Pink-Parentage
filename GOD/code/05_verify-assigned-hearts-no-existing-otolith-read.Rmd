...
# Verify Genotype Matches

Filter for genotype matches, see if there are existing otolith reads.
```{r}
PGOD21_48DWP_paired_genotype_matches_join_oceanak <- PGOD21_48DWP_paired_genotype_matches %>%
  dplyr::filter(genotype_match_assigned) %>%  # filter for matches to PGOD21 otolith
  dplyr::left_join(
...
## Any Existing Otolth Reads?

Do we even have a problem? Are there otoliths assigned to hearts with existing otolith reads?
```{r}
PGOD21_48DWP_paired_genotype_matches_join_oceanak %>% 
...
Yes, we do have a problem. It looks like 51 individuals already have valid otolith reads. There are a handful of other individuals with different `otolith_mark_status_codes`, however, they all indicate that the other otolith was unavailable/missing (i.e., 4N26 - unreadable, none, overgrind left, no right otolith).

Filter for these 51 individuals.
```{r}
PGOD21_48DWP_paired_genotype_matches_join_oceanak %>% 
  dplyr::filter(!is.na(otolith_mark_present))
```

Verify that there are no DWP issues.
```{r}
PGOD21_48DWP_paired_genotype_matches_join_oceanak %>% 
  dplyr::filter(!is.na(otolith_mark_present)) %>% 
  dplyr::filter(`48_DWP_tray_barcode` != as.numeric(dna_tray_code))
```

Dig in to some of these based on the heart `dna_tray_code` + `dna_tray_well_code`.

### 14297_24

This otolith did not move, both it and the heart were in well H3. However, the heart assigned to an otolith in E4. Looking at <"output\otolith_shift_DWP_visualization_dup-rate-0.8_diff-0.25.pdf">, almost all of the otoliths in this DWP stayed put. Seems legit.

### 22108_27

This otolith did not move, both it and the heart were in well C4. However, the heart assigned to an otolith in D4.

### 14573_33

This otolith was on an edge and did not move from well A5. However, the heart assigned to an otolith in B5.

### 21938_39

This otolith did not move, and shouldn't have been read as it was slated for GOD. Regardless, the otolith matched the same heart in PGOD, so we are good!

# Remove Genotype Matches With Existing Otolith Reads
...
Wipe out `heart_position` for these 51 individuals with existing otolith reads and add a boolean column for documentation.
```{r}
hearts_existing_otolith_read <- PGOD21_48DWP_paired_genotype_matches_join_oceanak %>%
  dplyr::filter(!is.na(otolith_mark_present)) %>% 
  dplyr::pull(heart_fish_id)

PGOD21_48DWP_paired_genotype_matches_clean <- PGOD21_48DWP_paired_genotype_matches %>%
  dplyr::mutate(existing_otolith_read = heart_fish_id %in% hearts_existing_otolith_read,
                heart_position = dplyr::case_when(existing_otolith_read == TRUE ~ NA_character_,
                                                  TRUE ~ heart_position)) %>% 
  dplyr::relocate(existing_otolith_read, .after = otolith_moved)
```

Verify this did as intended.
```{r}
PGOD21_48DWP_paired_genotype_matches_clean %>% 
  dplyr::count(existing_otolith_read)

PGOD21_48DWP_paired_genotype_matches_clean %>% 
  dplyr::filter(!is.na(heart_position))
```

Yup, down from 5,421 matches to 5,370 (51 removed).

Save output. Do not create a new file, just overwrite existing.
```{r}
readr::write_csv(x = PGOD21_48DWP_paired_genotype_matches_clean, file = "../output/PGOD21_48DWP_paired_genotype_matches.csv")
```
