---
title: "06 - Identify Wells In GOD"
...
The purpose of this notebook is to identify which DWP wells were part of GOD. We already have the DWP well data for where GOD otoliths ended up (i.e., where they were removed from at MTAL for genotyping/otolith reading in `GOD/output/PGOD21_48DWP_paired.csv`), but we do not have a tidy, data friendly version of where they started. We will extract this info from the DWP maps in `GOD/data/raw_working_oto_sheets.xlsm`, which denote GOD wells with red font.

The next steps after this will be to *investigate GOD otoliths with reads but no heart match* to make additional assignments, then *double check that we have not assigned otoliths to hearts that already have otolith reads* (repeat 05_verify), and ultimately *submit finalized PGOD21 matches to Eric Lardizabal*. 
...
**NOTE** `../data/raw_working_oto_sheets/DWP_maps_clean.xlsx` is a copy from `../data/raw_working_oto_sheets/working_oto_sheet_combined.xlsm`.
```{r}
(
  DWP_maps <- readxl::read_excel(
    path = "../data/raw_working_oto_sheets/DWP_maps_clean.xlsx",
    sheet = "DWP maps",
    range = "A1:G8073",
...
Get the Excel cells with red font that denote GOD DWP wells.
```{r}
(
...
Write out.
```{r}
GOD_48DWP_wells <- red_cells_mod %>% 
  dplyr::select(`48_DWP_tray_barcode`, heart_position)

readr::write_csv(x = GOD_48DWP_wells, file = "../output/GOD_48DWP_wells.csv")
```

# Compare GOD Wells to Salmon Biological Fact

Make sure that there are no otolith reads for these GOD DWP wells.
...
(
  GOD_48DWP_wells_oceanak <- GOD_48DWP_wells %>%
    ...
    dplyr::left_join(
      y = AHRP_oceanAK,
      by = dplyr::join_by(dna_tray_code, code == dna_tray_well_code)
    )
)
...
Houston, we have a problem. There are *at least 180 GOD samples that have an existing otolith read*. None of these should have otolith reads...
...
WTF are these 218 samples without a `silly_code`?

## Investigate GOD Samples With Otolith Reads

Of these 180 samples, how are they spread across DWPs?
```{r}
GOD_48DWP_wells_oceanak %>% 
...
The next steps after this will be to *investigate GOD otoliths with reads but no heart match*, using `GOD_48DWP_wells` to determine how many wells have unpaired heart/otolith data and see if we can assign remaining PGOD21 otolith reads to wells not genetically assigned to a heart.

# Summary

We output a data friendly version of the 10,898 GOD DWP wells.
