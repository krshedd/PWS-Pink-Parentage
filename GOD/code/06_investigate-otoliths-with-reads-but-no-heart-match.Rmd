...
In order to verify that GOD genotype matches are appropriate *and* assign PGOD21 otoliths to wells without a genotype match based on logic (i.e., all remaining otoliths are the same read), we need to know which wells were in GOD. There were 10,898 cells marked with red font in `data/raw_working_oto_sheets/working_oto_sheet_combined.xlsm` (aka `data/raw_working_oto_sheets/DWP_maps_clean.xlsm`), which were ostensibly GOD DWP wells. I removed unsampled wells (partially filled DWPs, otoliths migrated to unsampled wells) and any wells that had existing otolith data that were deemed to be trustworthy. This leaves 10,691 wells that are actually GOD wells.
...
Total of 10,691 DWP wells (samples) in GOD.
...
As of 2025-03-20, Kristen and I decided that it is not worth the effort to hunt and peck, going through each DWP 1 by 1 to see if we feel comfortable making additional assignments. We need to cut our losses and move on.
...
If the number of unassigned GOD otoliths is >= the number of unassigned GOD wells **and** they are all the same read type (`mark_present` + `mark_id`), then we can assign them without a genotype match.
...
There are 57 DWPs that have all remaining, unassigned GOD otoliths of a single read type. How many samples is that?
```{r}
PGOD21_48DWP_paired_genotype_matches_otolith_reads %>% 
