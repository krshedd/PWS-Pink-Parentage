...
2025-04-07 Eric L. said that he needs the `GOD_DETERMINATION_COLLECTION` info for all the target collections (i.e., PERB15, etc.).

Verify that all GOD Target `CONTAINER_ARRAY_TYPE_ID` is "DWP48" (aka CONTAINER_ARRAY_TYPE_ID = 2).
```{r}
target_tissue_tables <- GCLr::get_tissue_data(
  sillyvec = c(
    "PERB15",
    "PERB16",
    "PERB17",
  ...
  PGOD21_tissues_GOD <- PGOD21_tissues %>%
    ...
    dplyr::left_join(
      y = PGOD21_48DWP_paired_genotype_logic_matches_final %>% dplyr::mutate(
        TARGET_DNA_TRAY_CODE = stringr::str_pad(
        ...
        dplyr::select(SillySource, TARGET_DNA_TRAY_CODE, TARGET_DNA_TRAY_WELL_POS = heart_position),
      by = "SillySource"
    ) %>%
    dplyr::mutate(
      TARGET_CONTAINER_ARRAY_TYPE_ID = dplyr::case_when(is.na(TARGET_DNA_TRAY_WELL_POS) ~ NA_real_, TRUE ~ 2),
      TARGET_DNA_TRAY_CODE = dplyr::case_when(
        is.na(TARGET_DNA_TRAY_WELL_POS) ~ NA_character_,
        TRUE ~ TARGET_DNA_TRAY_CODE
      )
    ) %>%
    dplyr::relocate(TARGET_CONTAINER_ARRAY_TYPE_ID, .before = TARGET_DNA_TRAY_CODE) %>%
    dplyr::select(-SillySource)
)
```

Confirm there are still 5,520 matches
```{r}
PGOD21_tissues_GOD %>% 
  dplyr::filter(!is.na(TARGET_DNA_TRAY_WELL_POS)) %>% 
  nrow()
```

Solid, this matches.

## Target Tissue Tables

Need to add the Determination Collection for all samples with assignments.

Create a key for each DWP barcode _ position.
```{r}
PGOD21_assignments_DWP_key <- PGOD21_tissues_GOD %>% 
  tidyr::unite(
      col = "DWP_key",
      TARGET_DNA_TRAY_CODE:TARGET_DNA_TRAY_WELL_POS,
      sep = "_",
      remove = FALSE,
      na.rm = TRUE
    ) %>%  # make key
  dplyr::mutate(DWP_key = dplyr::na_if(DWP_key, "")) %>% 
  dplyr::filter(!is.na(DWP_key)) %>% 
  dplyr::pull(DWP_key)
```

Update target tissue tables.
```{r}
(
  target_tissue_tables_PGOD21 <- target_tissue_tables %>%
    tidyr::unite(
      col = "DWP_key",
      DNA_TRAY_CODE:DNA_TRAY_WELL_POS,
      sep = "_",
      remove = FALSE
    ) %>%  # make key
    dplyr::mutate(
      GOD_DETERMINATION_COLLECTION_ID = dplyr::case_when(
        DWP_key %in% PGOD21_assignments_DWP_key &
          PK_TISSUE_TYPE != "Otolith" ~ 55345,
        TRUE ~ NA_real_
      )
    )
)
...
Verify that all 5,520 samples are assigned.
```{r}
target_tissue_tables_PGOD21 %>% 
  dplyr::filter(!is.na(GOD_DETERMINATION_COLLECTION_ID))
```
...
readr::write_csv(x = target_tissue_tables_PGOD21, file = "../output/target_tissue_tables_GOD_20250407.csv")
```
...
Of the 5,520 final assignments, 5,394 were genotype and 126 were logic.

## By Silly

How many assignments by silly code?
```{r}
PGOD21_48DWP_paired_genotype_logic_matches_final %>% 
  dplyr::mutate(match_assigned = dplyr::case_when(!is.na(heart_position) ~ TRUE,
                                                  TRUE ~ FALSE)) %>% 
  dplyr::group_by(silly_code) %>% 
  dplyr::count(match_assigned) %>% 
  dplyr::mutate(proportion = n / sum(n)) %>% 
  dplyr::filter(match_assigned == TRUE)
```
...
Out of the 5,520 PGOD21 assignments, we have valid otolith reads for **5,064** (47%) of the 10,816 otoliths in PGOD21.

# Summary

Of the 10,816 samples in PGOD21, 10,791 have genotypes, 9,397 of which made it through genotype QA. In total, 5,520 PGOD21 otoliths were assigned to samples involved in GOD, 5,394 were genotype assignments and 126 were logic assignments. Of the 5,520 PGOD21 assignments, 5,064 have valid otolith read data.
