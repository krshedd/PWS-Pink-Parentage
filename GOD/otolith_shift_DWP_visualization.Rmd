...
```{r}
(matchy_data <- read_csv(file = "dup_check_results_DWP_1st12_pass_with_wells.csv"))
```
...
(
  matchy_data_mod <- matchy_data %>%
    ...
    mutate(
      OG_row = factor(x = OG_row, levels = LETTERS[8:1]),
      row = factor(x = row, levels = LETTERS[8:1]),
      OG_col = factor(x = OG_col, levels = as.character(1:6)),
...
sapply(unique(matchy_data_mod$`48_DWP_tray_barcode`), function(DWP) {
  matchy_data_mod %>%
    ...
    ggplot() +
    geom_point(aes(x = OG_col, y = OG_row, colour = "OG"),
               cex = 8,
               alpha = 0.5) +
