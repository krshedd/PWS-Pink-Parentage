...
.username <- "kgruenthal"
.password <- ""
source("c:/Users/kgruenthal/Documents/R/Functions.GCL.R")
```
...
loci298 <- dget("../../Objects/Gilmour_14_16/loci298.txt")
...
```{r}
table(PGILMOUR14.gcl$attributes$SillySource %in% stock_oceanak$silly_source)
table(PGILMOUR16.gcl$attributes$SillySource %in% stock_oceanak$silly_source)
```
...
We want to remove any NA from 2014 and hathcery-origin fish + NA from 2016. We'll need those hatchery-origin fish from 2016 for looking at 2016/2018 parentage, but not for 2014/2016. The NA's must have snuck in there from second reads?
...
dup_16n_ids <- filter(dups, silly == "PGILMOUR14n") %>% pull(id)
# dup_16h_ids <- filter(dups, silly == "PGILMOUR14h") %>% pull(id)  # none
dup_18_ids <- filter(dups, silly == "PGILMOUR16") %>% pull(id)

# Save attributes for posterity
# ID silly sources for duplicate individuals
dup_16n <- PGILMOUR14n.gcl$attributes %>% 
  filter(FK_FISH_ID %in% dup_16n_ids) %>% 
  mutate(FK_FISH_ID = as.numeric(FK_FISH_ID))
dup18 <- PGILMOUR16.gcl$attributes %>%  
...
RemoveIDs.GCL(silly = "PGILMOUR14n", IDs = dup_16n_ids)
RemoveIDs.GCL(silly = "PGILMOUR16", IDs = dup_18_ids)
