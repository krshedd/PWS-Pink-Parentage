---
title: "Gilmour 2014/2016 Exploratory Analysis with 298 Markers"
output:
  html_notebook:
    theme: united
    toc: yes
...
.username <- "krshedd"  # revise in console, if needed
.password <- ""
...
This is a quick exploratory analysis of the Gilmour 2014/2016 genotype data. Here we will:
...
```{r loki}
sillyvec <- ProjectSillys <- c("PGILMOUR14", "PGILMOUR16")
...
if(!dir.exists("../../Objects/Gilmour_14_16/")) {dir.create("../../Objects/Gilmour_14_16/")}
save_objects(objects = c("LocusControl", "loci", "sillyvec", "ProjectSillys"), path = "../../Objects/Gilmour_14_16/")

if(!dir.exists("../../Genotypes/Gilmour_14_16/")) {dir.create("../../Genotypes/Gilmour_14_16/")}
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Gilmour_14_16/")
```
# OceanAK
...
gilmour_oceanak <- oceanak %>% 
  dplyr::filter(`Location Code` == "Gilmour Creek" & `Sample Year` %in% c(2014, 2016)) %>% 
...
```{r}
table(PGILMOUR14.gcl$attributes$SillySource %in% gilmour_oceanak$silly_source)
...
```{r}
names(PGILMOUR14.gcl$attributes)
```
...
PGILMOUR14.gcl$attributes <- PGILMOUR14.gcl$attributes %>% 
...
if(!dir.exists("../../Genotypes/Gilmour_14_16/OceanAK")) {dir.create("../../Genotypes/Gilmour_14_16/OceanAK")}
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Gilmour_14_16/OceanAK")
```


## Split by otolith-origin

First we need to verify that we only have hatchery and natural-origin fish for the **F0** year (2014), and only natural-origin fish for the **F1** year (2016). **However** at the moment we do not have full otolith reads for the 2016 Gilmour samples, as they were part of the GOD event. We are going to just include all of them for now, as hatchery fish shouldn't assign to anyone.
```{r view_origin}
dplyr::bind_rows(PGILMOUR14.gcl$attributes, 
                 PGILMOUR16.gcl$attributes) %>% 
  count(SILLY_CODE, `Otolith Mark Present`) %>% 
  spread(`Otolith Mark Present`, n, fill = 0)
```

We want to remove any NA from 2014 and hatchery-origin fish, **but keep NA from 2016**. We are going to leave all 2016 fish in for now, including the 39 known hatchery-origin fish, since we don't know the origin of 3,762 samples!
```{r remove_stock_hatchery}
# PGILMOUR14_ids_remove <- PGILMOUR14.gcl$attributes %>% 
#   filter(is.na(`Otolith Mark Present`)) %>% 
#   mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
#   pull(FK_FISH_ID)
# RemoveIDs.GCL(silly = "PGILMOUR14", IDs = PGILMOUR14_ids_remove)
...
We will split 2014 otolith-origin for QA tabulating purposes. Also create a new *sillyvec*.
```{r split_stock15}
PGILMOUR14_ids_natural <- list("PGILMOUR14" = PGILMOUR14.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "NO") %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID))
PoolCollections.GCL(collections = "PGILMOUR14", loci = loci, IDs = PGILMOUR14_ids_natural, newname = "PGILMOUR14n")

PGILMOUR14_ids_hatchery <- list("PGILMOUR14" = PGILMOUR14.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "YES") %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID))
PoolCollections.GCL(collections = "PGILMOUR14", loci = loci, IDs = PGILMOUR14_ids_hatchery, newname = "PGILMOUR14h")

ProjectSillys_Origin <- c("PGILMOUR14n", "PGILMOUR14h", "PGILMOUR16")
if(!dir.exists("../../Genotypes/Gilmour_14_16/OceanAK_Origin")) {dir.create("../../Genotypes/Gilmour_14_16/OceanAK_Origin")}
save_sillys(ProjectSillys_Origin, "../../Genotypes/Gilmour_14_16/OceanAK_Origin/")
```
...
```{r failure_rate}
project <- "Gilmour_14_16" #this is required of the function; can be any name 
failure_rate <- FailureRate.GCL(sillyvec = ProjectSillys_Origin)
failure_rate_noplots <- failure_rate[1:4]
save_objects("failure_rate_noplots", "../../Objects/Gilmour_14_16")
failure_rate
```
...
save_objects("MissLoci", path = "../../Objects/Gilmour_14_16/")
...
save_objects("DuplicateCheckReportSummary", path = "../../Objects/Gilmour_14_16/")
```
...
save_objects("duplicates_to_remove", path = "../../Objects/Gilmour_14_16/")

# Remove fish from .gcl objects
RemoveIDs.GCL(silly = "PGILMOUR14n", IDs = dup_14n_ids)
...
PGILMOUR14n_ind_het <- apply(PGILMOUR14n.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
PGILMOUR14h_ind_het <- apply(PGILMOUR14h.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
...
# Convert to tibbles
tibble_PGILMOUR14n <- tibble(silly=PGILMOUR14n.gcl$attributes$SILLY_CODE, sillysource=PGILMOUR14n.gcl$attributes$SillySource, het=PGILMOUR14n_ind_het)
tibble_PGILMOUR14h <- tibble(silly=PGILMOUR14h.gcl$attributes$SILLY_CODE, sillysource=PGILMOUR14h.gcl$attributes$SillySource, het=PGILMOUR14h_ind_het)
...
het_tibble <- bind_rows(tibble_PGILMOUR14n, tibble_PGILMOUR14h, tibble_PGILMOUR16)
...
het_tibble <- het_tibble %>% 
  dplyr::mutate(lineage = dplyr::case_when(silly %in% c("PGILMOUR14", "PGILMOUR16") ~ "even",
                           TRUE ~ "even"))
save_objects("het_tibble", "../../Objects/Gilmour_14_16/")
...
How many fish would we lose if we went with the +-3SD cutoffs?
...
save_objects("removed_hets", "../../Objects/Gilmour_14_16/")

PGILMOUR14h_hets_remove <- PGILMOUR14h.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PGILMOUR14h", IDs = PGILMOUR14h_hets_remove)

PGILMOUR14n_hets_remove <- PGILMOUR14n.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PGILMOUR14n", IDs = PGILMOUR14n_hets_remove)
...
save_objects("ProjectSillys_Origin_SampleSizes", path = "../../Objects/Gilmour_14_16/")
```

Save post-QA genotypes.
```{r save_postQA_genotypes}
if(!dir.exists("../../Genotypes/Gilmour_14_16/OceanAK_Origin_PostQA")) {dir.create("../../Genotypes/Gilmour_14_16/OceanAK_Origin_PostQA")}
save_sillys(ProjectSillys_Origin, "../../Genotypes/Gilmour_14_16/OceanAK_Origin_PostQA/")
```
...
```{r genepop}
if(!dir.exists("../../Genepop/Gilmour_14_16")) {dir.create("../../Genepop/Gilmour_14_16")}

gcl2Genepop.GCL(sillyvec = c("PGILMOUR14n", "PGILMOUR14h", "PGILMOUR16"), 
                loci = loci, 
                path = "../../Genepop/Gilmour_14_16/Gilmour_14_16_postQA.gen", 
                VialNums = TRUE, 
                usat = FALSE)
```
...
genepop2franz.GCL(
  Genepop = "../../Genepop/Gilmour_14_16/Gilmour_14_16_postQA.gen",
