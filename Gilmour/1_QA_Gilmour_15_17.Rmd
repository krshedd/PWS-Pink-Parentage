---
title: "Gilmour 2015/2017 Exploratory Analysis with 298 Markers"
...
This is a quick exploratory analysis of the Gilmour 2015/2017 genotype data. Here we will:
...
```{r loki}
sillyvec <- ProjectSillys <- c("PGILMOUR15", "PGILMOUR17")
...
if(!dir.exists("../../Objects/Gilmour_15_17/")) {dir.create("../../Objects/Gilmour_15_17/")}
save_objects(objects = c("LocusControl", "loci", "sillyvec", "ProjectSillys"), path = "../../Objects/Gilmour_15_17/")

if(!dir.exists("../../Genotypes/Gilmour_15_17/")) {dir.create("../../Genotypes/Gilmour_15_17/")}
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Gilmour_15_17/")
```
...
Gilmour_oceanak <- oceanak %>% 
  dplyr::filter(`Location Code` == "Gilmour Creek" & `Sample Year` %in% c(2015, 2017)) %>% # Stream options: Erb Creek, Gilmour Creek, Hogan Creek, Paddy Creek, Stockdale Creek
...
```{r}
table(PGILMOUR15.gcl$attributes$SillySource %in% Gilmour_oceanak$silly_source)
...
```{r}
names(PGILMOUR15.gcl$attributes)
```
...
PGILMOUR15.gcl$attributes <- PGILMOUR15.gcl$attributes %>% 
...
if(!dir.exists("../../Genotypes/Gilmour_15_17/OceanAK")) {dir.create("../../Genotypes/Gilmour_15_17/OceanAK")}
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Gilmour_15_17/OceanAK")
```

## Split by otolith-origin

First, we need to verify that we only have hatchery and natural-origin fish for the parental year (2015), and only natural-origin fish for the offspring year (2017).
```{r view_origin}
dplyr::bind_rows(PGILMOUR15.gcl$attributes, 
                 PGILMOUR17.gcl$attributes) %>% 
  count(SILLY_CODE, `Otolith Mark Present`) %>% 
  spread(`Otolith Mark Present`, n, fill = 0)
```
**Results:** There are **thousands** of NAs from both years due to the GOD event.

Remove any NAs from 2015 and hatchery-origin fish + NA from 2017 - NAs must be from second reads. Hatchery-origin fish from 2015 are retained for looking at 2015/2017 parentage. **NOTE:** We're not going to do this step because it would decimate the dataset; we're going to leave everyone in until we have otolith reads.
```{r remove_Gilmour_hatchery}
# PGILMOUR15_ids_remove <- PGILMOUR15.gcl$attributes %>%
#   filter(is.na(`Otolith Mark Present`)) %>%
#   mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>%
#   pull(FK_FISH_ID)
# RemoveIDs.GCL(silly = "PGILMOUR15", IDs = PGILMOUR15_ids_remove)
# 
...
Split 2015 otolith-origin for QA tabulating purposes and create a new *sillyvec*
```{r split_stock15}
# PGILMOUR15_ids_natural <- list("PGILMOUR15" = PGILMOUR15.gcl$attributes %>% 
#   filter(`Otolith Mark Present` == "NO") %>% 
#   mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
#   pull(FK_FISH_ID))
# PoolCollections.GCL(collections = "PGILMOUR15", loci = loci, IDs = PGILMOUR15_ids_natural, newname = "PGILMOUR15n")
# 
# PGILMOUR15_ids_hatchery <- list("PGILMOUR15" = PGILMOUR15.gcl$attributes %>% 
#   filter(`Otolith Mark Present` == "YES") %>% 
#   mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
#   pull(FK_FISH_ID))
# PoolCollections.GCL(collections = "PGILMOUR15", loci = loci, IDs = PGILMOUR15_ids_hatchery, newname = "PGILMOUR15h")
# 
# ProjectSillys_Origin <- c("PGILMOUR15n", "PGILMOUR15h", "PGILMOUR17")
# if(!dir.exists("../../Genotypes/Gilmour_15_17/OceanAK_Origin")) {dir.create("../../Genotypes/Gilmour_15_17/OceanAK_Origin")}
# save_sillys(ProjectSillys_Origin, "../../Genotypes/Gilmour_15_17/OceanAK_Origin/")
```
...
```{r failure_rate}
project <- "Gilmour_15_17" #this is required of the function; can be any name 
...
save_objects("failure_rate_noplots", "../../Objects/Gilmour_15_17")
failure_rate
```
**Results:** There's a **5%** genotype failure rate in PGILMOUR15 and **<1%** in PGILMOUR17, with an overall rate of **3%*. The data look pretty good. 
...
save_objects("MissLoci", path = "../../Objects/Gilmour_15_17/")
...
```
**Results:** Removed **50** fish IDs from PGILMOUR15 and **428** from PGILMOUR17.
...
save_objects("DuplicateCheckReportSummary", path = "../../Objects/Gilmour_15_17/")
```
**Results:** There were **five** duplicates in PGILMOUR15 and **six** in PGILMOUR17.
...
dup_15_ids <- filter(dups, silly == "PGILMOUR15") %>% pull(id)
# dup_15n_ids <- filter(dups, silly == "PGILMOUR15n") %>% pull(id)
# dup_15h_ids <- filter(dups, silly == "PGILMOUR15h") %>% pull(id)
dup_17_ids <- filter(dups, silly == "PGILMOUR17") %>% pull(id)

# Save attributes for posterity
# ID silly sources for duplicate individuals
dup_15 <- PGILMOUR15.gcl$attributes %>% 
  filter(FK_FISH_ID %in% dup_15_ids) %>% 
  mutate(FK_FISH_ID = as.numeric(FK_FISH_ID))
# dup_15n <- PGILMOUR15n.gcl$attributes %>% 
#   filter(FK_FISH_ID %in% dup_15n_ids) %>% 
#   mutate(FK_FISH_ID = as.numeric(FK_FISH_ID))
# dup_15h <- PGILMOUR15h.gcl$attributes %>%
#   filter(FK_FISH_ID %in% dup_15h_ids) %>%
#   mutate(FK_FISH_ID = as.numeric(FK_FISH_ID))
dup17 <- PGILMOUR17.gcl$attributes %>%  
  filter(FK_FISH_ID %in% dup_17_ids)

# One tibble with all attributes for fish to remove
duplicates_to_remove <- bind_rows(dup_15, dup17)
# duplicates_to_remove <- bind_rows(dup_15n, dup_15h, dup17)
save_objects("duplicates_to_remove", path = "../../Objects/Gilmour_15_17/")

# Remove fish from .gcl objects
RemoveIDs.GCL(silly = "PGILMOUR15", IDs = dup_15_ids)
# RemoveIDs.GCL(silly = "PGILMOUR15n", IDs = dup_15n_ids)
# RemoveIDs.GCL(silly = "PGILMOUR15h", IDs = dup_15h_ids)
...
PGILMOUR15_ind_het <- apply(PGILMOUR15.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
# PGILMOUR15n_ind_het <- apply(PGILMOUR15n.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
# PGILMOUR15h_ind_het <- apply(PGILMOUR15h.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
PGILMOUR17_ind_het <- apply(PGILMOUR17.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} )

# Convert to tibbles
tibble_PGILMOUR15 <- tibble(silly=PGILMOUR15.gcl$attributes$SILLY_CODE, sillysource=PGILMOUR15.gcl$attributes$SillySource, het=PGILMOUR15_ind_het)
# tibble_PGILMOUR15n <- tibble(silly=PGILMOUR15n.gcl$attributes$SILLY_CODE, sillysource=PGILMOUR15n.gcl$attributes$SillySource, het=PGILMOUR15n_ind_het)
# tibble_PGILMOUR15h <- tibble(silly=PGILMOUR15h.gcl$attributes$SILLY_CODE, sillysource=PGILMOUR15h.gcl$attributes$SillySource, het=PGILMOUR15h_ind_het)
...
het_tibble <- bind_rows(tibble_PGILMOUR15, tibble_PGILMOUR17)
# het_tibble <- bind_rows(tibble_PGILMOUR15n, tibble_PGILMOUR15h, tibble_PGILMOUR17)
```
...
het_tibble <- het_tibble %>% 
  dplyr::mutate(lineage = dplyr::case_when(silly %in% c("PGILMOUR15", "PGILMOUR17") ~ "odd",
                           TRUE ~ "odd"))
save_objects("het_tibble", "../../Objects/Gilmour_15_17/")
...
het_tibble %>% 
  ...
  dplyr::count(silly)
```
**Results:** We'll remove another **443** fish total.
...
save_objects("removed_hets", "../../Objects/Gilmour_15_17/")

PGILMOUR15_hets_remove <- PGILMOUR15.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PGILMOUR15", IDs = PGILMOUR15_hets_remove)

# PGILMOUR15h_hets_remove <- PGILMOUR15h.gcl$attributes %>% 
#   dplyr::filter(SillySource %in% removed_hets) %>% 
#   dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
#   dplyr::pull(FK_FISH_ID)
# RemoveIDs.GCL(silly = "PGILMOUR15h", IDs = PGILMOUR15h_hets_remove)
# 
# PGILMOUR15n_hets_remove <- PGILMOUR15n.gcl$attributes %>% 
#   dplyr::filter(SillySource %in% removed_hets) %>% 
#   dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
#   dplyr::pull(FK_FISH_ID)
# RemoveIDs.GCL(silly = "PGILMOUR15n", IDs = PGILMOUR15n_hets_remove)
...
save_objects("ProjectSillys_SampleSizes", path = "../../Objects/Gilmour_15_17/")
...
# save_objects("ProjectSillys_Origin_SampleSizes", path = "../../Objects/Gilmour_15_17/")
```
**Results:** Final dataset contains **10,785** total fish: **5,075** from 2015 and **5,710** from 2017 (that's a lot!).

Save the post-QA genotypes
```{r save_postQA_genotypes}
if(!dir.exists("../../Genotypes/Gilmour_15_17/OceanAK_PostQA")) {dir.create("../../Genotypes/Gilmour_15_17/OceanAK_PostQA")}
save_sillys(ProjectSillys, "../../Genotypes/Gilmour_15_17/OceanAK_PostQA/")
# if(!dir.exists("../../Genotypes/Gilmour_15_17/OceanAK_Origin_PostQA")) {dir.create("../../Genotypes/Gilmour_15_17/OceanAK_Origin_PostQA")}
# save_sillys(ProjectSillys_Origin, "../../Genotypes/Gilmour_15_17/OceanAK_Origin_PostQA/")
```
...
```{r genepop}
if(!dir.exists("../../Genepop/Gilmour_15_17")) {dir.create("../../Genepop/Gilmour_15_17")}

# gcl2Genepop.GCL(sillyvec = c("PGILMOUR15n", "PGILMOUR15h", "PGILMOUR17"), 
#                 loci = loci, 
#                 path = "../../Genepop/Gilmour_15_17/Gilmour_15_17_postQA.gen", 
#                 VialNums = TRUE, 
#                 usat = FALSE)

gcl2Genepop.GCL(sillyvec = c("PGILMOUR15", "PGILMOUR17"), 
                loci = loci, 
                path = "../../Genepop/Gilmour_15_17/Gilmour_15_17_postQA.gen", 
                VialNums = TRUE, 
                usat = FALSE)
...
genepop2franz.GCL(
  Genepop = "../../Genepop/Gilmour_15_17/Gilmour_15_17_postQA.gen",
