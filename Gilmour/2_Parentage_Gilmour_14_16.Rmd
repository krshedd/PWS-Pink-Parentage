...
## *FRANz* parameters
*FRANz* was run at ??????. We used the following parameters:
FRANz.exe --Nmmax 5000 --Nfmax 5000 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 "V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\"

Do not use --fullsibtest if running on Kyle's Genetics Domain Computer, it won't save output!!!

The parameters are defined as follows:
--Nmmax and --Nfmax are the maximum numbers of candidate mothers and fathers. To obtain our values, we used an estimated escapement of 10,000 and divided by 2, for lack of any solid data (*note: aerial survey was not done in 2014 as Gilmour was not an index stream and stream walk was 553, which is smaller than the number of samples we have. So not knowing how many fish were there or how many hatchery strays we have in 2016, or where any of our other fish came from we went with a big number to let the genotype data speak for itself*). This escapement number is a wild ass guess, as I did not have an aerial survey AUC. This estimate is listed under 'pop est walk' in the Excel file found here: "V:\Documents\5_Coastwide\Multispecies\AHRP\Field data\Sample Summary Extract and Genotype.xlsx"   
...
All output files can be found here: "V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\Stockdale_2016_2018"  
...
parentage_16_18 <- read_csv("../../Franz/Stockdale_2016_2018/parentage.csv")
paired_16_18 <- read_csv("../Franz/stockdale_16_18_postQA_OceanAK_paired_2016_2018_STOCK.csv")
```
...
write_csv(paired_16_18_filter, "stock_paired_16_18_filter.csv")
```
...
paired_16_18_filter %>% 
  ...
  xlab("Day of Year") +
  labs(title= "Stockdale 2016/2018",
       fill = "Origin") +
...
write_csv(parents_paired_16_18, "stock_parents_paired_16_18.csv")
...
**Note** that *FRANz* assigned 566 offspring to 383 parents. 551 of these offspring have natural-origin parents and 15 offspring have hatchery-origin parents. The hatchery origin fish are from AFK14B.
...
O.even <- c("H", "N")
Spawners.even <- c(337, 5794) #  number of hatchery- and natural-origin parents
Offspring.even <- c(15, 551)  # number of offspring assigned to hatchery- and natural-origin parents
...
Here, we calculate and plot family size for both natural and hatchery parents.  Family size ranges from 1-6. Most assignments were to 1 parent (*single parent-offspring duos*) - there were 67 2 parent assignments (*parent pair-offspring trios*). 
...
Female parents produced 1-7 offspring, while male parents produced 1-8.  
...
write_csv(paired_16_18_filter_parents, "stock_paired_16_18_filter_parents.csv")
...
Calculate 95% CIs for Stockdale 16-18 females. RRS = 0.37 (0.16-0.73)
...
rrs_ci_kalinowski(n_h_off = 7, n_w_off = 327, n_h_par = 181, n_w_par = 3156, alpha = 0.05)
```

Calculate 95% CIs for Hogan 16-18 males. RRS = 0.60 (0.27-1.14)
...
rrs_ci_kalinowski(n_h_off = 8, n_w_off = 224, n_h_par = 156, n_w_par = 2638, alpha = 0.05)
```
...
write_csv(parents_paired_16_18_cross, "stock_parents_paired_16_18_cross.csv")
...
paired_16_18_filter_parents <- read_csv("stock_paired_16_18_filter_parents.csv") 
...
```{r}
rrs_ci_kalinowski(n_h_off = 30, n_w_off = 74, n_h_par = 373, n_w_par = 865, alpha = 0.05)
```
...
(parents_paired_16_18 <- read_csv("stock_parents_paired_16_18.csv") %>% 
  dplyr::select(-origin))
...
(
  stock_grandparentage <- full_join(sires_16, dams_16, by = "Offspring") %>% 
    arrange(Offspring) %>% 
...
2. 499 offspring were assigned to 383 parents, for an assignment rate of **12.35%**.  
3. 67 offspring assigned to 2 parents (triads).  
4. RRS (with 0's) was significant for females (0.37), but not for males (0.60).  
5. There was not enough cross type data (triads) to make meaningful comparisons.  
