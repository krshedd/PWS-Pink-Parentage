---
title: "Gilmour 2015/2017 Single-generation Parentage Analysis with 298 Markers"
...
We use the program *FRANz* [https://www.bioinf.uni-leipzig.de/Software/FRANz/About.html] to reconstruct parent-offspring relationships. Below are analyses of parentage for **Gilmour** for the **odd lineage** years **2015** (potential parents) and **2017** (potential offspring).

*FRANz* was begun on Fri Jan 15 2021 at 15:51 using the following parameters in the command line in the V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\**Gilmour_2015_2017** folder:
**FRANz --Nmmax 3000 --Nfmax 3000 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 Gilmour_15_17_postQA_2015_2017_GILMOUR.dat**

The parameters are defined as follows:

* --Nmmax and --Nfmax are the maximum numbers of candidate mothers and fathers. To obtain our values, we used an estimated escapement of 6,000 and divided by 2. This escapement number came from splitting the difference between the aerial survey AUC number of 5,460 and stream walk survey AUC of 6,120. These estimates are listed under 'pop est walk' and 'pop est aerial' in the Excel file found here: "V:\Documents\5_Coastwide\Multispecies\AHRP\Field data\Sample Summary Extract and Genotype.xlsx".    
...
parentage_15_17 <- read_csv("V:/Analysis/5_Coastwide/Multispecies/Alaska Hatchery Research Program/PWS Pink/Franz/Gilmour_2015_2017/parentage.csv") # for some reason, I keep getting and error with a relative path
paired_15_17 <- read_csv("../Franz/Gilmour_15_17_postQA_OceanAK_paired_2015_2017_GILMOUR.csv") # works for this one, tho
```
...
grep(pattern = "RAD", x = colnames(paired_15_17), value = TRUE, invert = TRUE)

# Filter for non-genotype columns
paired_15_17_filter <- as_tibble(paired_15_17) %>% 
...
write_csv(paired_15_17_filter, "Gilmour_paired_15_17_filter.csv")
```
...
parentage_15_17_filter <- parentage_15_17 %>% 
...
## Sample sizes
  ...
  * Quick look at sample sizes for potential parents (2015) and potential offspring (2017)
...
```
**Results:** Apparently there is no otolith data for any of these guys. What can we do from here?...
...
sample_size_plot(indata = paired_15_17_filter, title = "Gilmour 2015-2017")
```
...
(parents_paired_15_17 <- parentage_15_17_filter %>% 
   left_join(paired_15_17_filter, by = c("Offspring" = "franz_id")) %>% 
   left_join(paired_15_17_filter, by = c("Parent_ID" = "franz_id"), suffix = c(".off", ".par")) %>% 
...
write_csv(parents_paired_15_17, "Gilmour_parents_paired_15_17.csv")
...
n_distinct(parents_paired_15_17$Parent_ID)
...
parents_paired_15_17 %>% 
  ...
  spread(origin, n)
```
**Results:** Of the **5,075** natural-origin offspring genotyped in 2017, *FRANz* assigned **224**, for an assignment rate of **4.4%**. That's pretty terrible. A total of **174** unique parents were assigned offspring, with **204** offspring assigned one parent and **22** assigned both parents. All parents assigned offspring are of unknown origin at this time.
...
spawners_15 <- paired_15_17_filter %>% 
  filter(`Sample Year` == 2015) %>% 
  count(origin)

# How many offspring from each origin?
offspring_17 <- parents_paired_15_17 %>% 
  count(origin)
```

```{r chi_square_test, echo=FALSE, warning=FALSE, message=FALSE}
spawners_and_offspring_15_17 <- data.frame(spawners_15, offspring_17)
spawners_and_offspring_15_17 <- subset(spawners_and_offspring_15_17, select = c(n, n.1))

colnames(spawners_and_offspring_15_17) <- c("Gilmour_spawners_15", "Gilmour_offspring_17")
rownames(spawners_and_offspring_15_17) <- c("Natural", "Hatchery")

spawners_and_offspring_15_17
chisq.test(spawners_and_offspring_15_17)
```
**Results:** There was a highly significant difference in the proportions of offspring assigned to hatchery- and natural-origin parents relative to the proportions of potential parents sampled, indicating an under-representation of offspring assigning to hatchery-origin parents. Although **`r round(spawners_and_offspring_15_17["Hatchery", "Gilmour_spawners_15"] / sum(spawners_and_offspring_15_17[, "Gilmour_spawners_15"]), 2) * 100`%** of parents genotyped were of hatchery-origin, only **`r round(spawners_and_offspring_15_17["Hatchery", "Gilmour_offspring_17"] / sum(spawners_and_offspring_15_17[, "Gilmour_offspring_17"]), 2) * 100`%** of offspring assigned to hatchery-origin parents.
...
parents_paired_15_17 %>% 
  ...
  theme_bw()
```
**Results:** Female parents produced **0-6** offspring, while known male parents produced **0-4**.  
...
parents_paired_15_17_count <- parents_paired_15_17 %>% 
...
paired_15_17_filter_parents <- paired_15_17_filter %>% 
  filter(`Sample Year` == "2015")

# Join with all parents that produced no progeny
paired_15_17_filter_parents <- left_join(paired_15_17_filter_parents, parents_paired_15_17_count, by = c("franz_id" = "Parent_ID")) %>% 
  mutate(n = replace_na(n, 0)) %>% 
  rename(origin = origin.x) %>% 
  dplyr::select(-origin.y, -SEX.par, -`Sample Year.par`, -`Length Mm.par`, -`Sample Date.par`)

write_csv(paired_15_17_filter_parents, "Gilmour_paired_15_17_filter_parents.csv")

# Mean RS, including zeroes
rs_15_17_0 <- paired_15_17_filter_parents %>%
  group_by(origin, SEX) %>% 
...
# Save RS values
F_h_0_15_17 <- rs_15_17_0 %>% 
  filter(origin == "Hatchery" & SEX == "F") %>% 
  pull(RS)
M_h_0_15_17 <- rs_15_17_0 %>% 
  filter(origin == "Hatchery" & SEX == "M") %>% 
  pull(RS)
F_n_0_15_17 <- rs_15_17_0 %>% 
  filter(origin == "Natural" & SEX == "F") %>% 
  pull(RS)
M_n_0_15_17 <- rs_15_17_0 %>% 
...
paired_15_17_filter_parents %>%
 ...
 geom_vline(xintercept=F_n_0_15_17, size=2) +
 geom_vline(xintercept=F_n_0_15_17, color=cbPalette2[1], linetype = "dashed", size = 2) +
 geom_vline(xintercept=F_h_0_15_17, size=2) +
 geom_vline(xintercept=F_h_0_15_17, color=cbPalette2[2], linetype = "dashed", size = 2) +
 annotate("text", x = F_n_0_15_17 + 0.5, y = .250, label = paste("Female_natural_RS =", round(F_n_0_15_17, 2)), hjust = 0, size = 6, color = cbPalette2[1]) +
 annotate("text", x = F_h_0_15_17 + 0.5, y = .300, label = paste("Female_hatchery_RS =", round(F_h_0_15_17, 2)), hjust = 0, size = 6, color = cbPalette2[2]) +
...
paired_15_17_filter_parents %>%
 ...
 geom_vline(xintercept=M_n_0_15_17, size=2) +
 geom_vline(xintercept=M_n_0_15_17, color=cbPalette2[1], linetype = "dashed", size = 2) +
 geom_vline(xintercept=M_h_0_15_17, size=2) +
 geom_vline(xintercept=M_h_0_15_17, color=cbPalette2[2], linetype = "dashed", size = 2) +
 annotate("text", x = M_n_0_15_17 + 0.5, y = .250, label = paste("Male_natural_RS =", round(M_n_0_15_17, 2)), hjust = 0, size = 6, color = cbPalette2[1]) +
 annotate("text", x = M_h_0_15_17 + 0.5, y = .300, label = paste("Male_hatchery_RS =", round(M_h_0_15_17, 2)), hjust = 0, size = 6, color = cbPalette2[2]) +
  theme(legend.position = c(0.8, 0.75), legend.background = element_rect(linetype = 1, size = 0.5, colour = 1)) +
  scale_fill_manual(values = cbPalette2)
```
**Results:** RSs for females was **0.05** and for males was **0.04**. In the histograms, note the large number of potential parents assigned no offspring that were absent from the family size plots above.

**NOTE: ANALYSIS STOPPED HERE UNTIL WE CAN GAIN INFORMATION ON ORIGIN FROM OTOLITHS, PENDING RESOLUTION OF THE GOD EVENT.**
...
RRS_M_0_15_17 <- M_h_0_15_17/M_n_0_15_17
RRS_F_0_15_17 <- F_h_0_15_17/F_n_0_15_17

RRS_tibble_0_15_17 <- tibble(RRS_M_0_15_17, RRS_F_0_15_17)
round(RRS_tibble_0_15_17, 2)
```
**Results:** RRS for males = `r round(RRS_M_0_15_17, 2)`. RRS for females = `r round(RRS_F_0_15_17, 2)`.
...
mydata_M_0 <- paired_15_17_filter_parents %>%
  filter(SEX == "M")

mydata_F_0 <- paired_15_17_filter_parents %>%
  filter(SEX == "F")
...
perm_1tail_pvalue_Both_0 <- round(x = as.numeric(coin::pvalue(oneway_test(n ~ origin, data = paired_15_17_filter_parents, distribution = approximate(B = 10000), alternative = "greater"))), digits = 4)
...
fit_Both_0 <- glm.nb(n ~ origin, data = paired_15_17_filter_parents, init.theta = 1, link = log)
...
n_F_par <- paired_15_17_filter_parents %>% 
  select(-n) %>% 
  filter(SEX == "F") %>% 
  count(SEX, origin) %>% 
  spread(origin, n)
...
n_F_off <- paired_15_17_filter_parents %>% 
  filter(SEX == "F") %>% 
  group_by(SEX, origin) %>% 
  summarise(n_off = sum(n)) %>% 
  spread(origin, n_off)

rrs_F_15_17 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(n_F_off[1,3]), n_w_off = as.integer(n_F_off[1,2]), n_h_par = as.integer(n_F_par[1,3]), n_w_par = as.integer(n_F_par[1,2]), alpha = 0.05))

# Males
# Get parent sample sizes
n_M_par <- paired_15_17_filter_parents %>% 
  select(-n) %>% 
  filter(SEX == "M") %>% 
  count(SEX, origin) %>% 
  spread(origin, n)
...
# Get offspring sample sizes
n_M_off <- paired_15_17_filter_parents %>% 
  filter(SEX == "M") %>% 
  group_by(SEX, origin) %>% 
  summarise(n_off = sum(n)) %>% 
  spread(origin, n_off)

rrs_M_15_17 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(n_M_off[1,3]), n_w_off = as.integer(n_M_off[1,2]), n_h_par = as.integer(n_M_par[1,3]), n_w_par = as.integer(n_M_par[1,2]), alpha = 0.05))

rrs_15_17 <- cbind(rrs_F_15_17, rrs_M_15_17)

colnames(rrs_15_17) <- c("Gilmour_females_15_17", "Gilmour_males_15_17")
rownames(rrs_15_17) <- c("Lower 95% CL", "RRS", "Upper 95% CL")

rrs_15_17 %>% 
  round(2)
```
**Results:** Female RRS was **`r round(rrs_F_15_17[2,1], 2)` (95% CI [`r round(rrs_F_15_17[1,1], 2)`-`r round(rrs_F_15_17[3,1], 2)`])** and male RRS was **`r round(rrs_M_15_17[2,1], 2)` (95% CI [`r round(rrs_M_15_17[1,1], 2)`-`r round(rrs_M_15_17[3,1], 2)`])**.
...
```{r paired_15_17_filter_parents_by_sex, echo=FALSE, warning=FALSE, message=FALSE}
paired_15_17_filter_parents <- paired_15_17_filter_parents %>% 
  filter(SEX != "?") %>% 
...
triads_15_17 <- parents_paired_15_17 %>% 
  filter(Parent == "Parent 2") %>% 
  pull(Offspring)

parents_paired_15_17_cross <- parentage_15_17 %>% 
  filter(Posterior > 0.9) %>% 
  dplyr::select(Offspring, `Parent 1`, `Parent 2`) %>% 
  filter(Offspring %in% triads_15_17) %>% 
  left_join(paired_15_17_filter, by = c("Offspring" = "franz_id")) %>% 
  left_join(paired_15_17_filter, by = c("Parent 1" = "franz_id"), suffix = c(".off", ".par1")) %>% 
  left_join(paired_15_17_filter, by = c("Parent 2" = "franz_id")) %>% 
...
write_csv(parents_paired_15_17_cross, "Gilmour_parents_paired_15_17_cross.csv")

parents_paired_15_17_cross %>% 
...
cross_count_15_17 <- parents_paired_15_17_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  rename(nn = n) %>% 
  count(cross) %>% 
  rename(samp = n)
...
parents_paired_15_17_cross %>% 
  ...
  group_by(cross) %>% 
  mutate(p = nn / sum(nn)) %>% 
  left_join(cross_count_15_17, by = "cross") %>% 
...
```{r RRS_confidence_intervals_15_17, echo=FALSE, warning=FALSE, message=FALSE}
# RS of cross types
(parents_paired_15_17_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  group_by(cross) %>% 
...
Xtype_nmat <- parents_paired_15_17_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  count(cross)

# Get offspring sample sizes
Xtype_noff <- parents_paired_15_17_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  group_by(cross) %>% 
  summarise(nn = sum(n))
...
# 1. HH vs. NN
# rrs_HH_15_17 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(Xtype_noff[1,2]), n_w_off = as.integer(Xtype_noff[4,2]), n_h_par = as.integer(Xtype_nmat[1,2]), n_w_par = as.integer(Xtype_nmat[4,2]), alpha = 0.05)) # DNE

# 2. HN vs. NN
rrs_HN_15_17 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(Xtype_noff[1,2]), n_w_off = as.integer(Xtype_noff[3,2]), n_h_par = as.integer(Xtype_nmat[1,2]), n_w_par = as.integer(Xtype_nmat[3,2]), alpha = 0.05))

# 3. NH vs. NN
rrs_NH_15_17 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(Xtype_noff[2,2]), n_w_off = as.integer(Xtype_noff[3,2]), n_h_par = as.integer(Xtype_nmat[2,2]), n_w_par = as.integer(Xtype_nmat[3,2]), alpha = 0.05))

rrs_Xtype_15_17 <- cbind(rrs_HN_15_17, rrs_NH_15_17)

colnames(rrs_Xtype_15_17) <- c("Gilmour_HNvsNN_15_17", "Gilmour_NHvsNN_15_17")
rownames(rrs_Xtype_15_17) <- c("Lower 95% CL", "RRS", "Upper 95% CL")

rrs_Xtype_15_17 %>% 
  round(2)
```
**Results:** RRS for the comparisons were **`r round(rrs_Xtype_15_17[2,1], 2)` (95% CI [`r round(rrs_Xtype_15_17[1,1], 2)`-`r round(rrs_Xtype_15_17[3,1], 2)`])** for HN and **`r round(rrs_Xtype_15_17[2,2], 2)` (95% CI [`r round(rrs_Xtype_15_17[1,2], 2)`-`r round(rrs_Xtype_15_17[3,2], 2)`])** for NH, indicating no significant differences in RS relative to the NN cross type. It should be noted again, however, that the numbers of NH and HN crosses relative to NN were very small.
...
(parents_paired_15_17_cross_dam_sire <- parents_paired_15_17_cross %>% 
  ...
  left_join(paired_15_17_filter, by = c("Dam" = "franz_id")) %>% 
  left_join(paired_15_17_filter, by = c("Sire" = "franz_id"), suffix = c("_dam", "_sire")))

# How many unique matings?
(n_unique_matings_15_17 <- parents_paired_15_17_cross_dam_sire %>% 
  count(Dam, Sire))

# Do the sample dates for cross types make sense?
parents_paired_15_17_cross_dam_sire %>% 
...
n_sires_15_17 <- parents_paired_15_17_cross_dam_sire %>% 
  group_by(Dam) %>% 
  summarise(n_sires = n_distinct(Sire)) %>% 
  count(n_sires) %>% 
  rename(n_mates = n_sires, n_dams_15_17 = n)

# How many sires had multiple dams
n_dams_15_17 <- parents_paired_15_17_cross_dam_sire %>% 
  group_by(Sire) %>% 
  summarise(n_dams = n_distinct(Dam)) %>% 
  count(n_dams) %>%
  rename(n_mates = n_dams, n_sires_15_17 = n)

(n_dams_and_sires_15_17 <- full_join(n_sires_15_17, n_dams_15_17, by = "n_mates"))
```
...
dams_with_mult_sire <- parents_paired_15_17_cross_dam_sire %>% 
  group_by(Dam) %>% 
...
sires_with_mult_dam <- parents_paired_15_17_cross_dam_sire %>% 
  group_by(Sire) %>% 
  summarise(n_cross = n_distinct(cross)) %>% 
  filter(n_cross > 1) %>% 
  pull(Sire)
...
# **Grandparentage**
We find the parents (2015) that were assigned to grandparents (2017) and determine how many grandoffspring (2019) were assigned to them. We then estimate RRS according to grandparental origin. The set of panels below include:
```{r import_grandparent_data, echo=FALSE, warning=FALSE, message=FALSE}
# # Read in data
# parents_paired_15_17 <- read_csv("gilmour_parents_paired_15_17.csv") %>% 
#   dplyr::select(-origin)
# 
# paired_17_19_filter_parents <- read_csv("Gilmour_paired_17_19_filter_parents.csv")
# 
# parentage_paired_17_19 <- read_csv("Gilmour_parents_paired_17_19.csv")%>% 
#   dplyr::select(-origin)
```
...
# 
# colnames(rrs_gpar_17_19) <- c("Gilmour_grandparents_17_19")
...
# 
# write_csv(grandparentage, "Gilmour_17_19_grandparentage.csv")
```
...
# Read in data
# parents_paired_13_15 <- read_csv("gilmour_parents_paired_13_15.csv") %>% 
#   dplyr::select(-origin)
# 
# parents_paired_15_17 <- read_csv("gilmour_parents_paired_15_17.csv") %>% 
#   dplyr::select(-origin)
# 
# parents_paired_17_19 <- read_csv("Gilmour_parents_paired_17_19.csv") %>% 
#   dplyr::select(-origin)
# 
# paired_17_19_filter_parents <- read_csv("Gilmour_paired_17_19_filter_parents.csv")
# 
# paired_15_17_filter_parents <- read_csv("gilmour_paired_15_17_filter_parents.csv")
```
