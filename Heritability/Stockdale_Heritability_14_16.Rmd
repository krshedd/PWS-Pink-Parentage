...
stock_riverdist <- read_csv("../../GIS/R/all_streams/stockdale_distances.csv") %>% 
...
parents_paired_cross_riverdist_spacetime <- function(indata_spacetime = stock_parents_paired_14_16_cross) {
  indata_spacetime %>% 
  ...
  left_join(stock_riverdist, by = c("Sample.par1" = "Sample")) %>% 
  left_join(stock_riverdist, by = c("Sample.par2" = "Sample"), suffix = c(".par1", ".par2")) %>% 
...
(stock_parents_paired_14_16_cross_riverdist_spacetime <- parents_paired_cross_riverdist_spacetime(stock_parents_paired_16_18_cross))
```
...
# by offspring sex
stock_h2_length_14_16_F_slope <- lm(Length.off ~ Length.par, data = stock_triads_length_14_16 %>% filter(Sex.off == "F"), na.action = na.omit)
(stock_h2_length_14_16_F_slope_tbl <- tidy(stock_h2_length_14_16_F_slope))

stock_h2_length_14_16_M_slope <- lm(Length.off ~ Length.par, data = stock_triads_length_14_16 %>% filter(Sex.off == "M"), na.action = na.omit)
(stock_h2_length_14_16_M_slope_tbl <- tidy(stock_h2_length_14_16_M_slope))
...
dat.1 <- as.data.frame(stock_triads_length_14_16) %>% 
...
ped.1 <- as.data.frame(read.csv(file = "stock_triads_14_16_JH_ped.csv"))
# str(ped.1)
# headTail(ped.1)
...
# by offspring sex
stock_h2_date_14_16_F_slope <- lm(DOY.off ~ DOY.par, data = stock_triads_date_14_16 %>% filter(Sex.off == "F"), na.action = na.omit)
(stock_h2_date_14_16_F_slope_tbl <- tidy(stock_h2_date_14_16_F_slope))

stock_h2_date_14_16_M_slope <- lm(DOY.off ~ DOY.par, data = stock_triads_date_14_16 %>% filter(Sex.off == "M"), na.action = na.omit)
(stock_h2_date_14_16_M_slope_tbl <- tidy(stock_h2_date_14_16_M_slope))
...
# by offspring sex
(stock_parents_paired_14_16_rs6 <- parents_paired_rs() %>% 
  group_by(sex.off) %>%
  summarise(correlation = round(cor(n.off, rs.par), 3), r2 = round(cor(n.off, rs.par)^2, 3)))
...
# by offspring sex
stock_h2_rs_14_16_F_slope <- lm(n.off ~ rs.par, data = stock_triads_rs_14_16 %>% filter(Sex.off == "F"), na.action = na.omit)
(stock_h2_rs_14_16_F_slope_tbl <- tidy(stock_h2_rs_14_16_F_slope))

stock_h2_rs_14_16_M_slope <- lm(n.off ~ rs.par, data = stock_triads_rs_14_16 %>% filter(Sex.off == "M"), na.action = na.omit)
(stock_h2_rs_14_16_M_slope_tbl <- tidy(stock_h2_rs_14_16_M_slope))
...
# load data and pedigree files
dat.3 <- as.data.frame(stock_triads_rs_14_16) %>% 
  mutate(animal = Offspring)
