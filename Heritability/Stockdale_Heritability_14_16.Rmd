...
# run linear model
# lme4::lmer
# fixed effects: offspring sex, parental brood year (doesn't matter for pinks), parental origin
# random effects: offspring breeding value (deviation of the offspring of a given parent from the population average), parental ID

# breeding values
stock_triads_length_14_16_bv <- stock_triads_length_14_16 %>% 
  mutate(Length.bv.off = stock_slope_as_h2_length_14_16[1,2] * (Length.off - mean(Length.off, na.rm = TRUE))) %>% 
  mutate(Length.bv.dam = stock_slope_as_h2_length_14_16[1,2] * (Length.dam - mean(Length.dam, na.rm = TRUE))) %>% 
  mutate(Length.bv.sire = stock_slope_as_h2_length_14_16[1,2] * (Length.sire - mean(Length.sire, na.rm = TRUE)))

# overall
# stock_h2_length_14_16_dam_sire <- lmer(Length.off ~ Length.par + (1|Length.sire)+(1|Length.dam), 
                                        # data = stock_triads_length_14_16, 
                                        # na.action = na.omit)
stock_h2_length_14_16_dam_sire <- lmer(Length.bv.off ~ Sex.off+Origin.dam+Origin.sire + (1|Parent_ID.sire)+(1|Parent_ID.dam)+(1|Length.bv.sire)+(1|Length.bv.dam), 
