---
title: "Hogan 2013/2015 Exploratory Analysis with 298 Markers"
output:
  html_notebook:
    theme: united
    toc: yes
...
This is a quick exploratory analysis of the Hogan 2013/2015 genotype data. Here we will:
...
```{r loki}
sillyvec <- ProjectSillys <- c("PHOGAN13", "PHOGAN15")
# LocusControl <- dget("../Objects/Hogan/LocusControl.txt")
loci298 <- dget("../../Objects/Hogan_13_14_15_16/loci298.txt")
...
save_objects(objects = c("LocusControl", "loci", "sillyvec", "ProjectSillys"), path = "../../Objects/Hogan_13_15/")
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Hogan_13_15/")
```
# OceanAK
...
hogan_oceanak <- oceanak %>% 
  dplyr::filter(`Location Code` == "Hogan Creek" & `Sample Year` %in% c(2013, 2015)) %>% 
  tidyr::unite(silly_source, c("Silly Code", "Fish ID"), sep = "_", remove = FALSE)
```

Verify all genotypes in OceanAK
```{r}
table(PHOGAN13.gcl$attributes$SillySource %in% hogan_oceanak$silly_source)
...
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Hogan_13_15/OceanAK")
```
...
ProjectSillys_Origin <- c("PHOGAN13n", "PHOGAN13h", "PHOGAN15")
save_sillys(ProjectSillys_Origin, "../../Genotypes/Hogan_13_15/OceanAK_Origin/")
```
...
```{r failure_rate}
project <- "Hogan_13_15" #this is required of the function; can be any name 
failure_rate <- FailureRate.GCL(sillyvec = ProjectSillys_Origin)
failure_rate_noplots <- failure_rate[1:4]
save_objects("failure_rate_noplots", "../../Objects/Hogan_13_15")
failure_rate
```
...
save_objects("MissLoci", path = "../../Objects/Hogan_13_15/")
...
save_objects("DuplicateCheckReportSummary", path = "../../Objects/Hogan_13_15/")
```
...
# dup_13n_ids <- filter(dups, silly == "PHOGAN13n") %>% pull(id)
dup_13h_ids <- filter(dups, silly == "PHOGAN13h") %>% pull(id)  # none
...
duplicates_to_remove <- bind_rows(dup_13h, dup15)
save_objects("duplicates_to_remove", path = "../../Objects/Hogan_13_15/")
...
het_tibble <- bind_rows(tibble_PHOGAN13n, tibble_PHOGAN13h, tibble_PHOGAN15)
...
het_tibble <- het_tibble %>% 
  dplyr::mutate(lineage = dplyr::case_when(silly %in% c("PHOGAN13", "PHOGAN15") ~ "odd",
                           TRUE ~ "even"))
save_objects("het_tibble", "../../Objects/Hogan_13_15/")
...
chris_het_tibble <- dplyr::bind_rows(odd_chris_het)
...
save_objects("removed_hets", "../../Objects/Hogan_13_15/")
...
save_objects("ProjectSillys_Origin_SampleSizes", path = "../../Objects/Hogan_13_15/")
```

Save post-QA genotypes.
```{r save_postQA_genotypes}
save_sillys(ProjectSillys_Origin, "../../Genotypes/Hogan_13_15/OceanAK_Origin_PostQA/")
```
...
gcl2Genepop.GCL(sillyvec = c("PHOGAN13n", "PHOGAN13h", "PHOGAN15"), 
                loci = loci, 
                path = "../../Genepop/Hogan_13_15/hogan_13_15_postQA.gen", 
                VialNums = TRUE, 
                usat = FALSE)
```
...
genepop2franz.GCL(
  Genepop = "../../Genepop/Hogan_13_15/hogan_13_15_postQA.gen",
