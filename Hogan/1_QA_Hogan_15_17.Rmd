---
title: "Hogan 2015/2017 Exploratory Analysis with 298 Markers"
output:
  html_notebook:
    theme: united
    toc: yes
...
This is a quick exploratory analysis of the Hogan 2015/2017 genotype data. Here we will:
...
```{r loki}
sillyvec <- ProjectSillys <- c("PHOGAN15", "PHOGAN17")
...
Sample size?
```{r}
sapply(sillyvec, function(x) {get(paste0(x, ".gcl"))$n})
```

Save objects
```{r save_locus_control_sillys}
save_objects(objects = c("LocusControl", "loci", "sillyvec", "ProjectSillys"), path = "../../Objects/Hogan_15_17/")
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Hogan_15_17/")
```
# OceanAK
...
hogan_oceanak <- oceanak %>% 
  dplyr::filter(`Location Code` == "Hogan Creek" & `Sample Year` %in% c(2015, 2017)) %>% 
  tidyr::unite(silly_source, c("Silly Code", "Fish ID"), sep = "_", remove = FALSE)
```

Verify all genotypes in OceanAK
```{r}
table(PHOGAN15.gcl$attributes$SillySource %in% hogan_oceanak$silly_source)
...
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Hogan_15_17/OceanAK")
```


## Split by otolith-origin

First we need to verify that we only have hatchery and natural-origin fish for the **F0** year (2015), and only natural-origin fish for the **F1** year (2017).
```{r view_origin}
dplyr::bind_rows(PHOGAN15.gcl$attributes, 
                 PHOGAN17.gcl$attributes) %>% 
  count(SILLY_CODE, `Otolith Mark Present`) %>% 
  spread(`Otolith Mark Present`, n, fill = 0)
```

We want to remove any NA from 2015 and hathcery-origin fish + NA from 2017. We'll need those hatchery-origin fish from 2017 for looking at 2017/2019 parentage, but not for 2015/2017. The NA's must have snuck in there from second reads?
...
Now that we have removed those fish that we did not want due to their otolith status, we will split 2015 otolith-origin for QA tabulating purposes. Also create a new *sillyvec*.
```{r split_hogan15}
PHOGAN15_ids_natural <- list("PHOGAN15" = PHOGAN15.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "NO") %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID))
PoolCollections.GCL(collections = "PHOGAN15", loci = loci, IDs = PHOGAN15_ids_natural, newname = "PHOGAN15n")

PHOGAN15_ids_hatchery <- list("PHOGAN15" = PHOGAN15.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "YES") %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID))
PoolCollections.GCL(collections = "PHOGAN15", loci = loci, IDs = PHOGAN15_ids_hatchery, newname = "PHOGAN15h")

ProjectSillys_Origin <- c("PHOGAN15n", "PHOGAN15h", "PHOGAN17")
save_sillys(ProjectSillys_Origin, "../../Genotypes/Hogan_15_17/OceanAK_Origin/")
```
...
```{r failure_rate}
project <- "Hogan_15_17" #this is required of the function; can be any name 
failure_rate <- FailureRate.GCL(sillyvec = ProjectSillys_Origin)
failure_rate_noplots <- failure_rate[1:4]
save_objects("failure_rate_noplots", "../../Objects/Hogan_15_17")
failure_rate
```
...
save_objects("MissLoci", path = "../../Objects/Hogan_15_17/")
...
save_objects("DuplicateCheckReportSummary", path = "../../Objects/Hogan_15_17/")
```
...
dup_15n_ids <- filter(dups, silly == "PHOGAN15n") %>% pull(id)
dup_15h_ids <- filter(dups, silly == "PHOGAN15h") %>% pull(id)
dup_17_ids <- filter(dups, silly == "PHOGAN17") %>% pull(id)

# Save attributes for posterity
# ID silly sources for duplicate individuals
dup_15n <- PHOGAN15n.gcl$attributes %>% 
  filter(FK_FISH_ID %in% dup_15n_ids) %>% 
  mutate(FK_FISH_ID = as.numeric(FK_FISH_ID))
dup_15h <- PHOGAN15h.gcl$attributes %>% 
  filter(FK_FISH_ID %in% dup_15h_ids) %>% 
  mutate(FK_FISH_ID = as.numeric(FK_FISH_ID))
dup17 <- PHOGAN17.gcl$attributes %>%  
...
save_objects("duplicates_to_remove", path = "../../Objects/Hogan_15_17/")

# Remove fish from .gcl objects
RemoveIDs.GCL(silly = "PHOGAN15n", IDs = dup_15n_ids)
RemoveIDs.GCL(silly = "PHOGAN15h", IDs = dup_15h_ids)
RemoveIDs.GCL(silly = "PHOGAN17", IDs = dup_17_ids)
...
PHOGAN15n_ind_het <- apply(PHOGAN15n.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
PHOGAN15h_ind_het <- apply(PHOGAN15h.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
PHOGAN17_ind_het <- apply(PHOGAN17.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} )
```
...
# Convert to tibbles
tibble_PHOGAN15n <- tibble(silly=PHOGAN15n.gcl$attributes$SILLY_CODE, sillysource=PHOGAN15n.gcl$attributes$SillySource, het=PHOGAN15n_ind_het)
tibble_PHOGAN15h <- tibble(silly=PHOGAN15h.gcl$attributes$SILLY_CODE, sillysource=PHOGAN15h.gcl$attributes$SillySource, het=PHOGAN15h_ind_het)
tibble_PHOGAN17 <- tibble(silly=PHOGAN17.gcl$attributes$SILLY_CODE, sillysource=PHOGAN17.gcl$attributes$SillySource, het=PHOGAN17_ind_het)

# Create a single tibble
het_tibble <- bind_rows(tibble_PHOGAN15n, tibble_PHOGAN15h, tibble_PHOGAN17)
...
het_tibble <- het_tibble %>% 
  dplyr::mutate(lineage = dplyr::case_when(silly %in% c("PHOGAN15", "PHOGAN17") ~ "odd",
                           TRUE ~ "even"))
save_objects("het_tibble", "../../Objects/Hogan_15_17/")
...
If we went with the 3SD cutoff, we would lose 321 samples from the odd lineage. Still, this +-3SD by lineage approach does not seem ideal given the large number of outliers that **pull** the SD. We can see this long right tail with a qqplot.
...
save_objects("removed_hets", "../../Objects/Hogan_15_17/")

PHOGAN15h_hets_remove <- PHOGAN15h.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PHOGAN15h", IDs = PHOGAN15h_hets_remove)

PHOGAN15n_hets_remove <- PHOGAN15n.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PHOGAN15n", IDs = PHOGAN15n_hets_remove)

PHOGAN17_hets_remove <- PHOGAN17.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PHOGAN17", IDs = PHOGAN17_hets_remove)
...
save_objects("ProjectSillys_Origin_SampleSizes", path = "../../Objects/Hogan_15_17/")
```

Save post-QA genotypes.
```{r save_postQA_genotypes}
save_sillys(ProjectSillys_Origin, "../../Genotypes/Hogan_15_17/OceanAK_Origin_PostQA/")
```
...
```{r genepop}
gcl2Genepop.GCL(sillyvec = c("PHOGAN15n", "PHOGAN15h", "PHOGAN17"), 
                loci = loci, 
                path = "../../Genepop/Hogan_15_17/hogan_15_17_postQA.gen", 
                VialNums = TRUE, 
                usat = FALSE)
```
...
genepop2franz.GCL(
  Genepop = "../../Genepop/Hogan_15_17/hogan_15_17_postQA.gen",
