---
title: "Hogan 2013/2015 Parentage Analysis with 298 Markers"
output:
  html_notebook:
    theme: united
    toc: yes
...
We re-ran the 2013/2015 data because we originally only had 1,919 genotyped offspring (limited by grant funding for the number of fish to analyze). However, when we went back to do 2015/2017, we genotyped more natural-origin fish from 2015, thus giving us a total of 3,775 potential offspring. Thus, we re-ran to 2013/2015 parentage analysis with these additional potential offspring.

## *FRANz* parameters
*FRANz* was re-run at Fri Dec 13 12:04:13 2019. We used the following parameters:
FRANz.exe --Nmmax 23500 --Nfmax 23500 --femrepro 1:2 --malerepro 1:2 --typingerror 0.005 --updatefreqs --poutformat 2 "V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\hogan_13_15_postQA_2013_2015_HOGAN.dat"

Do not use --fullsibtest if running on Kyle's Genetics Domain Computer, it won't save output!!!

The parameters are defined as follows:
--Nmmax and --Nfmax are the maximum numbers of candidate mothers and fathers. To obtain our values, we used what we used last time (23500). This escapement number came from rounding up the smaller of the two estimates (aerial survey AUC and stream walk AUC). This estimate is listed under 'pop est aerial' in the Excel file found here: "V:\Documents\5_Coastwide\Multispecies\AHRP\Field data\Sample Summary Extract and Genotype.xlsx"   
...
All output files can be found here: "V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\Hogan_2013_2015"  
...
parentage_13_15 <- read_csv("../../Franz/Hogan_2013_2015/parentage.csv")
paired_13_15 <- read_csv("../Franz/hogan_13_15_postQA_OceanAK_paired_2013_2015_HOGAN.csv")
```
...
write_csv(paired_13_15_filter, "hogan_paired_13_15_filter.csv")
```
...
write_csv(parents_paired_13_15, "hogan_parents_paired_13_15.csv")
...
**Note** that *FRANz* assigned 110 offspring to 29 parents. 104 of these offsrping have natural-origin parents and 6 offspring have hatchery-origin parents. The hatchery origin fish are from AFK11A, AFK11B, and WNH11PINKB.
...
O.odd <- c("H", "N")
Spawners.odd <- c(442, 321) #  number of hatchery- and natural-origin parents
Offspring.odd <- c(6, 104)  # number of offspring assigned to hatchery- and natural-origin parents
...
Here, we calculate and plot family size for both natural and hatchery parents.  Family size ranges from 1-41. Most assignments were to 1 parent (*single parent-offspring duos*) - there were 0 2 parent assignments (*parent pair-offspring trios*). 

Of the 3,775 offspring collected in 2015, 110 were assigned to parents, for an overall offspring assignment rate of **2.9%**. 
...
Female parents produced 1-41 offspring, while male parents produced 1-7.  
...
write_csv(paired_13_15_filter_parents, "hogan_paired_13_15_filter_parents.csv")
...
Calculate 95% CIs for Hogan 13-15 females. RRS = 0.03 (0.01-0.08)
...
rrs_ci_kalinowski(n_h_off = 4, n_w_off = 60, n_h_par = 280, n_w_par = 131, alpha = 0.05)
```

Calculate 95% CIs for Hogan 13-15 males. RRS = 0.05 (0.01-0.17)
...
rrs_ci_kalinowski(n_h_off = 2, n_w_off = 44, n_h_par = 162, n_w_par = 190, alpha = 0.05)
```
...
#CI's
rrs_ci_kalinowski(n_h_off = 6, n_w_off = 104, n_h_par = 442, n_w_par = 321, alpha = 0.05)
...
2. 104 offspring were assigned to 29 parents, for an assignment rate of **2.9%**.  
3. 0 offspring assigned to 2 parents (triads).  
4. RRS (with 0's) was significant for both females (0.03) and males (0.05).  
5. Family size varied from 0-41 for females and 0-7 for males.  
