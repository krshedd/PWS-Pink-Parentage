---
title: "Hogan 2015/2017 Parentage Analysis with 298 Markers"
output:
  html_notebook:
    theme: united
    toc: yes
...
## *FRANz* parameters
*FRANz* was run at Wed Nov 27 08:56:59 2019. We used the following parameters:
FRANz.exe --Nmmax 9500 --Nfmax 9500 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 "V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\hogan_postQA_2015_2017_HOGAN.dat"

Do not use --fullsibtest if running on Kyle's Genetics Domain Computer, it won't save output!!!

The parameters are defined as follows:
--Nmmax and --Nfmax are the maximum numbers of candidate mothers and fathers. To obtain our values, we used an estimated escapement of 19,000 from aerial survey and divided by 2 (*note: aerial survey was 18,998 and stream walk was 9,958*). This escapement number came from rounding up the larger of the two estimates (aerial survey AUC and stream walk AUC). This estimate is listed under 'pop est aerial' in the Excel file found here: "V:\Documents\5_Coastwide\Multispecies\AHRP\Field data\Sample Summary Extract and Genotype.xlsx"   
--femrepro and --malerepro specify the age range in which an individual can reproduce  
--typingerror refers to the overall genotyping error rate. Ours was ~0.006 (for Hogan was actually ???, from QC)
...
All output files can be found here: "V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\Hogan_2015_2017"  
...
parentage_15_17 <- read_csv("../../Franz/Hogan_2015_2017/parentage.csv")
paired_15_17 <- read_csv("../Franz/hogan_15_17_postQA_OceanAK_paired_2015_2017_HOGAN.csv")
```
...
write_csv(paired_15_17_filter, "hogan_paired_15_17_filter.csv")
```
...
write_csv(parents_paired_15_17, "hogan_parents_paired_15_17.csv")
...
**Note** that *FRANz* assigned 201 offspring to 202 parents. 221 of these offsrping have natural-origin parents and 41 offspring have hatchery-origin parents. The hatchery origin fish are from AFK13B, CCH13, and WNH13PINKB.
...
O.odd <- c("H", "N")
Spawners.odd <- c(4383, 3775) #  number of hatchery- and natural-origin parents
Offspring.odd <- c(41, 221)  # number of offspring assigned to hatchery- and natural-origin parents
...
Here, we calculate and plot family size for both natural and hatchery parents.  Family size ranges from 1-5. Most assignments were to 1 parent (*single parent-offspring duos*) - there were 61 2 parent assignments (*parent pair-offspring trios*). 

Of the 3,160 offspring collected in 2017, 262 were assigned to parents, for an overall offspring assignment rate of **8.3%**. 
...
write_csv(paired_15_17_filter_parents, "hogan_paired_15_17_filter_parents.csv")
...
Calculate 95% CIs for Hogan 15-17 females. RRS = 0.17 (0.10-0.26)
...
rrs_ci_kalinowski(n_h_off = 22, n_w_off = 94, n_h_par = 2417, n_w_par = 1717, alpha = 0.05)
```

Calculate 95% CIs for Hogan 15-17 males. RRS = 0.16 (0.09-0.25)
...
rrs_ci_kalinowski(n_h_off = 19, n_w_off = 127, n_h_par = 1965, n_w_par = 2056, alpha = 0.05)
```
...
write_csv(parents_paired_15_17_cross, "hogan_parents_paired_15_17_cross.csv")
...
```
Offspring from all four types of crosses are represented in our dataset. HH crosses produced 0 offspring, HN crosses produced 1 offspring, NH crosses produced 1 offspring, and NN crosses produced 1-5 offspring.
...
# HH vs. NN
rrs_ci_kalinowski(n_h_off = 2, n_w_off = 51, n_h_par = 2, n_w_par = 40, alpha = 0.05) # NS

# HN vs. NN
rrs_ci_kalinowski(n_h_off = 5, n_w_off = 51, n_h_par = 5, n_w_par = 40, alpha = 0.05) # NS

# NH vs. NN
rrs_ci_kalinowski(n_h_off = 3, n_w_off = 51, n_h_par = 3, n_w_par = 40, alpha = 0.05) # NS
```
...
```{r}
parents_paired_13_15 <- read_csv("hogan_parents_paired_13_15.csv") %>% 
  select(-origin)
...
None of the hatchery fish from 2013 produced grandoffspring. In 2013, hatchery-origin fish produced 6 offspring in 2015 and 0 grandoffspring in 2017. In 2013, natural-origin fish produced 104 offspring in 2015 and 5 grandoffspring in 2017.
...
2. 201 offspring were assigned to 202 parents, for an assignment rate of **6.5%**.  
3. 61 offspring assigned to 2 parents (triads).  
4. RRS was significant for both males (0.16) and females (0.17).  
5. There was not enough cross type data (triads) to make meaningful comparisons.  
6. Family size varied from 0-5 for females and 0-5 for males.  
7. Of the 110 fish from 2015 that were assigned as offspring from 2013 parents, 5 of them had offspring assigned in 2017. All 5 had 1 offspring only, all had natural origin parents. In other words, of the 104 offspring assigned to natural-origin, they produced 5 grandoffspring, and of the 6 offspring assigned to hatchery-origin, they produced 0 grandoffspring.
