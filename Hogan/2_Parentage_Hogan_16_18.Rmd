---
title: "Hogan 2016/2018 Single-generation and 2014/2016/2018 Multigenerational Parentage Analysis with 298 Markers"
...
We use the program *FRANz* [https://www.bioinf.uni-leipzig.de/Software/FRANz/About.html] to reconstruct parent-offspring relationships. Below are analyses of parentage for **Hogan** for the **odd lineage** years **2016** (potential parents) and **2018** (potential offspring).

*FRANz* was begun on Fri Jan 15 2021 at 15:51 using the following parameters in the command line in the V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\**Hogan_2016_2018** folder:
**FRANz --Nmmax 4500 --Nfmax 4500 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 "Hogan_16_18_postQA_2016_2018_HOGAN.dat"**

The parameters are defined as follows:

* --Nmmax and --Nfmax are the maximum numbers of candidate mothers and fathers. To obtain our values, we used an estimated escapement of 9,000 and divided by 2. This escapement number came from rounding up the stream walk survey number of 8,130; there was no aerial survey AUC. This estimate is listed under 'pop est walk' in the Excel file found here: "V:\Documents\5_Coastwide\Multispecies\AHRP\Field data\Sample Summary Extract and Genotype.xlsx".    
...
parentage_16_18 <- read_csv("../../Franz/Hogan_2016_2018/parentage.csv") 
paired_16_18 <- read_csv("../Franz/Hogan_16_18_postQA_OceanAK_paired_2016_2018_HOGAN.csv") 
```
...
paired_16_18_filter <- as_tibble(paired_16_18) %>% 
  ...
  mutate(date = mdy_hm(`Sample Date`)) %>%
  mutate(DOY = yday(date))
 
write_csv(paired_16_18_filter, "Hogan_paired_16_18_filter.csv") # mod Sample Date
```
...
## Sample sizes
  ...
  * Quick look at sample sizes for potential parents (2016) and potential offspring (2018)
...
sample_size_plot(indata = paired_16_18_filter, title = "Hogan 2016-2018")
```
# **Basic Statistics**
...
write_csv(parents_paired_16_18, "Hogan_parents_paired_16_18.csv")
...
parents_paired_16_18 %>% 
  ...
  spread(origin, n)
```
**Results:** Of the **7,547** natural-origin offspring genotyped in 2018, *FRANz* assigned **2,608**, for an assignment rate of **34.6%**, to **928** parents (**788** natural and **140** hatchery). Most assignments (**1,897**) were to one parent, but there were **711** two-parent assignments. A total of **2,817** offspring assignments were to a natural-origin parent and **502** were to a hatchery-origin parent, with **383** from AFK14B, **40** from CCH14, **24** from SGH14, and **55** from WNH14PINKB.
...
spawners_16 <- paired_16_18_filter %>% 
  filter(`Sample Year` == 2016) %>% 
  count(origin)

# How many offspring from each origin?
offspring_18 <- parents_paired_16_18 %>% 
  count(origin)
```

```{r chi_square_test, echo=FALSE, warning=FALSE, message=FALSE}
spawners_and_offspring_16_18 <- data.frame(spawners_16, offspring_18)
spawners_and_offspring_16_18 <- subset(spawners_and_offspring_16_18, select = c(n, n.1))

colnames(spawners_and_offspring_16_18) <- c("Hogan_spawners_16", "Hogan_offspring_18")
rownames(spawners_and_offspring_16_18) <- c("Natural", "Hatchery")

spawners_and_offspring_16_18
chisq.test(spawners_and_offspring_16_18)
```
**Results:** There was a highly significant difference in the proportions of offspring assigned to hatchery- and natural-origin parents relative to the proportions of potential parents sampled, indicating an under-representation of offspring assigning to hatchery-origin parents. Although **`r round(spawners_and_offspring_16_18["Hatchery", "Hogan_spawners_16"] / sum(spawners_and_offspring_16_18[, "Hogan_spawners_16"]), 2) * 100`%** of parents genotyped were of hatchery-origin, only **`r round(spawners_and_offspring_16_18["Hatchery", "Hogan_offspring_18"] / sum(spawners_and_offspring_16_18[, "Hogan_offspring_18"]), 2) * 100`%** of offspring assigned to hatchery-origin parents.
...
parents_paired_16_18 %>% 
  ...
  theme_bw()
```
**Results:** Female parents produced **0-28** offspring, while known male parents produced **0-25**. Wow!
...
write_csv(paired_16_18_filter_parents, "Hogan_paired_16_18_filter_parents.csv")
...
paired_16_18_filter_parents %>%
 ...
 geom_vline(xintercept=F_n_0_16_18, color=cbPalette2[1], linetype = "dashed", size = 2) +
 geom_vline(xintercept=F_h_0_16_18, size=2) +
 geom_vline(xintercept=F_h_0_16_18, color=cbPalette2[2], linetype = "dashed", size = 2) +
 annotate("text", x = F_n_0_16_18 + 0.5, y = .250, label = paste("Female_natural_RS =", round(F_n_0_16_18, 2)), hjust = 0, size = 6, color = cbPalette2[1]) +
 annotate("text", x = F_h_0_16_18 + 0.5, y = .300, label = paste("Female_hatchery_RS =", round(F_h_0_16_18, 2)), hjust = 0, size = 6, color = cbPalette2[2]) +
...
paired_16_18_filter_parents %>%
 ...
 geom_vline(xintercept=M_n_0_16_18, color=cbPalette2[1], linetype = "dashed", size = 2) +
 geom_vline(xintercept=M_h_0_16_18, size=2) +
 geom_vline(xintercept=M_h_0_16_18, color=cbPalette2[2], linetype = "dashed", size = 2) +
 annotate("text", x = M_n_0_16_18 + 0.5, y = .250, label = paste("Male_natural_RS =", round(M_n_0_16_18, 2)), hjust = 0, size = 6, color = cbPalette2[1]) +
 annotate("text", x = M_h_0_16_18 + 0.5, y = .300, label = paste("Male_hatchery_RS =", round(M_h_0_16_18, 2)), hjust = 0, size = 6, color = cbPalette2[2]) +
  theme(legend.position = c(0.8, 0.75), legend.background = element_rect(linetype = 1, size = 0.5, colour = 1)) +
  scale_fill_manual(values = cbPalette2)
```
...
```
**Results:** RRS for males = `r round(RRS_M_0_16_18, 2)`. RRS for females = `r round(RRS_F_0_16_18, 2)`.
...
RRS_0 %>%
  spread(Sex, p_value)
```
**Results:** There are significant differences in RRS for males, females, and overall in the permutation testing as well as for males and overall in the GLM. 
...
n_F_par <- paired_16_18_filter_parents %>% 
  # select(-n) %>% 
  filter(SEX == "F") %>% 
  count(SEX, origin) %>% 
  spread(origin, n)
...
n_F_off <- paired_16_18_filter_parents %>% 
  filter(SEX == "F") %>% 
  group_by(SEX, origin) %>% 
  summarise(n_off = sum(n)) %>% 
  spread(origin, n_off)

rrs_F_16_18 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(n_F_off[1,3]), n_w_off = as.integer(n_F_off[1,2]), n_h_par = as.integer(n_F_par[1,3]), n_w_par = as.integer(n_F_par[1,2]), alpha = 0.05))

# Males
# Get parent sample sizes
n_M_par <- paired_16_18_filter_parents %>% 
  # select(-n) %>% 
  filter(SEX == "M") %>% 
  count(SEX, origin) %>% 
  spread(origin, n)
...
# Get offspring sample sizes
n_M_off <- paired_16_18_filter_parents %>% 
  filter(SEX == "M") %>% 
  group_by(SEX, origin) %>% 
  summarise(n_off = sum(n)) %>% 
  spread(origin, n_off)

rrs_M_16_18 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(n_M_off[1,3]), n_w_off = as.integer(n_M_off[1,2]), n_h_par = as.integer(n_M_par[1,3]), n_w_par = as.integer(n_M_par[1,2]), alpha = 0.05))

rrs_16_18 <- cbind(rrs_F_16_18, rrs_M_16_18)

colnames(rrs_16_18) <- c("Hogan_females_16_18", "Hogan_males_16_18")
rownames(rrs_16_18) <- c("Lower 95% CL", "RRS", "Upper 95% CL")

rrs_16_18 %>% 
  round(2)
```
**Results:** Female RRS was **`r round(rrs_F_16_18[2,1], 2)` (95% CI [`r round(rrs_F_16_18[1,1], 2)`-`r round(rrs_F_16_18[3,1], 2)`])** and male RRS was **`r round(rrs_M_16_18[2,1], 2)` (95% CI [`r round(rrs_M_16_18[1,1], 2)`-`r round(rrs_M_16_18[3,1], 2)`])**.
...
```{r paired_16_18_filter_parents_by_sex, echo=FALSE, warning=FALSE, message=FALSE}
...
paired_16_18_filter_parents %>% 
  ...
  scale_fill_manual(values = cbPalette2)
```
**Results:* No obvious trends; so many zeroes.
...
triads_16_18 <- parents_paired_16_18 %>% 
...
parents_paired_16_18_cross <- parentage_16_18 %>% 
  filter(Posterior > 0.9) %>% 
  dplyr::select(Offspring, `Parent 1`, `Parent 2`) %>% 
  filter(Offspring %in% triads_16_18) %>% 
  left_join(paired_16_18_filter, by = c("Offspring" = "franz_id")) %>% 
...
write_csv(parents_paired_16_18_cross, "Hogan_parents_paired_16_18_cross.csv")
...
cross_count_16_18 <- parents_paired_16_18_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  rename(nn = n) %>% 
  count(cross) %>% 
  rename(samp = n)
...
parents_paired_16_18_cross %>% 
  ...
  group_by(cross) %>% 
  mutate(p = nn / sum(nn)) %>% 
  left_join(cross_count_16_18, by = "cross") %>% 
  ...
  scale_fill_manual(values = cbPalette1)
```
**Results:** Offspring from all four types of crosses are represented. The majority (68%) are NN, and there are only eight HH crosses.

### *Cross type RRS plus 95% CIs*
Below, we report the mean RS of each cross type and RRS (95% CI).
```{r RRS_confidence_intervals_16_18, echo=FALSE, warning=FALSE, message=FALSE}
# RS of cross types
(parents_paired_16_18_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  group_by(cross) %>% 
...
Xtype_nmat <- parents_paired_16_18_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  count(cross)

# Get offspring sample sizes
Xtype_noff <- parents_paired_16_18_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  group_by(cross) %>% 
  summarise(nn = sum(n))
...
# 1. HH vs. NN
rrs_HH_16_18 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(Xtype_noff[1,2]), n_w_off = as.integer(Xtype_noff[4,2]), n_h_par = as.integer(Xtype_nmat[1,2]), n_w_par = as.integer(Xtype_nmat[4,2]), alpha = 0.05)) 

# 2. HN vs. NN
rrs_HN_16_18 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(Xtype_noff[2,2]), n_w_off = as.integer(Xtype_noff[4,2]), n_h_par = as.integer(Xtype_nmat[2,2]), n_w_par = as.integer(Xtype_nmat[4,2]), alpha = 0.05))

# 3. NH vs. NN
rrs_NH_16_18 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(Xtype_noff[3,2]), n_w_off = as.integer(Xtype_noff[4,2]), n_h_par = as.integer(Xtype_nmat[3,2]), n_w_par = as.integer(Xtype_nmat[4,2]), alpha = 0.05))

rrs_Xtype_16_18 <- cbind(rrs_HH_16_18, rrs_HN_16_18, rrs_NH_16_18)

colnames(rrs_Xtype_16_18) <- c("Hogan_HHvsNN_16_18", "Hogan_HNvsNN_16_18", "Hogan_NHvsNN_16_18")
rownames(rrs_Xtype_16_18) <- c("Lower 95% CL", "RRS", "Upper 95% CL")

rrs_Xtype_16_18 %>% 
  round(2)
```
**Results:** RRS for the comparisons were **`r round(rrs_Xtype_16_18[2,1], 2)` (95% CI [`r round(rrs_Xtype_16_18[1,1], 2)`-`r round(rrs_Xtype_16_18[3,1], 2)`])** for HH, **`r round(rrs_Xtype_16_18[2,2], 2)` (95% CI [`r round(rrs_Xtype_16_18[1,2], 2)`-`r round(rrs_Xtype_16_18[3,2], 2)`])** for HN and **`r round(rrs_Xtype_16_18[2,3], 2)` (95% CI [`r round(rrs_Xtype_16_18[1,3], 2)`-`r round(rrs_Xtype_16_18[3,3], 2)`])** for NH, indicating no significant differences in RS relative to the NN cross type. It should be noted, however, that the numbers of HH crosses relative to all other crosses were very small.
...
(n_unique_matings_16_18 <- parents_paired_16_18_cross_dam_sire %>% 
  count(Dam, Sire))
...
n_sires_16_18 <- parents_paired_16_18_cross_dam_sire %>% 
  group_by(Dam) %>% 
  summarise(n_sires = n_distinct(Sire)) %>% 
  count(n_sires) %>% 
  rename(n_mates = n_sires, n_dams_16_18 = n)

# How many sires had multiple dams
n_dams_16_18 <- parents_paired_16_18_cross_dam_sire %>% 
  group_by(Sire) %>% 
  summarise(n_dams = n_distinct(Dam)) %>% 
  count(n_dams) %>%
  rename(n_mates = n_dams, n_sires_16_18 = n)

(n_dams_and_sires_16_18 <- full_join(n_sires_16_18, n_dams_16_18, by = "n_mates"))
```
**Results:** The relationship between dam and sire sample date was linear, indicating the cross data are likely correct. Sampled fish mated with 1-4 partners. There were **270** unique matings accounting for all 711 offspring, and both sexes mated with multiple partners. A total of 184 females and 198 males mated with one partner, 28 females and 28 males mated with two partners, 6 females and 4 males mated with three partners, and 3 females and 1 male mated with four partners. Note also that the question can be further conceptualized, if warranted, using a pedigree visualization platform (e.g. Helium). 
...
```
**Results:** Of those dams with multiple mates, 18 matings were with a sire of a different origin.
...
```
**Results:** Of those sires with multiple mates, 15 matings were with dams of a different origin.

# **Grandparentage**
We find the parents (2016) that were assigned to grandparents (2014) and determine how many grandoffspring (2018) were assigned to them. We then estimate RRS according to grandparental origin. The set of panels below include:
```{r import_grandparent_data, echo=FALSE, warning=FALSE, message=FALSE}
# Read in data
parents_paired_14_16 <- read_csv("hogan_parents_paired_14_16.csv") %>% 
  dplyr::select(-origin)

paired_16_18_filter_parents <- read_csv("Hogan_paired_16_18_filter_parents.csv")

parentage_paired_16_18 <- read_csv("Hogan_parents_paired_16_18.csv")%>% 
  dplyr::select(-origin)
```
...
(gparent_table <- parentage_paired_16_18 %>% 
...
(gparents2 <- paired_16_18_filter_parents %>% 
...
(gparents_origin <- paired_16_18_filter_parents %>% 
  right_join(parents_paired_14_16, by = c("franz_id" = "Offspring")) %>% 
  group_by(origin.par) %>% 
...
rrs_gpar_16_18 <- as.data.frame(rrs_ci_kalinowski(n_h_off = as.integer(gparents_origin[1,4]), n_w_off = as.integer(gparents_origin[2,4]), n_h_par = as.integer(gparents_origin[1,3]), n_w_par = as.integer(gparents_origin[2,3]), alpha = 0.05))

colnames(rrs_gpar_16_18) <- c("Hogan_grandparents_16_18")
rownames(rrs_gpar_16_18) <- c("Lower 95% CL", "RRS", "Upper 95% CL")
...
sires_16 <- parentage_paired_16_18 %>%
    filter(Sex.par == "Male") %>%
    rename(Sire = Parent_ID, Sire_origin = origin.par) %>%
    dplyr::select(Offspring, Sire, Sire_origin)

# Grandoffspring, dam, and dam origin
dams_16 <- parentage_paired_16_18 %>%
    filter(Sex.par == "Female") %>%
    rename(Dam = Parent_ID, Dam_origin = origin.par) %>%
    dplyr::select(Offspring, Dam, Dam_origin)

# Grandsire, granddam, sire, and dam data combined
(grandparentage <- full_join(sires_16, dams_16, by = "Offspring") %>% 
    arrange(Offspring) %>% 
...
write_csv(grandparentage, "Hogan_16_18_grandparentage.csv")
```
...
```
**Results:** A total of **112** unique hatchery-origin grandparents were assigned to **265** parents, **22** of which assigned to **56** grandoffspring, and **72** unique natural-origin grandparents assigned to **208** parents, **16** of which assigned to **60** grandoffspring. Each assigned grandparent contributed to **1-23** grandoffspring. RRS from the parental to grandoffspring generation was **`r round(rrs_gpar_16_18[2,1], 2)` (95% CI [`r round(rrs_gpar_16_18[1,1], 2)`-`r round(rrs_gpar_16_18[3,1], 2)`])**. However, this result requires circumspection and cautious interpretation. It is not a pure RRS estimate because it is based on a fraction of full familial origin/ancestry information. Only one F~1~ parent is known for the majority of grandoffpring; an unknown parent may be of either origin and may have been sampled or unsampled. Compounding that, **100** grandoffspring had one and **2** had two grandparents assigned. No grandoffspring had three or four grandparents identified. Again, any unknown grandparent(s) are of unknown origin.

# **Great Grandparentage**

**Note: this section is N/A to the Hogan even lineage**
...
# # Read in data
# parents_paired_13_14 <- read_csv("hogan_parents_paired_13_14.csv") %>%
#   dplyr::select(-origin)
# 
# parents_paired_14_16 <- read_csv("hogan_parents_paired_14_16.csv") %>%
#   dplyr::select(-origin)
# 
# parents_paired_16_18 <- read_csv("Hogan_parents_paired_16_18.csv") %>%
#   dplyr::select(-origin)
# 
# paired_16_18_filter_parents <- read_csv("Hogan_paired_16_18_filter_parents.csv")
# 
# paired_14_16_filter_parents <- read_csv("hogan_paired_14_16_filter_parents.csv")
```
...
# ggparents_IDs <- parents_paired_14_16 %>%
#   inner_join(parents_paired_13_14, by = c("Parent_ID" = "Offspring"), suffix = c("_14", "_13"))
# 
# (ggparents_table <- parents_paired_16_18 %>%
#   inner_join(parents_paired_14_16, by = c("Parent_ID" = "Offspring"), suffix = c("_16", "_14")) %>% # same as in gparent_table from Grandparentage above
#   inner_join(ggparents_IDs, by = c("Parent_ID_14" = "Parent_ID")))
```
...
# (ggparents_origin <- paired_16_18_filter_parents %>%
#   full_join(parents_paired_14_16, by = c("franz_id" = "Offspring")) %>%
#   right_join(parents_paired_13_14, by = c("Parent_ID" = "Offspring")) %>%
...
2. 2,608 of 7,547 total offspring were assigned to 928 parents (788 natural and 140 hatchery), for an assignment rate of 34.6%.  
3. 711 offspring assigned to 2 parents (triads).  
4. RRS was significant for females (0.76) and males (0.68).  
...
6. Family size varied from 0-28 for females and 0-25 for males.  

## 2014-2016-2018 (Grandparentage)

1. A total of 184 unique grandparents were identified that produced 116 grandoffspring assignments, 100 of which had one 8 of which had two grandparents assigned.
2. 2016 to 2018 RRS was 0.73 (95% CI [0.51, 1.05]) when taken in light of grandparental origin.
