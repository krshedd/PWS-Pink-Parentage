---
title: "Hogan 2017/2019 Single-generation and 2013/2015/2017/2019 Multigenerational Parentage Analysis with 298 Markers"
...
#    a. Specific terms requiring replacement in the Introduction and below for future use include: "Hogan", "HOGAN", 2013", "2015", 2017", "2019", "_13", "_15", "_17", "_19"
...
We use the program *FRANz* [https://www.bioinf.uni-leipzig.de/Software/FRANz/About.html] to reconstruct parent-offspring relationships. Below are analyses of parentage for **Hogan** for the **odd lineage** years **2017** (potential parents) and **2019** (potential offspring).

*FRANz* was begun on Fri Jan 15 2021 at 15:51 using the following parameters in the command line in the V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\**Hogan_2017_2019** folder:
**FRANz --Nmmax 13000 --Nfmax 13000 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 "Hogan_17_19_postQA_2017_2019_HOGAN.dat"**
...
parentage_17_19 <- read_csv("V:/Analysis/5_Coastwide/Multispecies/Alaska Hatchery Research Program/PWS Pink/Franz/Hogan_2017_2019/parentage.csv") # for some reason, I keep getting and error with a relative path
paired_17_19 <- read_csv("../Franz/Hogan_17_19_postQA_OceanAK_paired_2017_2019_HOGAN.csv") # works for this one, tho
```
...
write_csv(paired_17_19_filter, "Hogan_paired_17_19_filter.csv")
```
...
sample_size_plot <- function(indata = paired_XX_XX_filter, title = "Stream Year-Year") {
  ...
  mutate(Sex = case_when(SEX == "M" ~ "Male", SEX == "F" ~ "Female")) %>%
  mutate(date = ymd(`Sample Date`)) %>% # imported data format for Sample Date changing - make sure to check
  mutate(DOY = yday(date)) %>%
  ggplot(aes(x = DOY)) +
...
sample_size_plot(indata = paired_17_19_filter, title = "Hogan 2017-2019")
```
# **Basic Statistics**
...
write_csv(parents_paired_17_19, "Hogan_parents_paired_17_19.csv")
...
parents_paired_17_19 %>% 
  ...
  spread(origin, n)
```
**Results:** Of the **4,197** natural-origin offspring genotyped in 2019, *FRANz* assigned **129**, for an assignment rate of **3.1%** (ugh!), to **64** parents (**61** natural and **3** hatchery). Most assignments (**126**) were to one parent only, but there were **3** two-parent assignments. A total of **127** offspring assignments were to a natural-origin parent and **5** were to a hatchery-origin parent, with **2** from AFK15B, **0** from CCH15, **3** from SGH15, and **0** from WNH15PINKB.
...
colnames(spawners_and_offspring_17_19) <- c("Hogan_spawners_17", "Hogan_offspring_19")
...
parents_paired_17_19 %>% 
  ...
  theme_bw()
```
**Results:** Female parents produced **0-9** offspring, while known male parents produced **0-7**.  
...
write_csv(paired_17_19_filter_parents, "Hogan_paired_17_19_filter_parents.csv")
...
paired_17_19_filter_parents %>%
 ...
 annotate("text", x = M_h_0_17_19 + 0.5, y = .300, label = paste("Male_hatchery_RS =", round(M_h_0_17_19, 2)), hjust = 0, size = 6, color = cbPalette2[2]) +
  ...
  scale_fill_manual(values = cbPalette2)
```
**Results:** RSs for natural-origin fish for both sexes were higher than for hatchery-origin fish, but our sample sizes are low!!! In the histograms, note the large number of potential parents assigned no offspring that were absent from the family size plots above.
...
RRS_0 %>%
  spread(Sex, p_value)
```
**Results:** There are significant differences in RRS for both males and females as well as overall (of course). 
...
colnames(rrs_17_19) <- c("Hogan_females_17_19", "Hogan_males_17_19")
...
paired_17_19_filter_parents <- paired_17_19_filter_parents %>% 
  ...
  mutate(sex = case_when(SEX == "M" ~ "Male",
                         SEX == "F" ~ "Female")) %>% 
  mutate(date = ymd(`Sample Date`)) # check date format again
```
...
paired_17_19_filter_parents %>% 
  ...
  scale_fill_manual(values = cbPalette2)
```
**Results:* No trends; too many zeroes.
...
write_csv(parents_paired_17_19_cross, "Hogan_parents_paired_17_19_cross.csv")
...
parents_paired_17_19_cross %>% 
  ...
  scale_fill_manual(values = cbPalette1)
```
**Results:** Offspring from only one type of cross (NN) is represented.
...
# 
# colnames(rrs_Xtype_17_19) <- c("Hogan_HNvsNN_17_19", "Hogan_NHvsNN_17_19")
# rownames(rrs_Xtype_17_19) <- c("Lower 95% CL", "RRS", "Upper 95% CL")
# 
# rrs_Xtype_17_19 %>% 
#   round(2)
```
**Results:** There was only one cross type, so this step is uninformative.
...
parents_paired_17_19_cross_dam_sire %>% 
  mutate(`Sample Date_dam` = ymd(`Sample Date_dam`),
         `Sample Date_sire` = ymd(`Sample Date_sire`)) %>% # check date format again
...
```
**Results:** This analysis was largely pointless given the number of triads. However, it can be noted that one female mated with two males and another female mated with one other different male. Note also that the question can be further conceptualized, if warranted, using a pedigree visualization platform (e.g. Helium). 
...
# Read in data
parents_paired_15_17 <- read_csv("hogan_parents_paired_15_17.csv") %>% 
  dplyr::select(-origin)

paired_17_19_filt_parents <- read_csv("Hogan_paired_17_19_filter_parents.csv")

parentage_paired_17_19 <- read_csv("Hogan_parents_paired_17_19.csv")%>% 
  dplyr::select(-origin)
```
...
(gparents2 <- paired_17_19_filt_parents %>% 
...
(gparents_origin <- paired_17_19_filt_parents %>% 
  right_join(parents_paired_15_17, by = c("franz_id" = "Offspring")) %>% 
  group_by(origin.par) %>% 
...
colnames(rrs_gpar_17_19) <- c("Hogan_grandparents_17_19")
...
write_csv(grandparentage, "Hogan_17_19_grandparentage.csv")
```
...
```
**Results:** A total of **26** unique hatchery-origin grandparents were assigned to **41** parents, **3** of which assigned to **4** grandoffspring, and **165** unique natural-origin grandparents assigned to **221** parents, **24** of which assigned to **62** grandoffspring. Each assigned grandparent contributed to **1-9** grandoffspring. RRS from the parental to grandoffspring generation was **`r round(rrs_gpar_17_19[2,1], 2)` (95% CI [`r round(rrs_gpar_17_19[1,1], 2)`-`r round(rrs_gpar_17_19[3,1], 2)`])**. However, this result requires circumspection and cautious interpretation. It is not a pure RRS estimate because it is based on a fraction of full familial origin/ancestry information. Only one F~1~ parent is known for the majority of grandoffpring; an unknown parent may be of either origin and may have been sampled or unsampled. Compounding that, **35** grandoffspring had one , **14** had two , and **one** grandoffspring had three grandparents assigned. None had all four grandparents assigned. Again, any unknown grandparent(s) are of unknown origin.
...
parents_paired_17_19 <- read_csv("Hogan_parents_paired_17_19.csv") %>% 
  dplyr::select(-origin)

paired_17_19_filter_parents <- read_csv("Hogan_paired_17_19_filter_parents.csv")

paired_15_17_filter_parents <- read_csv("hogan_paired_15_17_filter_parents.csv")
```
...
```
**Results:** We found zero great grandparents.
...
2. 129 of 4,197 total offspring were assigned to 64 parents (61 natural and 3 hatchery), for an assignment rate of 3.1%.  
3. 3 offspring assigned to 2 parents (triads).  
4. RRS was significant for females (0.07) and males (0.18).  
5. Only the NN cross (n = 3) was apparent in the data, so there was not enough cross type data (triads) to make meaningful comparisons.  
6. Family size varied from 0-9 for females and 0-7 for males.  

## 2015-2017-2019 (Grandparentage)

1. A total of 26 unique grandparents were identified that produced 66 grandoffspring, 35 of which had one, 14 of which had two, and one of which had three grandparents assigned.
2. 2017 to 2019 RRS was 0.35 (95% CI [0.11, 0.84]), which is significant, when taken in light of grandparental origin.

## 2013-2015-2017-2019 (Great grandparentage)

1. We found zero great grandparents.
