...
# Duplicates
In our analysis, we mistakenly retained duplicate individuals that now need to be removed, post-*FRANz*. We followed the standard GCL protocol of detecting duplicates and removing the one with the least amount of genotypes. While this is ideal for MSA applications where we want to 1) retain the maximum number of individuals, and 2) do not need to worry about paired data (ASL, origin, etc.); this approach does NOT work for parentage. For this RRS study, the paired data is *critical*, thus, if we have any genetic duplicates, we can not be sure which individual has the **correct** metadata. We need to remove *both* individuals. Since we only removed one individual from each duplicate pair before we ran *FRANz*, we now need to remove the other individual from each pair prior to calculating RRS.
...
(duplicates_to_remove_odd_extra <- duplicates_to_remove %>% 
  ...
  dplyr::left_join(paired_13_15_filter, by = c("DNA_TRAY_CODE" = "DNA Tray Code", "DNA_TRAY_WELL_CODE" = "DNA Tray Well Code")) %>% 
  tidyr::drop_na(franz_id))

dups_odd_franz_id <- duplicates_to_remove_odd_extra %>% 
  pull(franz_id)

# Remove these "extra" duplicates from `paired_13_15_filter`
paired_13_15_filter <- paired_13_15_filter %>% 
  filter(!franz_id %in% dups_odd_franz_id)
```
...
# Did any of the "extra" duplicates have parent/offpsring relationships?
if(any(dups_odd_franz_id %in% c(parentage_13_15_filter$Offspring, parentage_13_15_filter$Parent_ID))) {
  print(paste(sum(dups_odd_franz_id %in% parentage_13_15_filter$Offspring), "'extra' duplicates were offspring"))
  print(paste(sum(dups_odd_franz_id %in% unique(parentage_13_15_filter$Parent_ID)), 
              "'extra' duplicates were parents, and they had", 
              sum(parentage_13_15_filter$Parent_ID %in% dups_odd_franz_id), "total offspring"))
  
  # remove "extra" duplicates
  parentage_13_15_filter <- parentage_13_15_filter %>% 
    filter(!Offspring %in% dups_odd_franz_id) %>% 
    filter(!Parent_ID %in% dups_odd_franz_id)
} else {
...
write_csv(paired_13_15_filter_parents, "paired_13_15_filter_parents.csv")
...
parents_paired_14_16 %>% 
  ...
  spread(origin, n)
```
**Note** that *FRANz* assigned 454 offspring to 185 parents in single parent assignments (*single parent-offspring duos*). 209 of these assignments are to natural origin parents and 267 assignments were to hatchery origin parents. Hatchery origin parents are from AFK12B, CCH12, and WNH12PINKB, none are from Solomon Gulch (VFDA).  
...
Of the 4,020 offspring collected in 2016, 454 were assigned to at least 1 parent, for an an overall offspring assignment rate of **11%**.  
...
aov.rs <- aov(n ~ lineage * origin * SEX.par * `Length Mm.par` * `Sample Date.par`, RS_combined_count)
summary(aov.rs)
```
...
aov.rs.2 <- aov(n ~ lineage + origin + SEX.par + `Length Mm.par` + `Sample Date.par`, RS_combined_count)
summary(aov.rs.2)
```
