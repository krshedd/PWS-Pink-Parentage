...
pTop <- ggplot(finsight_parentage_hogan, aes(x = `Sample Date`)) + 
                 geom_histogram()
pRight <- ggplot(finsight_parentage_hogan, aes(x = Distance)) +
                   geom_histogram() +
                   coord_flip()
pEmpty <- ggplot(finsight_parentage_hogan, aes(x = `Sample Date`, y = Distance)) + 
  geom_blank() +
...
ggpairs(reduced, columns=c("Sample Date", "Length Mm", "RS", "Distance"), ggplot2::aes(colour = origin, shape = SEX))
```
...
pairs(reduced[, c(1,3,5,6)], diag.panel = panel.hist, lower.panel = panel.smooth, upper.panel = panel.cor, main = "All Individuals Pooled")
...
These plots show males and females of both origins pooled. Histograms of each variable's distribution appear on the diagonal. The lower panels show scatterplots with smoothed averages. The upper panels show correlations, with the size of the number representing the strength of the correlation. The strongest correlation is between sample date and length (0.28), indicating that on average larger individuals were sampled later. 
...
pairs(Male.even[, c(1,3,5,6)], diag.panel = panel.hist, lower.panel=panel.smooth, 
upper.panel=panel.cor, main = "Males")
```

For males, the strongest correlation was between length and sample date (0.43), indicating that larger individuals were sampled later. 
...
pairs(Female.even[, c(1,3,5,6)], diag.panel = panel.hist, lower.panel=panel.smooth, 
upper.panel=panel.cor, main = "Females")
```

For females, distance was negatively correlated with RS (n; -0.074) and sample date (-0.10), indicating that females were sampled closer to the mouth of the stream later in the season and that RS was lower later in the season. 
...
pairs(Hatchery.even[, c(1,3,5,6)], diag.panel = panel.hist, lower.panel=panel.smooth, 
upper.panel=panel.cor, main = "Hatchery")
```

For hatchery-origin individuals, the strongest relationships were between sample date and length (0.17) and distance (-0.16). Larger hatchery-origin fish were sampled later in the season and more samples were collected near the mouth of the stream later in the season. 
...
pairs(Natural.even[, c(1,3,5,6)], diag.panel = panel.hist, lower.panel=panel.smooth, 
upper.panel=panel.cor, main = "Natural")
```

For natural-origin individuals, the strongest relationship was between sample date and length (0.31), with larger individuals sampled later in the season.  
...
finsight_parentage_hogan_sw <- finsight_parentage_hogan %>% 
  dplyr::mutate(statweek = dplyr::case_when(
    ...
    `Sample Date` == "2014-08-25" | `Sample Date` == "2014-08-27" | `Sample Date` == "2014-08-28" | `Sample Date` == "2014-08-31" ~ "35",
    `Sample Date` == "2014-09-03" | `Sample Date` == "2014-09-6" ~ "36",
    `Sample Date` == "2014-09-10" | `Sample Date` == "2014-09-11" | `Sample Date` == "2014-09-14" ~ "37"))
...
finsight_parentage_hogan_sw %>% 
  ...
  geom_jitter() +
  geom_smooth() +
  facet_grid(vars(statweek), vars(SEX)) +
  labs(title = "Reproductive Success by Parental Sample Location",  colour = "Parent: Origin",
       shape = "Parent: Sex") +
  xlab("Distance from Stream Mouth (m)") +
  ylab("Reproductive Success")
...
Female.Weights <- round(Weights(AICc(RS.glm.full, RS.glm.2, RS.glm.3, RS.glm.4, RS.glm.5, RS.glm.6, RS.glm.7, RS.glm.8, RS.glm.9, RS.glm.10, RS.glm.11, RS.glm.12, RS.glm.13, RS.glm.14, RS.glm.15, RS.glm.16)),3)
...
Male.Weights <- round(Weights(AICc(RS.glm.full, RS.glm.2, RS.glm.3, RS.glm.4, RS.glm.5, RS.glm.6, RS.glm.7, RS.glm.8, RS.glm.9, RS.glm.10, RS.glm.11, RS.glm.12, RS.glm.13, RS.glm.14, RS.glm.15, RS.glm.16)),3)
