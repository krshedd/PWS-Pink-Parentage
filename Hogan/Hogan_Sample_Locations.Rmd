...
In summary, we see that while Hogan Bay is a very short stream (only ~30m of spawning habitat), there are some spatial patterns in RS, with higher average RS in the intertidal than upstream, particularly with natural-origin fish. Natural-origin fish tended to spawn lower down in the stream for most sample dates. When controlling for sample date and intertidal vs. upstream, natural-origin females have a higher RS than hatchery-origin females early in the season in the intertidal. Thus spawning location did not change the previously found relationship that female natural-origin fish have a higher RS than hatchery-origin fish early in the season.

Hower, negative binomial GLMs did not explain very much of the variance in RS for either females or males (*note* the small difference between residual deviance and null deviance). With that said, the highest ranked models for females always included origin, often included sample date and length, and occasionally included one of the location variables (Intertidal[categorical] or Distance[continuous]). For males, the highest ranked models always included length.

We will use this R Notebook as a template to explore Stockdale Creek even-lineage (2014/2016). Stockdale should be a much more intersting analysis given both the higher sample size of parent/offspring relationships and the longer stream length.