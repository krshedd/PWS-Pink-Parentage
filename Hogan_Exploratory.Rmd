...
Simple histogram using *base* plotting
...
Switch from *base* to *tidyverse* for ease of use/visualization. Add +- 3SD cutoff from literature.
...
het_tibble %>% 
  group_by(lineage) %>% 
  summarise(upper = mean(het) + 3* sd(het)) %>% 
  right_join(het_tibble) %>% 
  filter(het >= upper) %>% 
  count(lineage)
```
...
We can see that the upper bound is virtually identical for both Chris' method and IQR. I propose that we go with the 1.5 IQR method given that it is well known and well documented. Thoughts?
## OceanAK
...
## Note, you will want to use dplyr::left_join here, not merge, left_join is safer by far
