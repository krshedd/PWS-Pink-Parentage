...
```{r save_map_pdf}
pdf(file = "V:/Documents/5_Coastwide/Multispecies/AHRP/Manuscripts/02.Hogan_Stockdale_EA/0002.Figures&Tables/Figures/Figure_1.pdf", width = 6.5, height = 6.5)
grid.newpage()
vp_b <- viewport(width = 1, height = 1, x = 0.5, y = 0.5)  # the larger map
vp_a <- viewport(width = 0.23, height = 0.35, x = 0.63, y = 0.31)  # the inset in lower left
print(pws_map, vp = vp_b)
...
Hogan_Odd_RS <- hogan_paired_13_15_filter_parents %>% 
  ...
  scale_x_continuous(limits = c(-0.5, max(hogan_paired_13_15_filter_parents$n) + 0.5), breaks = seq(0,40, by = 5)) +
...
Hogan_Even_RS <- paired_14_16_filter_parents %>% 
  ...
  theme(legend.position = 'none',
        panel.grid = element_blank()) +
  scale_y_continuous(limits = c(0,1), breaks = seq(0,1, by = 0.1)) +
  scale_x_continuous(limits = c(-0.5, max(paired_14_16_filter_parents$n) + 0.5), breaks = seq(0,20, by = 2)) +
  facet_grid(~ SEX) +
...
Stockdale_Odd_RS <- stock_paired_13_15_filter_parents %>% 
  ...
  theme(legend.position = 'none',
        panel.grid = element_blank()) +
  scale_y_continuous(limits = c(0,1), breaks = seq(0,1, by = 0.1)) +
  scale_x_continuous(limits = c(-0.5, max(stock_paired_13_15_filter_parents$n) + 0.5), breaks = seq(0,20, by = 2)) +
  facet_grid(~ SEX, labeller = labeller(F = "Female", M = "Male")) +
  labs(title="Stockdale",
       fill = "Parent origin") +
  ylab(" ") +
