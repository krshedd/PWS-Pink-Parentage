...
map_data <- read_csv("V:/Analysis/5_Coastwide/Multispecies/Alaska Hatchery Research Program/PWS Pink/GitHub-PWS-Pink-Parentage/map_data.csv") 
...
## Quick check for missed duplicates
It does not look like any of the "extra" duplicates that we needed to remove had any parent/offspring relationships.

```{r dups_odd}
duplicates_to_remove %>% 
  dplyr::filter(SILLY_CODE %in% c("PHOGAN13", "PHOGAN15")) %>% 
  dplyr::select(SILLY_CODE, SillySource)

# Are parents in dups?
parentage_13_15_filter %>% 
  filter(Parent_ID %in% c("PH13_00560", "PH13_00560", "PH13_00820", "PH13_00822"))

# Are offspring in dups
parentage_13_15_filter %>% 
  filter(Offspring %in% c("PH15_03217", "PH15_03217"))
```
...
(parents_paired_13_15 <- parentage_13_15_filter %>% 
   ...
   mutate(origin = case_when(`Otolith Mark Present.y` == "NO" ~ "Natural",
                             `Otolith Mark Present.y` == "YES" ~ "Hatchery")) %>% 
...
parents_paired_13_15 %>% 
  count(origin, `Otolith Mark ID.y`) %>% 
  spread(origin, n)
```
...
O.odd <- c("H", "N")
Spawners.odd <- c(445, 322) #  number of hatchery- and natural-origin parents
Offspring.odd <- c(3, 45)  # nuber of offspring assigned to hatchery- and natural-origin parents
...
parents_paired_13_15 %>% 
  count(`Fish ID.y`, origin, SEX.y) %>% 
  count(origin, SEX.y, n) %>% 
  complete(origin, SEX.y, n, fill = list(nn = 0)) %>% 
...
parents_paired_13_15 %>% 
  count(`Fish ID.y`, origin, SEX.y) %>% 
  count(SEX.y, origin, n) %>% 
  group_by(SEX.y, origin) %>% 
  mutate(p = nn / sum(nn)) %>% 
...
(rs_13_15 <- parents_paired_13_15 %>%
  count(`Fish ID.y`, origin, SEX.y) %>%
  group_by(origin, SEX.y) %>% 
  summarise(RS = mean(n))) %>% 
  spread(SEX.y, RS)
  

F_h_odd <- rs_13_15 %>% 
  filter(origin == "Hatchery" & SEX.y == "F") %>% 
  pull(RS)
M_h_odd <- rs_13_15 %>% 
  filter(origin == "Hatchery" & SEX.y == "M") %>% 
  pull(RS)
F_n_odd <- rs_13_15 %>% 
  filter(origin == "Natural" & SEX.y == "F") %>% 
  pull(RS)
M_n_odd <- rs_13_15 %>% 
  filter(origin == "Natural" & SEX.y == "M") %>% 
  pull(RS)
```
...
parents_paired_13_15_count <- parents_paired_13_15 %>% 
  count(Parent_ID, origin, SEX.y, `Sample Year.y`, `Length Mm.y`, `Sample Date.y`) %>% 
...
write.csv(paired_13_15_filter_parents, "paired_13_15_filter_parents.csv")

# Mean RS including 0's
rs_13_15_0 <- paired_13_15_filter_parents %>%
  group_by(origin.x, SEX) %>% 
...
# Plot histogram of proportion of parents with a given family size
paired_13_15_filter_parents %>% 
  count(SEX, origin.x, n) %>% 
  group_by(SEX, origin.x) %>% 
  mutate(p = nn / sum(nn)) %>% 
  ggplot(aes(x=n, y=p, fill = origin.x)) +
  geom_col(position = position_dodge2(preserve="single")) +
  scale_y_continuous(breaks = pretty_breaks()) +
  scale_x_continuous(breaks = pretty_breaks()) +
  facet_grid(~ SEX) +
...
F_h_0_13_15 <- rs_13_15_0 %>% 
  filter(origin.x == "Hatchery" & SEX == "F") %>% 
  pull(RS)
M_h_0_13_15 <- rs_13_15_0 %>% 
  filter(origin.x == "Hatchery" & SEX == "M") %>% 
  pull(RS)
F_n_0_13_15 <- rs_13_15_0 %>% 
  filter(origin.x == "Natural" & SEX == "F") %>% 
  pull(RS)
M_n_0_13_15 <- rs_13_15_0 %>% 
  filter(origin.x == "Natural" & SEX == "M") %>% 
  pull(RS)
```
...
perm_1tail_pvalue_M_0 <- round(x = as.numeric(coin::pvalue(oneway_test(n ~ origin.x, data = mydata_M_0, distribution = approximate(B = 10000), alternative = "greater"))), digits = 4)

perm_1tail_pvalue_F_0 <- round(x = as.numeric(coin::pvalue(oneway_test(n ~ origin.x, data = mydata_F_0, distribution = approximate(B = 10000), alternative = "greater"))), digits = 4)

#And to do a 1-tailed negative binomial GLM

fit_M_0 <- glm.nb(n ~ origin.x, data = mydata_M_0, init.theta = 1, link = log)
nbGLM_1tail_pvalue_M_0 <- round(summary(fit_M_0)$coefficients[2, 4], 4)

fit_F_0 <- glm.nb(n ~ origin.x, data = mydata_F_0, init.theta = 1, link = log)
...
parents_paired_13_15 %>% 
  ...
  ggplot(aes(x=`Length Mm.y`, y=`Length Mm.x`, colour = origin, shape = SEX.x))+
  geom_point(size = 2)+
...
parents_paired_13_15 %>% 
  filter(`Length Mm.y` != "NA") %>% 
  filter(SEX.y == "M") %>% 
  ggplot(aes(x=`Length Mm.y`, y=`Length Mm.x`, colour = SEX.x))+
...
```
Hatchery males and females are larger than natural males and females. Both hatchery and natural females are larger than males, but just by a few mm. This is probably not a biologically significant difference.  
...
write.csv(parents_paired_13_15, "parents_paired_13_15.csv")
```


```{r Examine sample date Odd}
parents_paired_13_15 %>% 
  ggplot(aes(x=`Sample Date.y`, y=`Sample Date.x`, colour = origin, shape = SEX.x))+
...
write.csv(paired_13_15_filter, "paired_13_15_filter.csv")
...
lm.date.length.odd <- lm(`Length Mm.y`~ `Sample Date.y`, parents_paired_13_15)
summary(lm.date.length.odd)
```

There is a significant relationship between parental sample date and length (p=0.033), although the effect is very small (fractions of a mm per day). 

```{r Fit LM for parental sample date and length + sex + origin}
lm.date.length.odd <- lm(`Length Mm.y`~ `Sample Date.y` + SEX.y + origin, parents_paired_13_15)
summary(lm.date.length.odd)
```

However, this relationship breaks down a bit if we add sex and origin as variables. 

```{r Calculate mean length by sample date odd}
parents_paired_13_15 %>% 
  filter(`Sample Year.y` == 2013) %>% 
  group_by(origin, SEX.y, `Sample Date.y`) %>% 
...
Almost all parentage assigments have a posterior probability of 1; one offspring is split between two pedigrees.
...
## Quick check for missed duplicates
It *does* look like one of the "extra" duplicate 2014 parents had 2 offspring assigned to it that we needed to remove. None of the "extra" duplicate 2016 offspring had any parent/offspring relationships.
...
# Are parents in dups?
parentage_14_16_filter %>% 
  filter(Parent_ID %in% c("PH14_01107", "PH14_01112"))

# Are offspring in dups
parentage_14_16_filter %>% 
  filter(Offspring %in% c("PH16_03922", "PH16_03923", "PH16_05684", "PH16_05691", "PH16_09721", "PH16_10544", "PH16_10553", "PH16_11811", "PH16_11812", "PH16_12040", "PH16_12041", "PH16_12448"))
```
## Join parentage and individual data
Here, we joined parentage data from *FRANz* with individual data so that we can match data to each offspring and parent. **Note** that any column ending in .x refers to offspring and .y refers to parent.  
...
O.even <- c("H", "N")
Spawners.even <- c(440, 214) #  number of hatchery- and natural-origin parents
Offspring.even <- c(269, 209)  # nuber of offspring assigned to hatchery- and natural-origin parents
...
# RS_combined_count %>% 
#   filter(n > 0 & lineage == "even" & SEX.y == "F") %>% 
...
# RS_combined_count %>% 
#   filter(n > 0 & lineage == "even" & SEX.y == "M") %>% 
#   ggplot(aes(x=n, y=(..count..)/sum(..count..), fill=origin)) +
#   geom_bar(position = position_dodge2(preserve = "single"))+
#    scale_y_continuous(breaks = pretty_breaks()) +
#   scale_x_continuous(breaks = pretty_breaks()) +
...
(rs_14_16_date <- parents_paired_14_16 %>%
  count(`Fish ID.y`, origin, SEX.y, `Sample Date.y`))

rs_14_16_date %>% 
  ggplot(aes(x=`Sample Date.y`, y=n, color=origin, shape=SEX.y))+
  geom_jitter(height = 0)+
...
lm.rs.date <- lm(n ~ `Sample Date.y`, rs_14_16_date)
summary(lm.rs.date)
```
...
(rs_14_16_length <- parents_paired_14_16 %>%
  count(`Fish ID.y`, origin, SEX.y, `Length Mm.y`))

rs_14_16_length %>% 
  ggplot(aes(x=`Length Mm.y`, y=n, color=origin, shape=SEX.y))+
...
rs_14_16_date_length <- left_join(rs_14_16_date, rs_14_16_length, by = "Fish ID.y")

rs_14_16_date_length %>% 
  ggplot(aes(x=`Sample Date.y`, y=`Length Mm.y`, color=origin.x, shape=SEX.y.x, size = n.x))+
  geom_jitter(height=0.5)+
...
rs_14_16_date_length %>% 
  ggplot(aes(x=`Sample Date.y`, y=`Length Mm.y`, color =SEX.y.x, size = n.x))+
  facet_grid(origin.x ~.) +
  geom_jitter(height=0.5)+
...
write.csv(paired_14_16_filter_parents, "paired_14_16_filter_parents.csv")
...
parents_paired_14_16_cross %>% 
  filter (SEX != "NA") %>% 
  ggplot(aes(x=`Length Mm.y`, y=`Length Mm`, colour = cross))+
  geom_point(size = 2) +
...
lm.date.length.even <- lm(`Length Mm.y` ~ `Sample Date.y`, parents_paired_14_16)
...
We performed an ANOVA to determine which variables influence RS for both even and odd lineages. 

```{r ANOVA of RS across lineages}
aov.rs <- aov(n ~ lineage * origin * SEX.y * `Length Mm.y` * `Sample Date.y`, RS_combined_count)
summary(aov.rs)
```

Origin, lineage x sex, lineage x sex x length, lineage x length x date, origin x length x date, and sex x length x date all have a significant effect on RS.
In the future, we will do a stepwise linear model using AIC and variable weighting to determine which variables impact RS.
...
aov.rs.2 <- aov(n ~ lineage + origin + SEX.y + `Length Mm.y` + `Sample Date.y`, RS_combined_count)
summary(aov.rs.2)
```

None of the variables significantly influence RS on their own. Note that origin and length are closest to being statistically significant (i.e likely *biologically significant*).
...
3. For the **odd-year** lineage, we saw that male and female natural origin fish had higher average numbers of offspring than hatchery fish, however, given the small sample size of families, we did not calculate *RRS*.  
4. For the **even-year** lineage, there is **significant difference** in *RRS* betwen female hatchery- and natural-origin parents (RRS  = `r round(RRS_f, 2)`, pval < 0.005 for both parametric and non-paramteric tests). However, there **is no significant difference** in *RRS* between male hatchery- and natural-origin parents (RRS  = `r round(RRS_m, 2)`, pval > 0.05 for both parametric and non-paramteric tests). These relationships hold when we also include non-parents in the analysis. 
...
8. We ran *FRANz* again for each dataset using escapement estimates from stream walks to inform our *Nmmax* and *Nfmax* parameters (*i.e. see how sensitive parentage assignments are to our estimate of parent year escapement*). Assignments were identical for the **odd** year lineage, while the re-analysis of the **even** year lineage added two additional parent assignments (458 in runs 2 and 3 instead of 456 in run 1).  These results indicate that our marker panel is very robust and that pedigree results are not highly sensitive to the *Nmmax* and *Nfmax* parameters. This is fantastic news given that our estimates of escapement are not very precise (**odd**: run1 - Nmmax = Nfmax = 23500 and typingerror = 0.005 vs. run 2 - Nmmax = Nfmax = 4300 and typingerror = 0.005 vs. run 3 - Nmmax = NfMax = 23500 and typingerror = 0.01; **even**: run1 - Nmmax = Nfmax = 4500  and typingerror = 0.005 vs. run 2 - Nmmax = Nfmax = 3300 and typingerror = 0.005 vs. run 3 - Nmmax = Nfmmax = 4500 and typingerror = 0.01).
