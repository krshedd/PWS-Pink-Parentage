...
There is a significant difference in the proportions of hatchery and natural fish sampled in the parental generation versus assigned as parents, indicating an under-representation of hatchery fish in the parent assignments. 
...
RS_combined_count %>% 
  filter(n > 0 & lineage == "odd") %>% 
  ggplot(aes(x=n, y=(..count..)/sum(..count..), fill=`Otolith Mark Present.y`)) +
...
paired_14_16_filter_parents %>% 
  ...
  xlab("Number of Offspring") +
  ylab("Number of Parents") +
  labs(title="Family Size for Males and Females")

paired_14_16_filter_parents %>% 
  ggplot(aes(x=n, y=(..count..)/sum(..count..), fill = `Otolith Mark Present`)) +
  geom_bar(position = position_dodge2(preserve="single")) +
  facet_grid(~SEX) +
   xlab("Number of Offspring") +
  ylab("Proportion of Parents") +
...
Orogin, lineage x sex, lineage x sex x length, lineage x length x date, origin x length x date, and sex x length x date all have a significant effect on RS. 
