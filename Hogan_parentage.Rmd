...
# Fitler for non-genotype columns
...
Also filter for only parentage assignments with posterior probabilities > 0.9. This will ensure that we dont' get offspring with two different single-parent assignments.
...
## Family size
Here, we calculate and plot family size for both natural and hatchery parents. **Note** that *FRANz* assigned 456 offspring to 186 parents in single parent assignments (*single parent-offspring duos*). 209 of these parents are natural origin and 269 are hatchery origin. Hatchery origin parents are from AFK12B, CCH12, and WNH12PINKB, none are from Solomon Gulch (VFDA).  

Additionally, *FRANz* made 2-parent assignments (*parent pair-offspring trios*) for 22 offspring. All potential cross types were present.  
...
For single parent-offspring assignments by sex, females produced 1-10 offspring and males produced 1-13. For parent 2 assignments, females produced 1-3 offspring, while males produced 1-2. **This is likely to be a publication figure**.  
...
Offspring length has a significant, albeit very small, linear relationship with parent 1 length (p=0.005), but there is not an effect of sex (p=0.85).  
...
Because run timing is highly heritable, we want to examine relationships between parent and offspring sample date. We also examine sample dates of both parents in two parent families to see if they make sense. 
...
Parent 1 and parent 2 sample dates closely align. All of the NN crosses occur early (before September 1). Crosses including hatchery fish occur throughout the run.  
