...
# Hi Emily, I think we want to calculate proportion differently. We want to show what proportion of females produced X number of offspring. As written, based on the .html, it was the proportion of all parents (not split by sex). We want the proportions to add up to 1 for each sex.

# Since we don't create `RS_combined_count` until later on in the notebook, I used what we had.
...
When we plot the sexes separately, we can see that most of the assigned parents are males. Female parentss produced 1-16 offspring, while male parents produced 1-4.  
...
This is a plot of reproductive success (family size) by sample data. **Note** the points are jittered, but only by sample date.
...
RRS <- tibble(Sex = c("M", "F", "M", "F"),
              Test = c(rep("Permuation", 2), rep("Negative Binomial GLM", 2)),
