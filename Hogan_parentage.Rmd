...
#Abstract
We are using genetic parentage analysis to estimate the relative reproductive success (RRS) of hatchery-origin pink salmon collected in Hogan Bay, a stream with moderate escapements in the tens of thousands and high observed stray rates. We genotyped 8,000 hatchery and natural origin fish collected between 2013 and 2016 at 304 markers. Standard quality control and quality assurance were performed, which included removing duplicate individuals, those who were successfully genotyped at <80% of markers, and those with abnormally high levels of heterozygosity. Population genetics analyses revealed that most of the variation was partitioned between lineages (odd versus even), but that there was little variation within lineages and between natural and hatchery origin individuals. *FRANz* produced parental assignment rates of 2.5% for the odd lineage and 11% for the even lineage. Both natural and hatchery parents were assigned to offspring in both lineages. There were too few parentage assignments in the odd lineage to be able to calculate RRS, but we were able to determine that natural origin females in the even lineage had significantly higher RRS than hatchery origin females and that there was not a significant difference between males. 
...
Hatchery fish have a tighter length distribution than natural origin fish, particularly in males.

```{r Calculate mean lengths of hatchery and natural origin fish}

length_h_f <- paired_13_15_filter %>% 
  filter(`Otolith Mark Present` == "YES" & `SEX`=="F" & `Length Mm` != "NA") %>%
  summarize(Mean=mean(`Length Mm`, na.rm=TRUE))
length_h_f

length_n_f <- paired_13_15_filter %>% 
  filter(`Otolith Mark Present` == "NO" & `SEX`=="F" & `Length Mm` != "NA") %>%
  summarize(Mean=mean(`Length Mm`, na.rm=TRUE))
length_n_f

length_h_m <- paired_13_15_filter %>% 
  filter(`Otolith Mark Present` == "YES" & `SEX`=="M" & `Length Mm` != "NA") %>%
  summarize(Mean=mean(`Length Mm`, na.rm=TRUE))
length_h_m

length_n_m <- paired_13_15_filter %>% 
  filter(`Otolith Mark Present` == "NO" & `SEX`=="M" & `Length Mm` != "NA") %>%
  summarize(Mean=mean(`Length Mm`, na.rm=TRUE))
length_n_m


```
Hatchery males and females are larger than natural males and females. Hatchery females are on average larger than hatchery males, but natural females are on average larger than natural males. 
...
From this histogram, we can see that natural origin fish tended to have earlier sampling dates than hatchery origin fish, which confirms what we know about differences in run timing. We can also see how sparse the field sampling was in 2013. 
...
## Join parentage and individual data
Here, we joined parentage data from *FRANz* with individual data so that we can match data to each offspring and parent. **Note** that any column ending in .x refers to offspring and .y refers to parent 1.  
...
269 parents were hatchery-origin, while 209 were natural-origin. 
...
Offspring from all four types of crosses are represented in our dataset. HH crosses produced 1-2 offspring, HN and NH crosses produced 1-3 offspring, and NN crosses produced 1-2 offspring. 
...
Male hatchery-origin fish have a tighter length distribution than natural-origin fish. 

```{r Calculate mean lengths of hatchery and natural origin fish Even}

length_h_f_e <- paired_13_15_filter %>% 
  filter(`Otolith Mark Present` == "YES" & `SEX`=="F" & `Length Mm` != "NA") %>%
  summarize(Mean=mean(`Length Mm`, na.rm=TRUE))
length_h_f_e

length_n_f_e <- paired_13_15_filter %>% 
  filter(`Otolith Mark Present` == "NO" & `SEX`=="F" & `Length Mm` != "NA") %>%
  summarize(Mean=mean(`Length Mm`, na.rm=TRUE))
length_n_f_e

length_h_m_e <- paired_13_15_filter %>% 
  filter(`Otolith Mark Present` == "YES" & `SEX`=="M" & `Length Mm` != "NA") %>%
  summarize(Mean=mean(`Length Mm`, na.rm=TRUE))
length_h_m_e

length_n_m_e <- paired_13_15_filter %>% 
  filter(`Otolith Mark Present` == "NO" & `SEX`=="M" & `Length Mm` != "NA") %>%
  summarize(Mean=mean(`Length Mm`, na.rm=TRUE))
length_n_m_e
```
As we saw in the odd lineage, hatchery origin males and females are larger than natural origin males and females. Hatchery females are larger than hatchery males and natural females are smaller than natural males. 
...
These are sample dates for all of the fish in the parental generation (2014). Natural fish tended to be sampled earlier in the run and hatchery fish were more common later in the run. However, there is a high degree of overlap that facilitates the ability for natural and hatchery fish to interbreed. 
...
4. Natural origin females have significantly higher RRS than hatchery origin females, but there was not a significant difference for males.  
5. Hatchery and natural fish interbreed.  
...
1. Run *FRANz* again for each dataset using escapement estimates from stream walks to inform our *Nmmax* and *Nfmax* parameters (*i.e. see how sensitive parentage assignments are to our estimate of parent year escapement*).  
  * **Note** Emily has already re-analyzed the **odd** year results with different *Nmmax* and *Nfmax* parameters and got the exact same parentage results. The results of the **even** year sensitivity analysis will be reported on Monday. The results from the **odd** year re-analysis indicate that our marker panel is very robust and that pedigree results are not sensitive to the *Nmmax* and *Nfmax* parameters. This is fantastic news given that our estimates of escapement are not very precise.
...
5. Apply the unbiased RRS calculation from Araki and Blouin (2005). 