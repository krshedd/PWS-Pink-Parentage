...
##Associations between parent and offspring body size

Because size is highly heritable, we compared length between parents and offspring.  
...
##Associations between length and origin in the parental generation
...
##Associations between parent and offspring sample date
...
##Comparison of sample date in hatchery and natural fish in the parental generation
...
We'll calculate relative reproductive success (RRS) using the following equation: mean hatchery family size/mean natural family size. We'll do this separately for males and females. 
...
Let's test for significant differences in RRS between male and female hatchery and natural origin fish. We'll perform a permutation test and fit a negative binomial GLM and then save the p values in a table. **Note** we are only doing this for the even lineage because the sample sizes are too low in the odd lineage. 
...
###Examination of 2-parent families
...
##Associations between parent and offspring body size
Because size is heritable, we would like to compare body length between parents and offspring.
...
This is a plot of the relationship between parent and offspring length. Otolith mark present refers to the parental generation and sex refers to the offspring.  
...
Offspring length has a significant linear relationship with parent 1 length (p=0.003), but there is not an effect of sex (p=0.85). Relationships between offspring length and parent 2 length were not significant (p=0.478), likely due to low sample sizes. 
...
##Comparison of sample date in hatchery and natural origin fish in the parental generation
...
1. Run Franz again for each dataset using escapement estimates from stream walks to inform our Nm and Nf parameters.  
