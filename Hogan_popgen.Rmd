...
```{r combine attributes table with PC scores}
Scores_Attributes <- pca1$li %>% 
  as_tibble(rownames="silly_source") %>% 
  left_join(HOGAN_attributes_tibble, by=c("silly_source"= "SillySource"))
```

```{r Exploratory Plots for PCs 1-2}
par(mfrow=c(1,3))
ggplot(Scores_Attributes, aes(x=Axis1, y=Axis2, color=Lineage)) + 
  geom_point(alpha=0.2) +
  labs(title="PC Scores By Lineage")
ggplot(Scores_Attributes, aes(x=Axis1, y=Axis2, shape=`Otolith Mark Present`, color=Lineage)) + 
  geom_point(alpha=0.2) +
  labs(title="PC Scores by Origin and Lineage")

```

```{r Plots of PCs 1-3}
ggplot(Scores_Attributes, aes(x=Axis1, y=Axis3, color=Lineage)) + 
  geom_point(alpha=0.2) +
  labs(title = "PC Scores by Lineage")
...
ggplot(Scores_Attributes, aes(x=Axis1, y=Axis3, shape=`Otolith Mark Present`, color=Sex)) + 
  geom_point(alpha=0.2) +
  labs(title = "PC Scores by Origin and Sex")

```

```{r Plots of PCs 2-3}
ggplot(Scores_Attributes, aes(x=Axis2, y=Axis3, color=Lineage)) + 
  geom_point(alpha=0.2) +
  labs(title = "PC Scores by Lineage")
...
Scores_Attributes %>% 
  ...
  labs(title="PC Scores by Hatchery")
...
This plot only contains hatchery fish. 
...
```{r Examine PC 2 scores by sex and origin}
ggplot(Scores_Attributes, aes(x=Axis1, y=Axis2, color=`Otolith Mark Present`, shape=Sex)) + 
  geom_point(alpha=0.2) +
  labs(title = "PC Scores by Origin and Sex")

ggplot(Scores_Attributes, aes(x=Axis2, y=Axis3, color=`Otolith Mark Present`, shape=Sex)) + 
  geom_point(alpha=0.2) +
  labs(title = "PC Scores by Origin and Sex")
```
...
Sex and year significantly partition variation in PC 3 scores. 

Let's examine the distributions of PC 3 scores by sex. 

```{r Boxplots of scores by sex}
par(mfrow=c(1,2))
...
```
The distributions of scores between the sexes don't appear to be different. 
...
```
Hatcheries are strongly differentiated in PC 1 scores. This is likely confounded by year effects.

Main take-aways:
1. The two lineages are strongly genetically differentiated. 
2. Fish from different hatcheries are differentiated in scores from PCs 1 and 3, most likely due to strong associations between hatchery and year. 
3. There may be some slight partitioning of variation by sex and origin. 

The remaining mystery seems to be explaining the outgroup in PC's 2 and 3. They appear to be natural-origin fish. Perhaps they are strays? 
