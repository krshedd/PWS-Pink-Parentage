---
title: "Paddy 2014/2016 Single-generation Parentage Analysis with 298 Markers"
...
We use the program *FRANz* [https://www.bioinf.uni-leipzig.de/Software/FRANz/About.html] to reconstruct parent-offspring relationships. Below are analyses of parentage for **Paddy** for the **even lineage** years **2014** (potential parents) and **2016** (potential offspring).

*FRANz* was begun on Fri May 28 2021 at 16:16 using the following parameters in the command line in the V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\**Paddy_2-14_2016** folder:
**FRANz --Nmmax 1700 --Nfmax 1700 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 "Paddy_14_16_postQA_2014_2016_PADDY.dat"**

The parameters are defined as follows:

* --Nmmax and --Nfmax are the maximum numbers of candidate mothers and fathers. To obtain our values, we used an estimated escapement of 3400 and divided by 2. This escapement number came from rounding up the pop est walk stream estimate of 3260; the stream walk survey AUC was 1221 and aerial survey AUC number was 1105. These estimates are listed under in the Excel file found here: "V:\Documents\5_Coastwide\Multispecies\AHRP\Field data\Sample Summary Extract and Genotype.xlsx".    
...
parentage_14_16 <- read_csv("../../Franz/Paddy_2014_2016/parentage.csv") 
paired_14_16 <- read_csv("../Franz/Paddy_14_16_postQA_OceanAK_paired_2014_2016_PADDY.csv") 
```
...
write_csv(paired_14_16_filter, "Paddy_paired_14_16_filter.csv")
```
...
sample_size_plot(indata = paired_14_16_filter, title = "Paddy 2014-2016")
```
# **Basic Statistics**
...
write_csv(parents_paired_14_16, "Paddy_parents_paired_14_16.csv")
...
parents_paired_14_16 %>% 
  ...
  spread(origin, n)
```
**Results:** Of the **3,079** natural-origin offspring genotyped in 2016, *FRANz* assigned **587**, for an assignment rate of **19.1%**, to **341** parents (**150** natural and **191** hatchery). Most assignments (**390**) were to one parent, but there were **197** two-parent assignments. A total of **340** offspring assignments were to a natural-origin parent and **30** were to a hatchery-origin parent, with **157** from AFK12B, **17** from CCH12, **3** from SGH12, and **267** from WNH12PINKB.
...
colnames(spawners_and_offspring_14_16) <- c("Paddy_spawners_14", "Paddy_offspring_16")
...
parents_paired_14_16 %>% 
  ...
  theme_bw()
```
**Results:** Female parents produced **0-15** offspring, while known male parents produced **0-14**.  
...
write_csv(paired_14_16_filter_parents, "Paddy_paired_14_16_filter_parents.csv")
...
RRS_0 %>%
  spread(Sex, p_value)
```
**Results:** There are significant differences in RRS for females and overall but not for males. 
...
colnames(rrs_14_16) <- c("Paddy_females_14_16", "Paddy_males_14_16")
...
paired_14_16_filter_parents %>% 
  ...
  scale_fill_manual(values = cbPalette2)
```
**Results:* No obvious trends. Natural origin males may be slightly more successful late-season.
...
rrs_Xtype_14_16 %>% 
  round(2)
```
**Results:** RRS for the comparisons were **`r round(rrs_Xtype_14_16[2,1], 2)` (95% CI [`r round(rrs_Xtype_14_16[1,1], 2)`-`r round(rrs_Xtype_14_16[3,1], 2)`])** for HH,**`r round(rrs_Xtype_14_16[2,1], 2)` (95% CI [`r round(rrs_Xtype_14_16[1,1], 2)`-`r round(rrs_Xtype_14_16[3,1], 2)`])** for HN and **`r round(rrs_Xtype_14_16[2,2], 2)` (95% CI [`r round(rrs_Xtype_14_16[1,2], 2)`-`r round(rrs_Xtype_14_16[3,2], 2)`])** for NH, indicating no significant differences in RS relative to the NN cross type. 
...
N/A for Paddy 2014/2016.

We find the parents that were assigned to grandparents and determine how many grandoffspring were assigned to them. We then estimate RRS according to grandparental origin. The set of panels below include:
```{r import_grandparent_data, echo=FALSE, warning=FALSE, message=FALSE}
# # Read in data
# parents_paired_15_14 <- read_csv("paddy_parents_paired_15_14.csv") %>% 
#   dplyr::select(-origin)
# 
# paired_14_16_filter_parents <- read_csv("Paddy_paired_14_16_filter_parents.csv")
# 
# parentage_paired_14_16 <- read_csv("Paddy_parents_paired_14_16.csv")%>% 
#   dplyr::select(-origin)
```
...
# 
# colnames(rrs_gpar_14_16) <- c("Paddy_grandparents_14_16")
...
# 
# write_csv(grandparentage, "Paddy_14_16_grandparentage.csv")
```
...
# Read in data
# parents_paired_13_15 <- read_csv("paddy_parents_paired_13_15.csv") %>% 
#   dplyr::select(-origin)
# 
# parents_paired_15_14 <- read_csv("paddy_parents_paired_15_14.csv") %>% 
#   dplyr::select(-origin)
# 
# parents_paired_14_16 <- read_csv("Paddy_parents_paired_14_16.csv") %>% 
#   dplyr::select(-origin)
# 
# paired_14_16_filter_parents <- read_csv("Paddy_paired_14_16_filter_parents.csv")
# 
# paired_15_14_filter_parents <- read_csv("paddy_paired_15_14_filter_parents.csv")
```
...
## Even Lineage Grandparentage

1. N/A

## Even Lineage Great Grandparentage
