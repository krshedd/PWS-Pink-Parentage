...
## *FRANz* parameters
Rundown of how the parentage results were generated using *FRANz* for even lineages in Stockdale (2016-2018) and Gilmour (2014-2016-2018) Creeks alone and Stockdale, Hogan, and Gilmour 2014-2016 considered together. Here, all three datasets were evaluated together as these initial steps are largely conserved and results are independent of study system. Subsequent sections include statistical analyses tailored to each study system.

***Stockdale***
*FRANz* was run Wed Oct 21 17:55 2020, with the following parameters:
...
**Gilmour**
*FRANz* was run Fri Oct 23 15:53 2020 for 2014-2016 and Mon Oct 26 13:45 2020 for 2016-2018, with the following parameters:
...
**Hogan/Stockdale/Gilmour**
*FRANz* was run at Fri Oct 23 16:29 2020, with the following parameters:
...
**Output**
All output files can be found in logically-named subfolders here: "v:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz"
...
Here, we plot histograms of *FRANz* parentage posterior probabilities to show robustness of assignments. Plots are for:
  1. Stockdale 2016-2018 and Gilmour 2014-2016-2018
  2. Stockdale, Hogan, and Gilmour 2014-2016
...
# Stacked bar plot of posterior probabilities for Stockdale 2016-2018 and Gilmour 2014-2016-2018
ggplot() +
  geom_histogram(data = stock_parentage_16_18, aes(x = Posterior, fill = "a"), breaks = seq(0, 1, 0.01)) +
  geom_histogram(data = gil_parentage_14_16, aes(x = Posterior, fill = "b"), breaks = seq(0, 1, 0.01)) +
  geom_histogram(data = gil_parentage_16_18, aes(x = Posterior, fill = "c"), breaks = seq(0, 1, 0.01)) +
  scale_colour_manual(name = "Stream and Years",
                      values=c("a" = "brown1", "b" = "cyan4", "c" = "darkgray"),
                      labels=c("a" = "Stockdale_16_18", "b" = "Gilmour_14_16", "c" = "Gilmour_16_18")) +
  scale_fill_manual(name = "Stream and Years",
                    values=c("a" = "brown1", "b" = "cyan4", "c" = "darkgray"),
                    labels=c("a" = "Stockdale_16_18", "b" = "Gilmour_14_16", "c" = "Gilmour_16_18")) +
  ggtitle("FRANz posterior probabilities for Stockdale 2016-2018 and Gilmour 2014-2016-2018 parentage")

# Bar plot for Stockdale, Hogan, and Gilmour 2014-2016
shg_parentage_14_16  %>%
  filter(!is.na(`Parent 1`)) %>%
  ggplot(aes(x = Posterior)) +
  geom_histogram(breaks = seq(0, 1, 0.01)) +
  ggtitle("FRANz posterior probabilities for Stockdale, Hogan, and Gilmour 2014-2016 parentage")
```
...
## Sample sizes
We calculate sample sizes for potential parents and potential offspring for each stream and year. Bar plots of sample sizes collected across the year are not included for the Stockdale-Hogan-Gilmour dataset.
...
We joined parentage data from *FRANz* with the individual data in order to match metadata to each offspring and parent. Note that columns ending in `.off` refer to offspring and `.par` refer to parent.  
...
# Nested function was turning into something longer than just repeating this code three times
...
# **Stockdale 2016-2018 Statistics and Analyses**
...
## Family size
We calculated and plotted family size for both natural and hatchery parents. 
...
**Results:** Female parents produced 1-7 offspring, while male parents produced 1-8, although this does not include potential parents not assigned offspring at this time (a.k.a. 0s).
...
**Results:** At this point, hatchery RS looks higher than natural RS, however...

### RS (including 0s)
We calculated RS again, including sampled fish from the parental generation to which offspring were not assigned, and plotted a historgram of the proportion of parents with a given family size, including lines showing mean reproductive success.
...
We calculated relative reproductive success (RRS) for females and males and tested for significant differences in RRS between male and female hatchery and natural origin fish. To do this, we performed two different statistical tests:  
...
#### RS by sample date
```{r Examine reproductive success by sample date with 0s stockdale, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.keep='all'}
...
**Results:** Natural-origin females may have marginally higher RS late in the season.

#### RS by parental body length
```{r Examine reproductive success by parent length with 0s stockdale, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.keep='all'}
stock_paired_16_18_filt_parents %>% 
...
**Results:** There appears to be no trend in RS associated with body length.

## Parent pair-offspring trios (triads)
Because we have **67** individuals who were assigned to two parents, we examined instances of crosses between hatchery fish, between natural fish, and between natural and hatchery fish. We divided natural-hatchery crosses into "HN", indicating a hatchery-origin mother and natural-origin father, and "NH", indicating a natural-origin mother and hatchery-origin father. 
...
**Results:** Offspring from two types of crosses (NN and NH) are represented in our dataset. Only one family was from a natural females and hatchery male, while the remainder were crosses among natural fish. 

### Cross type RS plus 95% CIs
...
**Results:** No significant difference in RS by cross type.

### Mating dynamics
We set out to discover who is mating with whom and when.
...
**Results:** Multiple matings clearly occur.

#### Mate pairings using triad data
With the information thus far, it is difficult to understand whether males and/or females have multiple partners. We thus assessed promiscuity among males and females and with whom they are mating. We also note that the question can be further conceptualized, if warranted, using a pedigree visualization platform (e.g. Helium).
...
# DNE for the most part - did not run
# # How many dams had multiple sires of differing origin
# stock_dams_with_mult_sire <- stock_parentage_paired_16_18_cross_dam_sire %>% 
#   group_by(Dam) %>% 
...
# 
# stock_parentage_paired_16_18_cross_dam_sire %>% 
#   filter(Dam %in% stock_dams_with_mult_sire) %>% 
...
# stock_sires_with_mult_dam <- stock_parentage_paired_16_18_cross_dam_sire %>% 
#   group_by(Sire) %>% 
#   summarise(n_cross = n_distinct(cross)) %>% 
#   filter(n_cross > 1) %>% 
#   pull(Sire)
# 
# stock_parentage_paired_16_18_cross_dam_sire %>% 
#   filter(Sire %in% stock_sires_with_mult_dam) %>% 
#   group_by(Dam, Sire, cross) %>% 
#   summarise(n = n_distinct(Offspring)) %>% 
#   spread(cross, n)
# 
...
**Results:** There were **56** unique matings, and both sexes mated with multiple partners, although males appeared marginally more promiscuous than females. 

## Grandparentage

### RRS plus 95% CI
We found the F1 fish that had F0 parents assigned and F2 offspring that returned in 2018, and then we estimated RRS according to grandparental origin.
```{r stock_gparents_rrs, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.keep='all'}
stock_parents_paired_14_16 <- read_csv("stock_parents_paired_14_16.csv") %>% 
  dplyr::select(-origin)
...
# Summary of above: origin of F0 grandparents (origin.par) and numbers of their F1 offspring (n_par) and F2 grandoffspring (n_off)
...
**Results:** Hatchery-origin grandparents contributed to **373** F1 parents that went on to contribute to **30** F2 offspring, while natural-origin grandparents contributed to **865** F1 parents that went on the contribute to **74** F2 offspring. RRS was **0.94 (95% CI [0.61, 1.42])**.

### Multigeneration dam, sire, and origin data
...
**Results:** A total of **76** F2 offspring had one grandparent assigned, and **14** had two grandparents assigned. Unfortunately, no F2 offspring had >2 grandparents assigned.
...
# **Gilmour 2014-2016-2018 Statistics and Analyses**
Much of the following has to be taken with a grain of salt as otolith reads are not available for the majority of the 2016 and 2018 samples.
...
## Family size
We calculated and plotted family size for both natural and hatchery parents from 2014 and 2016. 
...
**Results:** In 2014, dams produced **1-18** offspring, while sires produced **1-21**. In 2016, dams produced **1-14** offspring, and sires produced **1-11**. These measures do not include potential parents not assigned offspring at this time (a.k.a. 0s). Also, as a reminder, many potential offspring from both 2016 and 2018 do not yet have otolith reads.

## Single parent-offspring pairs (dyads)

### RS (excluding 0s)
We calculated RS of natural and hatchery females and males known to produce offspring. 
...
**Results:** At this point, RS is >=1 for all parental years and types, however...
...
#### Test for significant differences in RRS
We calculated RRS for females and males and tested for significant differences in RRS between male and female hatchery and natural origin fish. To do this, we performed two different statistical tests:  
  1. Nonparametric permutation test
  2. Parametric negative binomial GLM
...
**Results:** In 2014, RRS for males was **`r round(gil_RRS_m_0_14_16, 2)`** and RRS for females **`r round(gil_RRS_f_0_14_16, 2)`**. There is a borderline significant difference with permutation testing in overall RRS. In 2016, RRS for males was **`r round(gil_RRS_m_0_16_18, 2)`** and RRS for females **`r round(gil_RRS_f_0_16_18, 2)`**. There were no significant differences. 

#### RRS plus 95% CIs for females and males
We calculated 95% CIs for RRS based on Kalinowski and Taper (2005).
...
### RS by field data
We explored the potential for an association between RS and parental sample date and/or body length.
...
**Results:** For 2014-2016 only, females may have had higher RS later in the season. The 2016-2018 results are inconclusive at this time.

#### RS by parental body length
```{r Examine reproductive success by parent length with 0s gilmour, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.keep='all'}
# 2014-2016
gil_paired_14_16_filt_parents %>% 
  filter(`Length Mm` > 300) %>% 
  ggplot(aes(x = `Length Mm`, y = n, color = origin)) +
...
**Results:** For 2014-2016 only, larger fish (especially natural-origin males) may have been more successful. The 2016-2018 results are inconclusive at this time.
...
**Results:** For the 2014-2016 data, offspring resulting from **all four cross types (NN, NH, HN, and HH)** represented in our dataset. For 2016-2018, although a few NN crosses are apparent, the distribution of cross types overall is inconclusive at this time.
...
**Results:** Multiple matings again clearly occur.

#### Mate pairings using triad data
We assessed promiscuity among males and females and with whom they are mating.
...
# How many dams had multiple sires of differing origin
gil_dams_with_mult_sire_14_16 <- gil_parentage_paired_14_16_cross_dam_sire %>%
  group_by(Dam) %>%
  summarise(n_cross = n_distinct(cross)) %>%
  filter(n_cross > 1) %>%
  pull(Dam)
...
gil_parentage_paired_14_16_cross_dam_sire %>%
  filter(Dam %in% gil_dams_with_mult_sire_14_16) %>%
  group_by(Dam, Sire, cross) %>%
  summarise(n = n_distinct(Offspring)) %>%
  spread(cross, n)
...
# How many sires had multiple dams of differing origin
gil_sires_with_mult_dam_14_16 <- gil_parentage_paired_14_16_cross_dam_sire %>%
  group_by(Sire) %>%
  summarise(n_cross = n_distinct(cross)) %>%
  filter(n_cross > 1) %>%
  pull(Sire)
...
gil_parentage_paired_14_16_cross_dam_sire %>%
  filter(Sire %in% gil_sires_with_mult_dam_14_16) %>%
  group_by(Dam, Sire, cross) %>%
  summarise(n = n_distinct(Offspring)) %>%
  spread(cross, n)
...
# DNE at this time - did not run
# # How many dams had multiple sires of differing origin
# gil_dams_with_mult_sire_16_18 <- gil_parentage_paired_16_18_cross_dam_sire %>%
#   group_by(Dam) %>%
#   summarise(n_cross = n_distinct(cross)) %>%
#   filter(n_cross > 1) %>%
#   pull(Dam)
# 
# gil_parentage_paired_16_18_cross_dam_sire %>%
#   filter(Dam %in% gil_dams_with_mult_sire_16_18) %>%
#   group_by(Dam, Sire, cross) %>%
#   summarise(n = n_distinct(Offspring)) %>%
#   spread(cross, n)
# 
...
# 
# # How many sires had multiple dams of differing origin
# gil_sires_with_mult_dam_16_18 <- gil_parentage_paired_16_18_cross_dam_sire %>%
#   group_by(Sire) %>%
#   summarise(n_cross = n_distinct(cross)) %>%
#   filter(n_cross > 1) %>%
#   pull(Sire)
# 
# gil_parentage_paired_16_18_cross_dam_sire %>%
#   filter(Sire %in% gil_sires_with_mult_dam_16_18) %>%
#   group_by(Dam, Sire, cross) %>%
#   summarise(n = n_distinct(Offspring)) %>%
#   spread(cross, n)
# 
...
**Results:** There were **129** unique matings represented in 2014-2016 and **106** in 2016-2018. Both sexes mated with multiple partners and seemed equally promicuous, mating with **1-4** partners. 
...
# **Hogan-Stockdale-Gilmour 2014-2016**
```{r shg_14_16}
# How many parent offspring pair assignments?
...
**Results:** The vast majority of offspring assigned parents returned to their natal stream (**>94%**). However, it does appear that **5.8%** of Stockdale-origin offspring did stray into Hogan (**2.1%**) and Gilmour (**3.7**), whereas Hogan-origin fish did not stray much at all(**<1%**). Gilmour-origin fish strayed differentially into Stockdale (**4.2%**), which is a neighboring stream, versus Hogan (**<1%**).
...
# **Grand Summary of RRS Results to Date**
...
![an image caption Multigeneration RRS Results to Date](RRS_Grandparents.png)