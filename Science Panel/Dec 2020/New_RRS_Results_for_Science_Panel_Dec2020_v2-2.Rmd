---
title: "RRS Results to Date for Science Panel v2.2"
...
colnames(stock_n_dams_and_sires1618) <- c("Num_mates",  "Num_dams_16_18", "Num_sires_16_18")
...
(stock_gparents_origin <- stock_paired_16_18_filt_parents %>% 
  ...
  rename(Parental_origin = origin.par))
...
### Mating dynamics
We set out to discover who is mating with whom and when. Multiple matings clearly occur, but it can be difficult to understand whether it's males and/or females that have multiple partners. We thus set out to assess promiscuity within the sexes, first determining whether the cross data made sense in light of the sample date for each parent. The relationship between dam and sire sample date was generally linear, indicating the cross data are likely correct, so we next calculated the number of mates for each known female and male parent (dams and sires) from each year.
...
colnames(gil_n_dams_and_sires1416) <- c("Num_mates", "Num_dams_14_16", "Num_sires_14_16")
...
colnames(gil_n_dams_and_sires1618) <- c("Num_mates",  "Num_dams_16_18", "Num_sires_16_18")

(gil_n_dams_and_sires1418 <- merge(gil_n_dams_and_sires1416, gil_n_dams_and_sires1618, by = "Num_mates")) # merge 2014-2018
```
...
Here, we partition the parent and offspring data from 2014 2016 among three different streams, including Hogan, stockdale, and Gilmour, to assess whether staying may have an impact on our ability to assign parentage as best possible. Panels below include 1) the number of offspring detected in each stream from a particular source population (stream and origin) and 2) the number and location of offspring per a particular parent.
