...
# Background
## Quality Assurance
Initial data manipulation and filtering of all project gentoypes included:
...
## Parentage
Here, we provide a rundown of how the parentage results were generated using *FRANz* for even lineages in Stockdale (2016-2018) and Gilmour (2014-2016-2018) Creeks alone and Stockdale, Hogan, and Gilmour 2014-2016 combined. All three datasets were evaluated together as these initial steps are largely conserved and results are independent of study system. Subsequent sections include statistical analyses tailored to each study system.
...
**Result:** Almost all parentage assigments have a posterior probability of 1, which is very robust; few offspring assignments for any dataset were split among multiple potential parents. We can therefore be confident in the power of our 298 amplicons to make parent assignments.

## Sample sizes
After pulling out the columns from the OceanAK metadata that contain identifying information for each individual, as well as filtering the parentage data for posterior probabilities > 0.9, we calculated sample sizes for potential parents and potential offspring for each stream and year. Tables below are for 1) Stockdale 2016-2018, 2) Gilmour 2014-2016, 3) Gilmour 2016-2018, and 4) Stockdale-Hogan-Gilmour 2014-2016, respectively.
...
sample_size_plot <- function(indata = stock_paired_16_18_filt, title = "Stockdale 2016-2018") {
  ...
  mutate(date = dmy(`Sample Date`)) %>%
  mutate(DOY = yday(date)) %>%
  mutate(DOM = format(as.Date(date), "%b-%d")) %>%
  ggplot(aes(x = DOM)) +
  theme_bw() +
...
stock_paired_16_18_filt_parents %>%
 ...
 geom_vline(xintercept=stock_fem_natur_16_18, color=gg_color_hue(2)[1], linetype = "dashed", size = 2) +
 geom_vline(xintercept=stock_fem_hatch_16_18, size=2) +
 geom_vline(xintercept=stock_fem_hatch_16_18, color=gg_color_hue(2)[2], linetype = "dashed", size = 2) +
 annotate("text", x = stock_fem_natur_16_18 + 0.5, y = .250, label = paste("fem_natur_RS =", round(stock_fem_natur_16_18, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
 annotate("text", x = stock_fem_hatch_16_18 + 0.5, y = .300, label = paste("fem_hatch_RS =", round(stock_fem_hatch_16_18, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2])
...
stock_paired_16_18_filt_parents %>%
 ...
 geom_vline(xintercept=stock_male_natur_16_18, color=gg_color_hue(2)[1], linetype = "dashed", size = 2) +
 geom_vline(xintercept=stock_male_hatch_16_18, size=2) +
 geom_vline(xintercept=stock_male_hatch_16_18, color=gg_color_hue(2)[2], linetype = "dashed", size = 2) +
 annotate("text", x = stock_male_natur_16_18 + 0.5, y = .250, label = paste("male_natur_RS =", round(stock_male_natur_16_18, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
 annotate("text", x = stock_male_hatch_16_18 + 0.5, y = .300, label = paste("male_hatch_RS =", round(stock_male_hatch_16_18, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2])
```
...
#   # ylim(0, 40) +
#   # xlim(0, 5) +
#   labs(title = "Distribution of Family Size by Cross Type") +
#   xlab("Number of Offspring")+
...
stock_parentage_paired_16_18_cross %>% 
  ...
  theme_bw() +
  theme(text = element_text(size = 15), legend.position = c(0.7, 0.75))
```
...
Below, we report the RS of each cross type and RRS (95% CI).
...
stock_n_sires1618 <- as.data.frame(stock_n_sires1618)
stock_n_dams1618 <- as.data.frame(stock_n_dams1618)
stock_n_dams_and_sires1618 <- merge(stock_n_sires1618, stock_n_dams1618, by = 0, all = TRUE)[-1] # merge 2016-2018
stock_n_dams_and_sires1618 <- stock_n_dams_and_sires1618[, c(3, 2, 4)]
colnames(stock_n_dams_and_sires1618) <- c("n_mates",  "n_dams_16_18", "n_sires_16_18")
stock_n_dams_and_sires1618
```
...
## Grandparentage
We found the F1 fish that had F0 parents assigned as well as F2 offspring that returned in 2018. We then estimated RRS according to grandparental origin. The set of panels below include 1) how many grandparents of each origin contributed to a particular number of offspring that in turn contributed to a particular number of grandoffspring and 2) how many of those grandoffspring had a particular number of grandparents assigned to them.
...
**Results:** Each assigned grandparent contributed to **1-5** grandoffspring. A total of **167** unique hatchery-origin grandparents were assigned to **373** F1 parents, **20** of which assigned to **30** F2 offspring, and **238** unique natural-origin grandparents assigned to **865** F1 parents, **51** of which assigned to **74** F2 offspring. RRS between hatchery- and natural-origin grandparents was **`r round(rrs_sgpar1618[2,1], 2)` (95% CI [`r round(rrs_sgpar1618[1,1], 2)`-`r round(rrs_sgpar1618[3,1], 2)`])**. Finally, a total of **76** F2 offspring had one grandparent assigned, while **14** had two grandparents assigned. Unfortunately, no F2 offspring had >2 grandparents assigned.
...
## Basic statistics
We calculated the number of single-parent offspring (dyad) and two-parent offspring (triad) assignments, 2) console output for the number of unique parents assigned offspring, the number and origin of unique parents assigned offspring, and origin broken down further according to otolith marking.
...
#   spread(SEX.par, RS)
# 
# gil_fem_h_14_16 <- gil_rs_14_16 %>% 
#   filter(origin == "Hatchery" & SEX.par == "F") %>% 
#   pull(RS)
# gil_male_h_14_16 <- gil_rs_14_16 %>% 
#   filter(origin == "Hatchery" & SEX.par == "M") %>% 
#   pull(RS)
# gil_fem_n_14_16 <- gil_rs_14_16 %>% 
#   filter(origin == "Natural" & SEX.par == "F") %>% 
#   pull(RS)
# gil_male_n_14_16 <- gil_rs_14_16 %>% 
...
#   spread(SEX.par, RS)
# 
# gil_fem_h_16_18 <- gil_rs_16_18 %>% 
#   filter(origin == "Hatchery" & SEX.par == "F") %>% 
#   pull(RS)
# gil_male_h_16_18 <- gil_rs_16_18 %>% 
#   filter(origin == "Hatchery" & SEX.par == "M") %>% 
#   pull(RS)
# gil_fem_n_16_18 <- gil_rs_16_18 %>% 
#   filter(origin == "Natural" & SEX.par == "F") %>% 
#   pull(RS)
# gil_male_n_16_18 <- gil_rs_16_18 %>% 
#   filter(origin == "Natural" & SEX.par == "M") %>% 
#   pull(RS)

# We calculated RS again, including sampled fish from the parental generation to which offspring were not assigned
# 2014-2016
# Transform to one row per parent with n = number of offspring produced
...
gil_paired_14_16_filt_parents %>% 
  ...
  geom_vline(xintercept=gil_fem_natur_14_16, color=gg_color_hue(2)[1], linetype = "dashed", size=2) +
  geom_vline(xintercept=gil_fem_hatch_14_16, size=2) +
  geom_vline(xintercept=gil_fem_hatch_14_16, color=gg_color_hue(2)[2], linetype = "dashed", size=2) +
  annotate("text", x = gil_fem_natur_14_16 + 0.5, y = .250, label = paste("fem_natur_RS =", round(gil_fem_natur_14_16, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
  annotate("text", x = gil_fem_hatch_14_16 + 0.5, y = .300, label = paste("fem_hatch_RS =", round(gil_fem_hatch_14_16, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2]) +
  theme_bw() 
...
gil_paired_14_16_filt_parents %>% 
  ...
  geom_vline(xintercept=gil_male_natur_14_16, color=gg_color_hue(2)[1], linetype = "dashed", size=2) +
  geom_vline(xintercept=gil_male_hatch_14_16, size=2) +
  geom_vline(xintercept=gil_male_hatch_14_16, color=gg_color_hue(2)[2], linetype = "dashed", size=2) +
  annotate("text", x = gil_male_natur_14_16 + 0.5, y = .250, label = paste("male_natur_RS =", round(gil_male_natur_14_16, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
  annotate("text", x = gil_male_hatch_14_16 + 0.5, y = .300, label = paste("male_hatch_RS =", round(gil_male_hatch_14_16, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2]) +
  theme_bw() 
...
gil_paired_16_18_filt_parents %>% 
  ...
  geom_vline(xintercept=gil_fem_natur_16_18, color=gg_color_hue(2)[1], linetype = "dashed", size=2) +
  geom_vline(xintercept=gil_fem_hatch_16_18, size=2) +
  geom_vline(xintercept=gil_fem_hatch_16_18, color=gg_color_hue(2)[2], linetype = "dashed", size=2) +
  annotate("text", x = gil_fem_natur_16_18 + 0.5, y = .250, label = paste("fem_natur_RS =", round(gil_fem_natur_16_18, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
  annotate("text", x = gil_fem_hatch_16_18 + 0.5, y = .300, label = paste("fem_hatch_RS =", round(gil_fem_hatch_16_18, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2]) +
  theme_bw() 
...
gil_paired_16_18_filt_parents %>% 
  ...
  geom_vline(xintercept=gil_male_natur_16_18, color=gg_color_hue(2)[1], linetype = "dashed", size=2) +
  geom_vline(xintercept=gil_male_hatch_16_18, size=2) +
  geom_vline(xintercept=gil_male_hatch_16_18, color=gg_color_hue(2)[2], linetype = "dashed", size=2) +
  annotate("text", x = gil_male_natur_16_18 + 0.5, y = .250, label = paste("male_natur_RS =", round(gil_male_natur_16_18, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
  annotate("text", x = gil_male_hatch_16_18 + 0.5, y = .300, label = paste("male_hatch_RS =", round(gil_male_hatch_16_18, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2]) +
  theme_bw() 
```
...
### RRS
#### *Test for significant differences in RRS*
First, we calculated RRS for females and males. Next, we tested for significant differences in RRS between male and female hatchery and natural origin fish. To do this, we performed two different statistical tests on the RRS estimates in the first and third panels, including 1) a non-parametric permutation test and 2) a parametric negative binomial GLM. P-values are presented for each test. Results for 2014-2016 are presented first and 2016-2018 follows. 
...
#### *RRS plus 95% CIs for females and males*
We calculated 95% CIs for RRS based on Kalinowski and Taper (2005) for 1) 2014-2016 females, 2) 2014-2016 males, 3) 2016-2018 females, and 4) 2016-2018 males.
...
gil_parentage_paired_16_18_cross %>% 
  ...
  mutate(cross_n = paste0(cross, " (n = ", samp, ")")) %>% 
  ggplot(aes(x = n, y = p, fill = cross_n)) +
  scale_fill_manual(values=c("#8c8c8c", "#F8766D")) +
  geom_col(position = position_dodge2(preserve = "single"), width = 0.5) +
  scale_y_continuous(limits = c(0,1), breaks = seq(0, 1, by = 0.1)) +
...
The panels below include 1) the RS of each cross type and 2) the RRS with 95% CIs for NN vs. HH, HN, and NH, respectively.
...
### Mating dynamics
We set out to discover who is mating with whom and when. Multiple matings clearly occur, but it can be difficult to understand whether it's  males and/or females that have multiple partners. We thus set out to assess promiscuity within the sexes, first determining whether the cross data made sense in light of the sample date for each parent. Because the relationship between dam and sire sample date was generally linear, indicating the cross data are likely correct, we next calculated the number of mates for each known female and male parent, respectively.
```{r multiple_matings_gil, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, results='hide', fig.keep='all'}
# How many mates as determined in *FRANz* for each Parent 1 and Parent 2, respectively, independent of identity, sex, or origin. 
# # 2014-2016
# # Parent 1 multiple matings
# gil_parentage_paired_14_16_cross %>% 
...
gil_n_sires1416 <- as.data.frame(gil_n_sires1416)
gil_n_dams1416 <- as.data.frame(gil_n_dams1416)
gil_n_dams_and_sires1416 <- merge(gil_n_sires1416, gil_n_dams1416, by = 0, all = TRUE)[-1] # merge 2014-2016
gil_n_dams_and_sires1416 <- gil_n_dams_and_sires1416[, c(3, 2, 4)]
colnames(gil_n_dams_and_sires1416) <- c("n_mates", "n_dams_14_16", "n_sires_14_16")
...
gil_n_sires1618 <- as.data.frame(gil_n_sires1618)
gil_n_dams1618 <- as.data.frame(gil_n_dams1618)
gil_n_dams_and_sires1618 <- merge(gil_n_sires1618, gil_n_dams1618, by = 0, all = TRUE)[-1] # merge 2016-2018
gil_n_dams_and_sires1618 <- gil_n_dams_and_sires1618[, c(3, 2, 4)]
colnames(gil_n_dams_and_sires1618) <- c("n_mates",  "n_dams_16_18", "n_sires_16_18")

(gil_n_dams_and_sires1418 <- merge(gil_n_dams_and_sires1416, gil_n_dams_and_sires1618, by = "n_mates")) # merge 2014-2018
```

**Results:** There were **129** unique matings represented in 2014-2016 and **106** in 2016-2018. Both sexes mated with multiple partners and seemed equally promicuous, each successfully producing offspring primarily with **1** and up to **4** partners, depending on year and sex.
...
**Results:** Each assigned grandparent contributed to **1-13** grandoffspring. A total of **160** unique hatchery-origin grandparents were assigned to **645** F1 parents, **85** of which assigned to **176** F2 offspring, while **149** unique natural-origin grandparents assigned to **604** F1 parents, **92** of which went on to contribute to **197** F2 offspring. RRS between hatchery- and natural-origin grandparents was **`r round(rrs_ggpar1618[2,1], 2)` (95% CI [`r round(rrs_ggpar1618[1,1], 2)`-`r round(rrs_ggpar1618[3,1], 2)`])**. Finally, **175** F2 offspring had one, **84** had two, and **10** had three grandparents assigned, but no F2 offspring had all four grandparents assigned.
...
## Multigeneration RRS
![an image caption Multigeneration RRS Results to Date](RRS_Grandparents.png){#id .class width=40% height=40%}