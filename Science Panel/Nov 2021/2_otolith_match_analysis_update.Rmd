...
In general, most loci had fairly decent genotyping success for the otolith-DNA. Certainly lower than for heart tissue, but not terrible. No loci completely failed for the otoltihs.
...
A fairly wide distribution of otolith genotyping success rate. Not terribly surprising though given that we are trying to pull DNA off of otoliths. Normally, we exclude individuals with genotyping success <80%, however, we will need to be much less stringent with the otoliths if we want to retain as many as possible.
...
Excess heterozygosity can be an indication of contamination, and observed heterozygosity (H)o was clearly high for some otoliths. We know there is some contamination even among the heart-derived GT-seq genotypes, but it allowed us to understand the extent of the otolith contamination.
...
For the heart tissues, we kept out standard genotyping rate cutoff of 80% and performed the standard 95% duplicate rate check.
...
In order to do the re-pairing, we need both heart and otolith genotypes from the same DWPs. Unfortunately, we do not yet have heart genotypes for all of the collections represented by the DWPs involved in this initial live test of otoliths that were moved into the 12 96-well otolith trays genotyped here. For this dataset, PGOD21 was represented by 12 96-well otolith plates, which covered 48 DWPs. However, a total of 19 PGOD21 DWPs did not have heart genotype DWP matches, because they have not been genotyped yet. This left 29 DWPs with both heart and otolith genotypes for matching.
...
Ideally, each otolith would clearly and unambiguously match a single heart and vice versa. However, since that is not always the case, and we are not going to go through each individual otolith at time, we came up with two metrics to help us determine how good of a match we have.  

  1) Duplicate rate - how many loci were genotyped in common between the heart and otolith  ?
  2) Duplicate rate difference - how close is the duplicate rate between the 1st and 2nd best match?
  
The idea here is that correctly matching hearts/otoliths would have both a high duplicate rate and a large difference between the 1st and 2nd best matches.
...
Deciding which otolith-heart matches we were confident of required careful cutoffs for duplicate rate and duplicate rate difference. For this DWP, we settled on a duplicate_rate \>= 0.7 and diff \>= 0.15. These are those matches within that upper right quadrant.
