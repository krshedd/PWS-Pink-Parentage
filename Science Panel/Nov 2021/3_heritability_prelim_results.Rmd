---
title: "Preliminary narrow-sense heritability estimation: Stockdale 2014-2016"
...
The purpose of these analyses are to more fully explore the parent-pair offspring trio/triad data (i.e. cross data) in terms of spatiotemporal patterning and generate preliminary estimates of narrow-sense heritability (h^2^) using three methods, including correlation, slope, and the animal model. We used Stockdale Creek 2014-2016, a sample set with good dual parent-offspring data, as a case study.
...
stock_riverdist_14_16 <- read_csv("../../../GIS/R/stockdale/stockdale_distances.csv", show_col_types = FALSE) %>% 
  mutate(Intertidal = case_when(dist2tide > 0 ~ "Upstream",
                                dist2tide <= 0 ~ "Intertidal")) %>% 
  dplyr::rename(Distance = mouthdist, Segment = seg)

stock_parents_paired_14_16_cross <- read_csv("../../Stockdale/stock_parents_paired_14_16_cross.csv", show_col_types = FALSE)
...
parents_paired_cross_riverdist_spacetime <- function(indata_spacetime = stock_parents_paired_14_16_cross, indata_riverdist = stock_riverdist_14_16) {
  ...
  dplyr::rename(`Fish ID.par2` = `Fish ID`, 
  ...
  mutate(length.dam = case_when(SEX.par1 == "F" ~ `Length Mm.par1`,
                                  TRUE ~ `Length Mm.par2`)) %>%
  dplyr::select(cross, n, ends_with(".sire"), ends_with(".dam"))
}

(stock_parents_paired_14_16_cross_riverdist_spacetime <- parents_paired_cross_riverdist_spacetime())
```

## Offspring with assigned parents - dyads and triads

One row per offspring that was assigned at least one (1) parent.

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
# dyad relationships - one row per offspring
stock_parents_paired_14_16 <- read_csv("../../Stockdale/stock_parents_paired_14_16.csv", show_col_types = FALSE) %>% 
     mutate(origin.par = factor(x = origin.par, levels = c("Natural", "Hatchery")))

# Read in paired data
stock_postQA_OceanAK_paired_14_16 <- read_csv("../../Franz/stockdale_postQA_OceanAK_paired_2014_2016_STOCK.csv", show_col_types = FALSE)

OceanAK_paired <- function(indata_OceanAK_paired = stock_postQA_OceanAK_paired_14_16) {
   indata_OceanAK_paired %>% 
   dplyr::select(franz_id, starts_with("Sample"), SILLY, `Fish ID`, starts_with("DNA"), SEX, `Length Mm`, `Otolith Mark Present`, `Otolith Mark ID`) %>% 
...
ods_relationships <- function(indata_ods = stock_parents_paired_14_16) {
  offspring_sire <- indata_ods %>% 
    filter(SEX.par == "M") %>% 
    dplyr::select(Offspring, Parent_ID)
...
One row per offspring that was assigned both parents.
...
For traits associated with fitness in natural populations, heritability is typically 0.1--0.2 (Wray and Visscher 2008 [<https://www.nature.com/scitable/topicpage/estimating-trait-heritability-46889/>] per Visscher et al. 2008). Here, we estimated h^2^ of body length, sample location, and RS. Correlation between parent-offspring sample location and h^2^ of sample location *not yet available*; GIS-based stream distance mapping is *in progress* by Chase Jalbert. Again estimates of h^2^ were generated using the correlation and slope of the regression line. For the animal model, h^2^ of body length and sample location were estimated from the residuals of the random effects using the linear mixed effects lmer function in the r package lme4 [<https://cran.r-project.org/web/packages/lme4/lme4.pdf>], whereas we used the generalized linear mixed effects glmer.nb function for negative binomials in lme4 to estimate h^2^ of RS.
...
parents_paired_length_graph <- function(indata_parents_paired_length_graph = stock_parents_paired_14_16, title = "Stockdale_14_16\n\nRegression of Body Length by Sex") {
  ...
  dplyr::rename(length.par = "Length Mm.par", 
...
We estimate h^2^ here, as well as for location and sample date, from r = √(½)h\^2.
...
stock_h2_length_14_16_dam_sire <- lmer(Length.off ~ Length.par + (1|Length.sire)+(1|Length.dam), 
                                        data = stock_triads_length_14_16, 
                                        na.action = na.omit)
(summary(stock_h2_length_14_16_dam_sire))
stock_h2_length_14_16_all <- (0.00 + 13.35) / (0.00 + 13.35 + 482.61)
...
stock_h2_length_14_16_dam_sire_NN <- lmer(Length.off ~ Length.par + (1|Length.sire)+(1|Length.dam), 
...
stock_h2_length_14_16_dam_sire_NH <- lmer(Length.off ~ Length.par + (1|Length.sire)+(1|Length.dam), 
...
stock_h2_length_14_16_dam_sire_HN <- lmer(Length.off ~ Length.par + (1|Length.sire)+(1|Length.dam), 
...
stock_h2_length_14_16_dam_sire_HH <- lmer(Length.off ~ Length.par + (1|Length.sire)+(1|Length.dam), 
...
stock_h2_length_14_16_dam_sire_F <- lmer(Length.off ~ Length.par + (1|Length.sire)+(1|Length.dam), 
...
stock_h2_length_14_16_dam_sire_M <- lmer(Length.off ~ Length.par + (1|Length.sire)+(1|Length.dam), 
...
stock_h2_length_14_16 <- merge(stock_r_and_h2_length_14_16, stock_slope_as_h2_length_14_16,  by="Category") %>% 
    ...
    dplyr::rename(Stockdale_14_16 = Category, "h^2.correlation" = "Heritability (h^2).x", "h^2.slope" = "Heritability (h^2).y", "h^2.variance" = "Heritability (h^2)")
(stock_h2_length_14_16 <- stock_h2_length_14_16[c(8,7,6,5,4,1,9,2,3,10,11),])
```

**Conclusion:** There is some evidence for heritability of body size, but it is sporadic and not overly consistent among methods.
...
**Conclusion:** Better evidence for heritability of sample date as a proxy for return time, especially between female parents and offspring. There is some inconsistency among estimation methods, however.
