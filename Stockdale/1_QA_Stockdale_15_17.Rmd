---
title: "Stockdale 2015/2017 Exploratory Analysis with 298 Markers"
output:
  html_notebook:
    theme: united
    toc: yes
...
This is a quick exploratory analysis of the Stockdale 2015/2017 genotype data. Here we will:
...
```{r loki}
sillyvec <- ProjectSillys <- c("PSTOCK15", "PSTOCK17")
...
save_objects(objects = c("LocusControl", "loci", "sillyvec", "ProjectSillys"), path = "../../Objects/Stockdale_15_17/")
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Stockdale_15_17/")
```
# OceanAK
...
stock_oceanak <- oceanak %>% 
  dplyr::filter(`Location Code` == "Stockdale Creek" & `Sample Year` %in% c(2015, 2017)) %>% 
...
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Stockdale_15_17/OceanAK")
```
...
```{r view_origin}
dplyr::bind_rows(PSTOCK15.gcl$attributes, 
                 PSTOCK17.gcl$attributes) %>% 
...
```{r split_stock15}
PSTOCK15_ids_natural <- list("PSTOCK15" = PSTOCK15.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "NO") %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID))
PoolCollections.GCL(collections = "PSTOCK15", loci = loci, IDs = PSTOCK15_ids_natural, newname = "PSTOCK15n")

PSTOCK15_ids_hatchery <- list("PSTOCK15" = PSTOCK15.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "YES") %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID))
PoolCollections.GCL(collections = "PSTOCK15", loci = loci, IDs = PSTOCK15_ids_hatchery, newname = "PSTOCK15h")

ProjectSillys_Origin <- c("PSTOCK15n", "PSTOCK15h", "PSTOCK17")
save_sillys(ProjectSillys_Origin, "../../Genotypes/Stockdale_15_17/OceanAK_Origin/")
```
...
```{r failure_rate}
project <- "Stockdale_15_17" #this is required of the function; can be any name 
failure_rate <- FailureRate.GCL(sillyvec = ProjectSillys_Origin)
failure_rate_noplots <- failure_rate[1:4]
save_objects("failure_rate_noplots", "../../Objects/Stockdale_15_17")
failure_rate
```
...
save_objects("MissLoci", path = "../../Objects/Stockdale_15_17/")
...
save_objects("DuplicateCheckReportSummary", path = "../../Objects/Stockdale_15_17/")
```
...
dup_15n_ids <- filter(dups, silly == "PSTOCK15n") %>% pull(id)
# dup_15h_ids <- filter(dups, silly == "PSTOCK15h") %>% pull(id)  # none
dup_17_ids <- filter(dups, silly == "PSTOCK17") %>% pull(id)

# Save attributes for posterity
# ID silly sources for duplicate individuals
dup_15n <- PSTOCK15n.gcl$attributes %>% 
  filter(FK_FISH_ID %in% dup_15n_ids) %>% 
  mutate(FK_FISH_ID = as.numeric(FK_FISH_ID))
dup17 <- PSTOCK17.gcl$attributes %>%  
  filter(FK_FISH_ID %in% dup_17_ids)

# One tibble with all attributes for fish to remove
duplicates_to_remove <- bind_rows(dup_15n, dup17)
save_objects("duplicates_to_remove", path = "../../Objects/Stockdale_15_17/")

# Remove fish from .gcl objects
RemoveIDs.GCL(silly = "PSTOCK15n", IDs = dup_15n_ids)
RemoveIDs.GCL(silly = "PSTOCK17", IDs = dup_17_ids)
...
PSTOCK15n_ind_het <- apply(PSTOCK15n.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
PSTOCK15h_ind_het <- apply(PSTOCK15h.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
PSTOCK17_ind_het <- apply(PSTOCK17.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} )
```
...
# Convert to tibbles
tibble_PSTOCK15n <- tibble(silly=PSTOCK15n.gcl$attributes$SILLY_CODE, sillysource=PSTOCK15n.gcl$attributes$SillySource, het=PSTOCK15n_ind_het)
tibble_PSTOCK15h <- tibble(silly=PSTOCK15h.gcl$attributes$SILLY_CODE, sillysource=PSTOCK15h.gcl$attributes$SillySource, het=PSTOCK15h_ind_het)
tibble_PSTOCK17 <- tibble(silly=PSTOCK17.gcl$attributes$SILLY_CODE, sillysource=PSTOCK17.gcl$attributes$SillySource, het=PSTOCK17_ind_het)

# Create a single tibble
het_tibble <- bind_rows(tibble_PSTOCK15n, tibble_PSTOCK15h, tibble_PSTOCK17)
...
het_tibble <- het_tibble %>% 
  dplyr::mutate(lineage = dplyr::case_when(silly %in% c("PSTOCK15", "PSTOCK17") ~ "odd",
                           TRUE ~ "even"))
save_objects("het_tibble", "../../Objects/Stockdale_15_17/")
...
save_objects("removed_hets", "../../Objects/Stockdale_15_17/")

PSTOCK15h_hets_remove <- PSTOCK15h.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PSTOCK15h", IDs = PSTOCK15h_hets_remove)

PSTOCK15n_hets_remove <- PSTOCK15n.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PSTOCK15n", IDs = PSTOCK15n_hets_remove)

PSTOCK17_hets_remove <- PSTOCK17.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PSTOCK17", IDs = PSTOCK17_hets_remove)
...
save_objects("ProjectSillys_Origin_SampleSizes", path = "../../Objects/Stockdale_15_17/")
```

Save post-QA genotypes.
```{r save_postQA_genotypes}
save_sillys(ProjectSillys_Origin, "../../Genotypes/Stockdale_15_17/OceanAK_Origin_PostQA/")
```
...
```{r genepop}
gcl2Genepop.GCL(sillyvec = c("PSTOCK15n", "PSTOCK15h", "PSTOCK17"), 
                loci = loci, 
                path = "../../Genepop/Stockdale_15_17/stockdale_15_17_postQA.gen", 
                VialNums = TRUE, 
                usat = FALSE)
```
...
genepop2franz.GCL(
  Genepop = "../../Genepop/Stockdale_15_17/stockdale_15_17_postQA.gen",
