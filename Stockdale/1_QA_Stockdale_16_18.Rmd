---
title: "Stockdale 2016/2018 Exploratory Analysis with 298 Markers"
output:
  html_notebook:
    theme: united
    toc: yes
...
This is a quick exploratory analysis of the Stockdale 2016/2018 genotype data. Here we will:
...
```{r loki}
sillyvec <- ProjectSillys <- c("PSTOCK16", "PSTOCK18")
...
if(!dir.exists("../../Objects/Stockdale_16_18/")) {dir.create("../../Objects/Stockdale_16_18/")}
save_objects(objects = c("LocusControl", "loci", "sillyvec", "ProjectSillys"), path = "../../Objects/Stockdale_16_18/")

if(!dir.exists("../../Genotypes/Stockdale_16_18/")) {dir.create("../../Genotypes/Stockdale_16_18/")}
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Stockdale_16_18/")
```
# OceanAK
...
stock_oceanak <- oceanak %>% 
  dplyr::filter(`Location Code` == "Stockdale Creek" & `Sample Year` %in% c(2016, 2018)) %>% 
...
table(PSTOCK18.gcl$attributes$SillySource %in% stock_oceanak$silly_source)
```

What attribute names do we have since we used `read_project_genotypes.GCL` rather than `LOKI2R.GCL`?
```{r}
names(PSTOCK16.gcl$attributes)
```
...
PSTOCK18.gcl$attributes <- PSTOCK18.gcl$attributes %>% 
  dplyr::left_join(stock_oceanak, by = c("SillySource" = "silly_source")) %>% 
  dplyr::select(FK_FISH_ID, SILLY_CODE, PLATE_ID, `DNA Tray Code`, `DNA Tray Well Code`, SillySource, `Sample Date`, Sex, `Length Mm`, `Location Code`, `Otolith Mark Present`, `Otolith Mark ID`, `Sample Year`) %>% 
  dplyr::rename(DNA_TRAY_CODE = "DNA Tray Code",
                DNA_TRAY_WELL_CODE = "DNA Tray Well Code")

if(!dir.exists("../../Genotypes/Stockdale_16_18/OceanAK")) {dir.create("../../Genotypes/Stockdale_16_18/OceanAK")}
save_sillys(sillyvec = sillyvec, path = "../../Genotypes/Stockdale_16_18/OceanAK")
```


## Split by otolith-origin

First we need to verify that we only have hatchery and natural-origin fish for the **F0** year (2016), and only natural-origin fish for the **F1** year (2018).
```{r view_origin}
dplyr::bind_rows(PSTOCK16.gcl$attributes, 
                 PSTOCK18.gcl$attributes) %>% 
  count(SILLY_CODE, `Otolith Mark Present`) %>% 
  spread(`Otolith Mark Present`, n, fill = 0)
```

We want to remove any NA from 2016 and hathcery-origin fish + NA from 2018. We'll need those hatchery-origin fish from 2018 for looking at 2017/2019 parentage, but not for 2016/2018. The NA's must have snuck in there from second reads?
...
PSTOCK18_ids_remove <- PSTOCK18.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "YES" | is.na(`Otolith Mark Present`)) %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PSTOCK18", IDs = PSTOCK18_ids_remove)
```
...
Now that we have removed those fish that we did not want due to their otolith status, we will split 2016 otolith-origin for QA tabulating purposes. Also create a new *sillyvec*.
```{r split_stock15}
PSTOCK16_ids_natural <- list("PSTOCK16" = PSTOCK16.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "NO") %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID))
PoolCollections.GCL(collections = "PSTOCK16", loci = loci, IDs = PSTOCK16_ids_natural, newname = "PSTOCK16n")

PSTOCK16_ids_hatchery <- list("PSTOCK16" = PSTOCK16.gcl$attributes %>% 
  filter(`Otolith Mark Present` == "YES") %>% 
  mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  pull(FK_FISH_ID))
PoolCollections.GCL(collections = "PSTOCK16", loci = loci, IDs = PSTOCK16_ids_hatchery, newname = "PSTOCK16h")

ProjectSillys_Origin <- c("PSTOCK16n", "PSTOCK16h", "PSTOCK18")
if(!dir.exists("../../Genotypes/Stockdale_16_18/OceanAK_Origin")) {dir.create("../../Genotypes/Stockdale_16_18/OceanAK_Origin")}
save_sillys(ProjectSillys_Origin, "../../Genotypes/Stockdale_16_18/OceanAK_Origin/")
```
...
```{r failure_rate}
project <- "Stockdale_16_18" #this is required of the function; can be any name 
failure_rate <- FailureRate.GCL(sillyvec = ProjectSillys_Origin)
failure_rate_noplots <- failure_rate[1:4]
save_objects("failure_rate_noplots", "../../Objects/Stockdale_16_18")
failure_rate
```
...
save_objects("MissLoci", path = "../../Objects/Stockdale_16_18/")
...
save_objects("DuplicateCheckReportSummary", path = "../../Objects/Stockdale_16_18/")
```
...
dup_16n_ids <- filter(dups, silly == "PSTOCK16n") %>% pull(id)
# dup_16h_ids <- filter(dups, silly == "PSTOCK16h") %>% pull(id)  # none
dup_18_ids <- filter(dups, silly == "PSTOCK18") %>% pull(id)

# Save attributes for posterity
# ID silly sources for duplicate individuals
dup_16n <- PSTOCK16n.gcl$attributes %>% 
  filter(FK_FISH_ID %in% dup_16n_ids) %>% 
  mutate(FK_FISH_ID = as.numeric(FK_FISH_ID))
dup18 <- PSTOCK18.gcl$attributes %>%  
...
save_objects("duplicates_to_remove", path = "../../Objects/Stockdale_16_18/")

# Remove fish from .gcl objects
RemoveIDs.GCL(silly = "PSTOCK16n", IDs = dup_16n_ids)
RemoveIDs.GCL(silly = "PSTOCK18", IDs = dup_18_ids)
...
PSTOCK16n_ind_het <- apply(PSTOCK16n.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
PSTOCK16h_ind_het <- apply(PSTOCK16h.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} ) 
PSTOCK18_ind_het <- apply(PSTOCK18.gcl$scores, 1, function(ind) {sum(ind[ ,"Dose1"] != ind[ ,"Dose2"])/298} )
```
...
# Convert to tibbles
tibble_PSTOCK16n <- tibble(silly=PSTOCK16n.gcl$attributes$SILLY_CODE, sillysource=PSTOCK16n.gcl$attributes$SillySource, het=PSTOCK16n_ind_het)
tibble_PSTOCK16h <- tibble(silly=PSTOCK16h.gcl$attributes$SILLY_CODE, sillysource=PSTOCK16h.gcl$attributes$SillySource, het=PSTOCK16h_ind_het)
tibble_PSTOCK18 <- tibble(silly=PSTOCK18.gcl$attributes$SILLY_CODE, sillysource=PSTOCK18.gcl$attributes$SillySource, het=PSTOCK18_ind_het)

# Create a single tibble
het_tibble <- bind_rows(tibble_PSTOCK16n, tibble_PSTOCK16h, tibble_PSTOCK18)
...
het_tibble <- het_tibble %>% 
  dplyr::mutate(lineage = dplyr::case_when(silly %in% c("PSTOCK16", "PSTOCK18") ~ "even",
                           TRUE ~ "even"))
save_objects("het_tibble", "../../Objects/Stockdale_16_18/")
...
save_objects("removed_hets", "../../Objects/Stockdale_16_18/")

PSTOCK16h_hets_remove <- PSTOCK16h.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PSTOCK16h", IDs = PSTOCK16h_hets_remove)

PSTOCK16n_hets_remove <- PSTOCK16n.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PSTOCK16n", IDs = PSTOCK16n_hets_remove)

PSTOCK18_hets_remove <- PSTOCK18.gcl$attributes %>% 
  dplyr::filter(SillySource %in% removed_hets) %>% 
  dplyr::mutate(FK_FISH_ID = as.character(FK_FISH_ID)) %>% 
  dplyr::pull(FK_FISH_ID)
RemoveIDs.GCL(silly = "PSTOCK18", IDs = PSTOCK18_hets_remove)
...
save_objects("ProjectSillys_Origin_SampleSizes", path = "../../Objects/Stockdale_16_18/")
```

Save post-QA genotypes.
```{r save_postQA_genotypes}
if(!dir.exists("../../Genotypes/Stockdale_16_18/OceanAK_Origin_PostQA")) {dir.create("../../Genotypes/Stockdale_16_18/OceanAK_Origin_PostQA")}
save_sillys(ProjectSillys_Origin, "../../Genotypes/Stockdale_16_18/OceanAK_Origin_PostQA/")
```
...
```{r genepop}
if(!dir.exists("../../Genepop/Stockdale_16_18")) {dir.create("../../Genepop/Stockdale_16_18")}

gcl2Genepop.GCL(sillyvec = c("PSTOCK16n", "PSTOCK16h", "PSTOCK18"), 
                loci = loci, 
                path = "../../Genepop/Stockdale_16_18/stockdale_16_18_postQA.gen", 
                VialNums = TRUE, 
                usat = FALSE)
```
...
genepop2franz.GCL(
  Genepop = "../../Genepop/Stockdale_16_18/stockdale_16_18_postQA.gen",
