---
title: "Stockdale 2017/2019 Exploratory Analysis with 298 Markers"
output:
  html_notebook:
    theme: united
    toc: yes
...
stock_oceanak <- oceanak %>% 
  dplyr::filter(`Location Code` == "Stockdale Creek" & `Sample Year` %in% c(2017, 2019)) %>% 
...
```{r}
table(PSTOCK17.gcl$attributes$SillySource %in% stock_oceanak$silly_source)
table(PSTOCK19.gcl$attributes$SillySource %in% stock_oceanak$silly_source)
```
...
dplyr::bind_rows(PSTOCK17.gcl$attributes, 
  ...
  spread(`Otolith Mark Present`, n, fill = 0)
```
There are 44 NAs from 2017.

We want to remove any NAs from 2017 and hatchery-origin fish + NA from 2019. We'll need those hatchery-origin fish from 2017 for looking at 2017/2019 parentage. The NAs must be from second reads.
...
failure_rate
```
There's a 9% genotype failure rate in PSTOCK17 and 2% in PSTOCK19, with an overall rate of 6%. There are some specific plate effects, but they are very scattered, as are any locus-specific failures. 
...
```
Removed 1186 fish IDs from PSTOCK17n, 116 from PSTOCK17h, and 207 from PSTOCK19.
...
```
There were four apparent duplicates in PSTOCK17n and five in PSTOCK19. There were no duplicates in PSTOCK17h.
...
het_tibble %>% 
  ...
  dplyr::count(silly)
```
We'll remove another 161 fish total.
...
```
Final dataset contains 6,424 total fish: 9,620 from 2017 and 6,804 from 2019 (that's a lot!).
