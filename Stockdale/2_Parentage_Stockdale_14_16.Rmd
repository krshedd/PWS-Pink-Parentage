...
paired_14_16_filter %>% 
  ...
  xlab("Day of Year") +
  labs(title= "Stockdale 2014/2016",
       fill = "Origin") +
...
**Note** that *FRANz* assigned 1055 offspring to 405 parents. 865 of these offsrping have natural-origin parents and 373 offspring have hatchery-origin parents. The hatchery origin fish are from AFK12B, CCH12, and WNH12PINKB.
...
Of the 5,199 offspring collected in 2016, 1055 were assigned to parents, for an overall offspring assignment rate of **20.3%**. 
...
Female parents produced 1-12 offspring, while male parents produced 1-19.  
...
```{r calculate RRS and CIs for males and females combined even}
# Mean RS including 0's
rs_14_16_0 <- paired_14_16_filter_parents %>%
  group_by(origin) %>% 
  summarise(RS = mean(n, na.rm = TRUE))
...
We have 183 individuals who were assigned to 2 parents, but I don't have time to look at this now.
...
2. 1055 offspring were assigned to 405 parents, for an assignment rate of **20.3%**.  
3. 183 offspring assigned to 2 parents (triads).  
4. RRS (with 0's) was significant for both females (0.42) and males (0.28).  
5. Family size varied from 0-12 for females and 0-19 for males.  
