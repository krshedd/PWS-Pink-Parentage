---
title: "Stockdale 2014/2016 Single-generation Parentage REanalysis with 298 Markers"
...
#    a. Specific terms requiring replacement in the Introduction and below for future use include: "Stockdale", "STOCK", "stock_" 2013", "2015", 2017", "2019", "_13", "_15", "_14", "_16"
...
```
# **Background**
This is a rerun of Stockdale 2014-2016 due to genotyping of additional offspring.

## Pedigree reconstruction
We use the program *FRANz* [https://www.bioinf.uni-leipzig.de/Software/FRANz/About.html] to reconstruct parent-offspring relationships. Below are analyses of parentage for **Stockdale** for the **odd lineage** years **2014** (potential parents) and **2016** (potential offspring).

*FRANz* was begun on Thu Aug 23 2021 at 15:33 using the following parameters in the command line in the V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\**Stockdale_2014_2016r** folder:
**FRANz --Nmmax 2250 --Nfmax 2250 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 "Stockdale_14_16_postQA_2014_2016_STOCK.dat"**

The parameters are defined as follows:

* --Nmmax and --Nfmax are the maximum numbers of candidate mothers and fathers. To obtain our values, we used an estimated escapement of 4,500 and divided by 2. This escapement number came from rounding the stream walk survey number of 4,516, the largest of the three estimates. This estimate is listed under 'pop est walk stream' in the Excel file found here: "V:\Documents\5_Coastwide\Multispecies\AHRP\Field data\Sample Summary Extract and Genotype.xlsx".    
...
parentage_14_16 <- read_csv("../../Franz/Stockdale_2014_2016r/parentage.csv") 
paired_14_16 <- read_csv("../Franz/Stockdale_14_16r_postQA_OceanAK_paired_2014_2016_STOCK.csv") 
```
...
write_csv(paired_14_16_filter, "Stockdale_paired_14_16r_filter.csv")
```
...
```
YAY! There are more 2016 fish now.
...
sample_size_plot(indata = paired_14_16_filter, title = "Stockdale 2014-2016")
```
# **Basic Statistics**
...
write_csv(parents_paired_14_16, "Stockdale_parents_paired_14_16r.csv")
...
parents_paired_14_16 %>% 
  ...
  spread(origin, n)
```
**Results:** Of the **5,814** natural-origin offspring genotyped in 2016, *FRANz* assigned **1,377**, for an assignment rate of **23.7%**, to **417** parents (**243** natural and **174** hatchery). Most assignments (**979**) were to one parent, but there were **199** two-parent assignments. A total of **964** offspring assignments were to a natural-origin parent and **413** were to a hatchery-origin parent, with **399** from AFK12B, **13** from CCH12, **0** from SGH12, and **1** from WNH12PINKB.
...
colnames(spawners_and_offspring_14_16) <- c("Stockdale_spawners_14", "Stockdale_offspring_16")
...
parents_paired_14_16 %>% 
  ...
  theme_bw()
```
**Results:** Female parents produced **0-13** offspring, while known male parents produced **0-20**.  
...
write_csv(paired_14_16_filter_parents, "Stockdale_paired_14_16r_filter_parents.csv")
...
paired_14_16_filter_parents %>%
 ...
 annotate("text", x = M_h_0_14_16 + 0.5, y = .300, label = paste("Male_hatchery_RS =", round(M_h_0_14_16, 2)), hjust = 0, size = 6, color = cbPalette2[2]) +
  ...
  scale_fill_manual(values = cbPalette2)
```
**Results:** RSs for natural-origin fish for both sexes were much higher than for hatchery-origin fish. In the histograms, note the large number of potential parents assigned no offspring that were absent from the family size plots above.
...
RRS_0 %>%
  spread(Sex, p_value)
```
**Results:** There are apparently highly significant differences in RRS for both males and females as well as overall. 
...
colnames(rrs_14_16) <- c("Stockdale_females_14_16", "Stockdale_males_14_16")
...
paired_14_16_filter_parents %>% 
  ...
  scale_fill_manual(values = cbPalette2)
```
**Results:* Natural-origin fish of both sexes are more successful particularly early and late season.
...
write_csv(parents_paired_14_16_cross, "Stockdale_parents_paired_14_16r_cross.csv")
...
parents_paired_14_16_cross %>% 
  ...
  scale_fill_manual(values = cbPalette1)
```
**Results:** Offspring from all four types of crosses are represented. The majority (55%) are NN.
...
colnames(rrs_Xtype_14_16) <- c("Stockdale_HHvsNN_14_16", "Stockdale_HNvsNN_14_16", "Stockdale_NHvsNN_14_16")
rownames(rrs_Xtype_14_16) <- c("Lower 95% CL", "RRS", "Upper 95% CL")

rrs_Xtype_14_16 %>% 
  round(2)
```
**Results:** RRS for the comparisons were **`r round(rrs_Xtype_14_16[2,1], 2)` (95% CI [`r round(rrs_Xtype_14_16[1,1], 2)`-`r round(rrs_Xtype_14_16[3,1], 2)`])** for HH, **`r round(rrs_Xtype_14_16[2,2], 2)` (95% CI [`r round(rrs_Xtype_14_16[1,2], 2)`-`r round(rrs_Xtype_14_16[3,2], 2)`])** for HN, and **`r round(rrs_Xtype_14_16[2,3], 2)` (95% CI [`r round(rrs_Xtype_14_16[1,3], 2)`-`r round(rrs_Xtype_14_16[3,3], 2)`])** for NH, indicating no significant differences in RS relative to the NN cross type. It should be noted again, however, that the numbers of NH and HN crosses relative to NN were very small.
...
```
**Results:** The relationship between dam and sire sample date was generally linear, with males in the streams earlier than females, indicating the cross data are likely correct. There were **122** unique matings, and both sexes mated with multiple partners, although males were more promiscuous than females.  
...
```
**Results:** Eight females mated with a sires of a differing origin.
...
```
**Results:** Eight males mated with a dams of a differing origin.
...
# # Read in data
# parents_paired_15_14 <- read_csv("stock_parents_paired_15_14.csv") %>% 
#   dplyr::select(-origin)
# 
# paired_14_16_filter_parents <- read_csv("Stockdale_paired_14_16_filter_parents.csv")
# 
# parentage_paired_14_16 <- read_csv("Stockdale_parents_paired_14_16.csv")%>% 
#   dplyr::select(-origin)
```
...
# 
# colnames(rrs_gpar_14_16) <- c("Stockdale_grandparents_14_16")
...
# 
# write_csv(grandparentage, "Stockdale_14_16_grandparentage.csv")
```
...
#   dplyr::select(-origin)
# 
# parents_paired_14_16 <- read_csv("Stockdale_parents_paired_14_16.csv") %>% 
#   dplyr::select(-origin)
# 
# paired_14_16_filter_parents <- read_csv("Stockdale_paired_14_16_filter_parents.csv")
# 
# paired_15_14_filter_parents <- read_csv("stock_paired_15_14_filter_parents.csv")
```
...
2. 1,377 of 5,814 total offspring were assigned to 417 parents (243 natural and 174 hatchery), for an assignment rate of 23.7%.  
3. 199 offspring assigned to 2 parents (triads).  
4. RRS was significant for females (0.41) and males (0.29).  
5. All 4 crosses were represented (HH, NH, HN, and NN), and all 3 crosses had RRS < 1 when compared to the NN cross, although results were not significant.  
6. Family size varied from 0-13 for females and 0-20 for males.  
