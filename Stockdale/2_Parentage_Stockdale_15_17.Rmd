...
pedigree_df %>% 
  with(.,
     pedigree(id = Offspring, dadid = Sire, momid = Dam, sex = Sex, affected = Origin))
...
```{r}
parent1 <- parents_paired_15_17 %>% 
  filter(Sex.par == "Male") %>%
  spread(Sex.par, Parent_ID) %>% 
  dplyr::select(Offspring, Male, Sex.off, origin.off)

parent2 <- parents_paired_15_17 %>% 
  filter(Sex.par == "Female") %>%
  spread(Sex.par, Parent_ID) %>% 
  dplyr::select(Offspring, Female, Sex.off, origin.off)

parent1 %>% 
  full_join(parent2, by = c("Offspring", "Sex.off", "origin.off"))
```
