...
All output files can be found here: "V:\Analysis\5_Coastwide\Multispecies\Alaska Hatchery Research Program\PWS Pink\Franz\Stockdale_2015_2017"  
...
parentage_15_17 <- read_csv("../../Franz/Stockdale_2015_2017/parentage.csv")
paired_15_17 <- read_csv("../Franz/stockdale_15_17_postQA_OceanAK_paired_2015_2017_STOCK.csv")
```
...
Almost all parentage assigments have a posterior probability of 1, which is very robust.  
...
paired_15_17_filter <- paired_15_17 %>% 
...
write_csv(paired_15_17_filter, "stock_paired_15_17_filter.csv")
```
...
## Sample sizes
Quick look at what our sample sizes are for potential parents (2015) and potential offspring (2017).
...
paired_15_17_filter %>% 
  ...
  xlab("Day of Year") +
  labs(title= "Stockdale 2015/2017",
       fill = "Origin") +
...
write_csv(parents_paired_15_17, "stock_parents_paired_15_17.csv")
...
**Note** that *FRANz* assigned 564 offspring to 468 parents. 561 of these offsrping have natural-origin parents and 71 offspring have hatchery-origin parents. The hatchery origin fish are from AFK13B, CCH13, and WNH13PINKB.
...
O.odd <- c("H", "N")
Spawners.odd <- c(1487, 6064) #  number of hatchery- and natural-origin parents
Offspring.odd <- c(71, 561)  # number of offspring assigned to hatchery- and natural-origin parents
...
Here, we calculate and plot family size for both natural and hatchery parents.  Family size ranges from 1-6. Most assignments were to 1 parent (*single parent-offspring duos*) - there were 68 2 parent assignments (*parent pair-offspring trios*). 

Of the 8,702 offspring collected in 2017, 564 were assigned to parents, for an overall offspring assignment rate of **6.5%**. 
...
Female parents produced 1-6 offspring, while male parents produced 1-6.  

Below, we'll calculate reproductive success of natural and hatchery females and males. 

```{r odd_rrs}
(rs_15_17 <- parents_paired_15_17 %>%
  count(`Fish ID.par`, origin, SEX.par) %>%
...
# RS_combined <- bind_rows(parents_paired_15_17, parents_paired_15_17) 
...
write_csv(paired_15_17_filter_parents, "stock_paired_15_17_filter_parents.csv")
...
#  geom_vline(xintercept=F_n_0_15_17, color=gg_color_hue(2)[1], linetype = "dashed", size=2) +
#  geom_vline(xintercept=F_h_0_15_17, size=2) +
#  geom_vline(xintercept=F_h_0_15_17, color=gg_color_hue(2)[2], linetype = "dashed", size=2) +
#  annotate("text", x = F_n_0_15_17 + 0.5, y = 250, label = paste("F_n_0 =", round(F_n_0_15_17, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
#  annotate("text", x = F_h_0_15_17 + 0.5, y = 300, label = paste("F_h_0 =", round(F_h_0_15_17, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2])
...
#  geom_vline(xintercept=M_n_0_15_17, color=gg_color_hue(2)[1], linetype = "dashed", size=2) +
#  geom_vline(xintercept=M_h_0_15_17, size=2) +
#  geom_vline(xintercept=M_h_0_15_17, color=gg_color_hue(2)[2], linetype = "dashed", size=2) +
#  annotate("text", x = M_n_0_15_17 + 0.5, y = 150, label = paste("M_n_0 =", round(M_n_0_15_17, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
#  annotate("text", x = M_h_0_15_17 + 0.5, y = 100, label = paste("M_h_0 =", round(M_h_0_15_17, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2])
...
paired_15_17_filter_parents %>% 
  ...
  geom_vline(xintercept=M_h_0_15_17, size=2) +
  geom_vline(xintercept=M_h_0_15_17, color=gg_color_hue(2)[2], linetype = "dashed", size=2) +
  annotate("text", x = M_n_0_15_17 + 0.5, y = .250, label = paste("M_n_0 =", round(M_n_0_15_17, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[1]) +
  annotate("text", x = M_h_0_15_17 + 0.5, y = .300, label = paste("M_h_0 =", round(M_h_0_15_17, 2)), hjust = 0, size = 6, color = gg_color_hue(2)[2])
...
**RRS for males = `r round(RRS_m_0_15_17, 2)`. RRS for females = `r round(RRS_f_0_15_17, 2)`.**
...
Calculate 95% CIs for Stockdale 15-17 females. RRS = 0.41 (0.29-0.58)

```{r RRS_confidence_intervals_female_15_17}
# Females
...
rrs_ci_kalinowski(n_h_off = 35, n_w_off = 325, n_h_par = 791, n_w_par = 3040, alpha = 0.05)
```

Calculate 95% CIs for Hogan 15-17 males. RRS = 0.66 (0.46-0.93)

```{r RRS_confidence_intervals_male_15_17}
# Males
# Get parent sample sizes
paired_15_17_filter_parents %>% 
  filter(SEX == "M") %>% 
...
rrs_ci_kalinowski(n_h_off = 36, n_w_off = 236, n_h_par = 696, n_w_par = 3019, alpha = 0.05)
```

#### RS vs. field data plots

```{r paired_15_17_filter_parents_sex_odd}
...
parents_paired_15_17_cross <- parentage_15_17 %>% 
  filter(Posterior > 0.9) %>% 
  dplyr::select(Offspring, `Parent 1`, `Parent 2`) %>% 
  filter(Offspring %in% offspring_trio_15_17) %>% 
  left_join(paired_15_17_filter, by = c("Offspring" = "franz_id")) %>% 
...
write_csv(parents_paired_15_17_cross, "stock_parents_paired_15_17_cross.csv")
...
```
Offspring from all four types of crosses are represented in our dataset. HH crosses produced 0 offspring, HN crosses produced 1 offspring, NH crosses produced 1 offspring, and NN crosses produced 1-1 offspring.
...
Is this significant?
```{r RRS_confidence_intervals_15_17_setup}
...
```{r RRS_confidence_intervals_15_17}
# HH vs. NN
...
Find the 2015 fish that were offspring from 2013 and see how many offspring those had returning in 2017!
```{r}
...
paired_15_17_filter_parents %>% 
  right_join(parents_paired_13_15, by = c("franz_id" = "Offspring")) %>% 
  count(origin.par, n)
```

None of the hatchery fish from 2013 produced grandoffspring. In 2013, hatchery-origin fish produced 10 offspring in 2015 and 3 grandoffspring in 2017. In 2013, natural-origin fish produced 119 offspring in 2015 and 19 grandoffspring in 2017.

### RRS of fish with hatchery vs. natural parents

Origin of 2013 fish, their F1 offspring, and their F2 offspring.
```{r}
paired_15_17_filter_parents %>% 
  right_join(parents_paired_13_15, by = c("franz_id" = "Offspring")) %>% 
  group_by(origin.par) %>% 
...
```{r}
rrs_ci_kalinowski(n_h_off = 3, n_w_off = 19, n_h_par = 10, n_w_par = 119, alpha = 0.05)
```
...
Get dam and sire for each offspring 2013/2015
```{r}
(
  sires_13 <- parents_paired_13_15 %>%
    filter(Sex.par == "Male") %>%
...
(
  dams_13 <- parents_paired_13_15 %>%
    filter(Sex.par == "Female") %>%
    rename(Dam = Parent_ID, Dam_origin = origin.par) %>%
    dplyr::select(Offspring, Dam, Dam_origin)
)

(parents_13_15 <- full_join(sires_13, dams_13, by = "Offspring") %>%
    arrange(Offspring))
```

Get dam and sire for each offspring 2015/2017
```{r}
(
...
(
  stock_grandparentage <- full_join(sires_15, dams_15, by = "Offspring") %>% 
    arrange(Offspring) %>% 
    left_join(parents_13_15, by = c("Sire" = "Offspring"), suffix = c("", "_paternal")) %>% 
    left_join(parents_13_15, by = c("Dam" = "Offspring"), suffix = c("", "_maternal")) %>%
    group_by(Offspring) %>% 
...
pedigree_df <- bind_rows(
  stock_grandparentage %>%
    ...
    left_join(dplyr::select(parents_paired_15_17, Offspring, Sex.off), by = "Offspring") %>%
...
2. 564 offspring were assigned to 468 parents, for an assignment rate of **6.5%**.  
3. 68 offspring assigned to 2 parents (triads).  
4. RRS (with 0's) was significant for both males (0.66) and females (0.41).  
4. RRS (without 0's) was not significant.  
6. There was not enough cross type data (triads) to make meaningful comparisons.  
7. Family size varied from 0-6 for females and 0-6 for males.  
