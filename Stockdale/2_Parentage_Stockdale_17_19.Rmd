...
parents_paired_17_19 %>% 
  ...
  spread(origin, n)
```
**Results:** Of the **6,804** natural-origin offspring genotyped in 2019, *FRANz* assigned **604**, for an assignment rate of **8.9%**, to **602** parents (**576** natural and **26** hatchery). Most assignments (**479**) were to one parent (*single parent-offspring dyads*), but there were **125** two-parent assignments (*parent pair-offspring triads*). A total of **699** offspring assignments were to a natural-origin parent and **30** were to a hatchery-origin parent, with **20** were from AFK15B, **4** from CCH15, **4** from SGH15, and **2** from WNH14PINKB.
...
paired_17_19_filt_parents <- read_csv("Stockdale_paired_17_19_filter_parents.csv")
...
```
  ...
  2. How many grandparents (nn), independent of origin, produced a certain number of grandoffspring (n)
...
```{r stock_ggparents_counts_and_rrs, echo=FALSE, warning=FALSE, message=FALSE}
# Read in data
...
# **Summary of Results** NEEDS UPDATING
...
## 2015-2017-2019 (Great grandparentage)
