...
#    a. Specific terms requiring replacement in the Introduction and below for future use include: "Stockdale", "2013", "2015", "2017", "2019", "_13", "_15", "_17", "_19"
...
parentage_17_19 <- read_csv("V:/Analysis/5_Coastwide/Multispecies/Alaska Hatchery Research Program/PWS Pink/Franz/Stockdale_2017_1019/parentage.csv") # for some reason, I keep getting and error with a relative path
paired_17_19 <- read_csv("../Franz/Stockdale_17_19_postQA_OceanAK_paired_2017_2019_STOCK.csv") # works for this one, tho
```
...
sample_size_plot <- function(indata = paired_XX_XX_filter, title = "Stream Year-Year") {
  ...
  mutate(Sex = case_when(SEX == "M" ~ "Male", SEX == "F" ~ "Female")) %>%
  mutate(date = mdy_hm(`Sample Date`)) %>% # imported data format for Sample Date changing - make sure to check
  mutate(DOY = yday(date)) %>%
  ggplot(aes(x = DOY)) +
...
parents_paired_17_19 %>% 
  ...
  spread(origin, n)
```
**Results:** Of the **6,804** natural-origin offspring genotyped in 2019, *FRANz* assigned **604**, for an assignment rate of **8.9%**, to **602** parents (**576** natural and **26** hatchery). Most assignments (**479**) were to one parent only, but there were **125** two-parent assignments. A total of **699** offspring assignments were to a natural-origin parent and **30** were to a hatchery-origin parent, with **20** from AFK15B, **4** from CCH15, **4** from SGH15, and **2** from WNH15PINKB.
...
parents_paired_17_19_cross_dam_sire %>% 
  mutate(`Sample Date_dam` = mdy_hm(`Sample Date_dam`),
         `Sample Date_sire` = mdy_hm(`Sample Date_sire`)) %>% 
...
```
**Results:** A total of **54** unique hatchery-origin grandparents were assigned to **71** parents, **10** of which assigned to **11** grandoffspring, and **414** unique natural-origin grandparents assigned to **561** parents, **54** of which assigned to **74** grandoffspring. Each assigned grandparent contributed to **1-4** grandoffspring. RRS from the parental to grandoffspring generation was **`r round(rrs_gpar_17_19[2,1], 2)` (95% CI [`r round(rrs_gpar_17_19[1,1], 2)`-`r round(rrs_gpar_17_19[3,1], 2)`])**. However, this result requires circumspection and cautious interpretation. It is not a pure RRS estimate because it is based on a fraction of full familial origin/ancestry information. Only one parent is known for the majority of grandoffpring; an unknown parent may be of either origin and may have been sampled or unsampled. Compounding that, **52** grandoffspring had one grandparent assigned, **13** had two grandparents assigned, **one** grandoffspring each had three and four grandparents assigned. Again, any unknown grandparent(s) are of unknown origin.
...
1. A total of 468 unique grandparents were identified that produced 84 grandoffspring, 52 of which had one grandparent assigned, 13 of which had two grandparents assigned, and one each of which had three and four grandparents assigned.
