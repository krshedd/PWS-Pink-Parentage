...
write_csv(glm_model_summary, "stock_glm_summary.csv")
```

It looks like all of the top models, to some degree, incorporate all potential explanatory variables. The proportion of deviance explained (1 - residual_deviance / null_deviance) ~ 30%.
...
write_csv(female_glm_model_summary, "stock_glm_female_summary.csv")
```

It looks like the top model has 99% of the Akaike Weight and incorporates all potential explanatory variables. The proportion of deviance explained (1 - residual_deviance / null_deviance) ~ 25%.
...
Looking at these incident ratios, we can see that the RRS of hatchery-origin females relative to natural-origin females is ~ 0.60 (0.45-0.79) holding the other variables constant (length, distance, and date). Also, the percent change in the number of offspring increases by ~ 0.0% for every Mm increase in length, decreases by ~ 0.2% for every meter further upstream a fish is sampled, and decreases ~ 3.0% for every day later in the season a fish is sampled.
...
After going down numerous rabbit holes (prior to Randy's fix of `hier.part` above) trying to figure out the most appropriate way to determine the relative importance of our explanatory variables (dominance analysis didn't work for glm.nb, nor did relimpo), I came across the idea of using beta coefficients (see link below). I made all of our explanatory variables numeric, then scaled and centered them before running the GLM.

[link](https://stats.stackexchange.com/questions/46096/how-to-compare-coefficients-of-a-negative-binomial-regression-for-determining-re)

```{r}
RS.glm.F.1_numeric_beta <-
  glm.nb(n ~ scale(distance, center = TRUE) + scale(origin, center = TRUE) + scale(date, center = TRUE) + scale(length, center = TRUE),
         data = Female.even_numeric,
         link = log)
summary(RS.glm.F.1_numeric_beta)
```

Can use `anova` to look at the deviance residuals, however, each variable is added sequentially, so the deviance residuals change depending on the order of your explanatory variables. This is *not* super useful in our circumstance.
```{r}
anova(RS.glm.F.1_numeric_beta)
```
...
write_csv(male_glm_model_summary, "stock_glm_male_summary.csv")
```

It looks like the top model has 100% of the Akaike Weight and incorporates all potential explanatory variables. The proportion of deviance explained (1 - residual_deviance / null_deviance) ~ 36%.
