...
# Introduction
This document is a summary of parentage analysis and relative reproductive success (RRS) results from Stockdale Creek 2013-2016 for the Science Panel. Unless the Science Panel has concerns with how the analyses were done, these RRS results should be considered finalized. We are open to additional analyses and/or figures (e.g. investigating the impact of spawning location on reproductive success). We intend to present these results at both the March 7th AHRP Informational Meeting and at the Alaska Chapter of the American Fisheries Society meeting in Sitka March 19-22. Note that while the odd-year Stockdale analyses were fully funded by the AHRP, the even-year genetic analyses were funded by a Saltonstall-Kennedy grant which ends June 30, 2019.

# Odd lineage (2013/2015)

## Sample sizes
Below is a comparison number of samples per sample dates across all of the individuals collected in 2013 and 2015 to examine differences in run timing between hatchery- and natural-origin fish in the paren generation (2013) and look for completeness of sampling throughout the run. 
...
Quick look at what our sample sizes are for potential parents (2013) and potential offspring (2015) by sex and origin.
...
Of the 3,093 offspring collected in 2015, 72 were assigned to parents, for an overall offspring assignment rate of **2.3%**. All assignments were to 1 parent (*single parent-offspring duos*) - there were no 2 parent assignments (*parent pair-offspring trios*).
...
Here, we calculate and plot reproductive success (RS; number of offspring assigned to parents) for both natural and hatchery parents. The number of offspring assigned ranges from 0-4. 

### Calculate average RS
We'll calculate the average RS by origin and sex including sampled fish from the parental generation to which offspring were not assigned.
...
The above table shows the *p-values* for each test and sex. There is a significant difference in RRS for females, but not males. Female hatchery-origin fish produced significantly fewer offspring than natural-origin females, but there is not a significant diference in RS between hatchery- and natural-origin males. 
...
Stockdale 2013/2015 **female** RRS with 95% CI = **`r paste0(odd_female_rrs_CI[2], " (", odd_female_rrs_CI[1], "-", odd_female_rrs_CI[3], ")")`**

Stockdale 2013/2015 **male** RRS with 95% CI = **`r paste0(odd_male_rrs_CI[2], " (", odd_male_rrs_CI[1], "-", odd_male_rrs_CI[3], ")")`**

### Plots of RS vs. field data
Here, we look for associations between two potential explanatory variables (sample date and body size) and RS. 
...
2. 72 offspring were assigned to 54 parents (3 of which were hatchery-origin), for an assignment rate of **2.3%**. This assignment rate is similar to what was observed for Hogan 2013/2015 (huge parent escapement in 2013 relative to sampling rate). 
3. The number of offspring assigned to parents varied from 0-2 for females and 0-4 for males. 
4. RRS for females = 0.13 (0.01-0.59). RRS for males = 0.34 (0.05-1.10). Hatchery-origin females produce significantly fewer offspring than natural-origin females, but there is not a statistically significant difference for males. 


# Even lineage (2014/2016)

## Sample sizes
Below is a comparison number of samples per sample dates across all of the individuals collected in 2014 and 2016 to examine differences in run timing between hatchery- and natural-origin fish in the paren generation (2014) and look for completeness of sampling throughout the run. 

From this histogram, we can see that in the parent generation (2014), natural-origin fish tended to have earlier sampling dates than hatchery-origin fish, which confirms what we know about differences in run timing. We can also see that while a larger number of samples were collected in the offspring year (2016), sampling was representative throughout the season.
...
Quick look at what our sample sizes are for potential parents (2014) and potential offspring (2016) by sex and origin.
...
Almost all parentage assigments have a posterior probability of 1, which is very robust. This means that very few offspring assignments were split among multiple potential parents. 

Of the 5,199 offspring collected in 2016, 1,054 were assigned to parents, for an overall offspring assignment rate of **20.3%**.

While most of the assignments were to 1 parent (*single parent-offspring duos*) - there were 183 2 parent assignments (*parent pair-offspring trios*).
...
The above table shows the *p-values* for each test and sex. There is a significant difference in RRS for both females and males.
...
Stockdale 2014/2016 **female** RRS with 95% CI = **`r paste0(even_female_rrs_CI[2], " (", even_female_rrs_CI[1], "-", even_female_rrs_CI[3], ")")`**

Stockdale 2014/2016 **male** RRS with 95% CI = **`r paste0(even_male_rrs_CI[2], " (", even_male_rrs_CI[1], "-", even_male_rrs_CI[3], ")")`**

### Plots of RS vs. field data
As in the odd-lineage above, here we look for associations between two potential explanatory variables (sample date and body size) and RS. Since we have many more offspring assignments, we can more clearly visualize these relationships.
...
The above plot show RS by parent sample date, separately for females (top) and males (bottom). We see that reproductive success does vary due to sample date, with the lowest RS values in the middle of the season (perhaps due to increased fishing pressure or density dependence on the spawning ground), however, both early and late in the season natural-origin fish have higher RS than hatchery-origin fish in both sexes. This suggests that our measured difference in RRS is not due solely to differences in run timing (sample date) between hatchery- and nautral-origin fish. 
...
The above plot show RS by parent body length, separately for females (top) and males (bottom). Larger males are much more successful (but only natural-origin males). Again, we see at most size classes that natural-origin fish have higher RS than hatchery-origin fish. Size alone, therefore, doesn’t explain differences in RS.

## Parent pair-offspring trios
Because we have 183 individuals who were assigned to 115 different sets of 2 parent matings, we are able to examine instances of crosses between hatchery fish, between natural fish, and between natural and hatchery fish. **Note** that we have divided natural-hatchery crosses into "HN", which indicates a hatchery-origin mother and natural-origin father and "NH", which indicates a natural-origin mother and hatchery-origin father. 

### Sample sizes
Quick look at what our sample sizes are for the number of families found by cross type.
...
### Calculate average RS
Here we calculate the average RS for each cross types. Note that NN families have the highest RS, HH families have the lowest RS, and hybrids have an intermediate RS.
```{r cross_rs_even}
parents_paired_14_16_cross %>% 
  mutate(`Cross Type` = cross) %>% 
  count(`Cross Type`, `Fish ID`, `Fish ID.par1`) %>% 
...
Stockdale 2014/2016 **HH/NN** RRS with 95% CI = **`r paste0(even_HH_NN_rrs_CI[2], " (", even_HH_NN_rrs_CI[1], "-", even_HH_NN_rrs_CI[3], ")")`**

Stockdale 2014/2016 **HN/NN** RRS with 95% CI = **`r paste0(even_HN_NN_rrs_CI[2], " (", even_HN_NN_rrs_CI[1], "-", even_HN_NN_rrs_CI[3], ")")`**

Stockdale 2014/2016 **NH/NN** RRS with 95% CI = **`r paste0(even_NH_NN_rrs_CI[2], " (", even_NH_NN_rrs_CI[1], "-", even_NH_NN_rrs_CI[3], ")")`**

The only significant difference in RRS (CI < 1.00) is between HH and NN matings.

## Summary of even lineage results
1. Exclusion probabilities were equal to 1, which means we can be confident in the ability of our marker set to correctly assign parents to offspring. 
2. 1,059 offspring were assigned to 405 parents, for an assignment rate of **20.3%**. 
3. The number of offspring assigned to parents varied from 0-12 for females and 0-19 for males. 
4. RRS was significant for both males (0.28 [CI's 0.24-0.34]) and females (0.42 [CI's 0.35-0.5]). 
5. RS significantly varied with parent sample date (early fish do best, middle fish worst, late returning okay). Across the season, regardless of sample date, natural-origin fish had higher RS than hatchery-origin fish. 
6. RS significantly varied with parent size (large males had more offspring than smaller males). However, regardless of size, natural-origin fish do better. 
7. 183 offspring assigned to 2 parents (triads).
8. RRS was significant for HH vs. NN crosses (0.61 [CI's 0.35-0.99]).
9. Hybrids (HN and NH) had RS values that were intermediate between HH and NN crosses.
