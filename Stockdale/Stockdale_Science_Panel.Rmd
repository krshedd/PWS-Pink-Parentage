...
```{r odd_sample_sizes, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r FRANz_posterior_plot_odd, echo=FALSE, warning=FALSE, message=FALSE}
...
All parentage assigments have a posterior probability of 1, which is very robust. This means that no offspring assignments were split among multiple potential parents. 
...
```{r Chi square test odd, echo=FALSE, warning=FALSE, message=FALSE}
O.odd <- c("H", "N")
...
#### Calculate RS  
We'll calculate RS including sampled fish from the parental generation to which offspring were not assigned.
...
paired_13_15_filter_parents %>% 
  ...
  facet_grid(~ SEX) +
  labs(title="Distribution of Family Size by Females and Males \nProportion of Families",
       fill = "Parent: Origin") +
...
**RRS for males = `r round(Male_RRS_odd, 2)`. RRS for females = `r round(Female_RRS_odd, 2)`.**
...
There is a significant difference in RRS for females, but not males. Female hatchery-origin fish produced significantly fewer offspring than natural-origin females, but there is not a diference in RS between hatchery- and natural-origin males. 
...
Here, we test for associations between two explanatory variables (run timing and size) and RS. 
...
These plots show RS by parental sample date, separately for females (left) and males (right). We see that hatchery-origin males and females appear about mid-way through the season. RS for natural-origin females appears consistent throughout the season, whereas male natural-origin fish appear to be more productive early in the season than later on. 
...
This plot shows RS by parental length (mm) separately for females (left) and males (right). It is difficult to draw conclusions about this relationship for hatchery-origin parents since there were so few assignments. There may be evidence for stabilizing selection for natural-origin female size (intermediate-sized females may have a peak in RS). Larger natural-origin males may have higher RS than smaller males. Again, sample sizes are small, which makes it difficult 
...
2. 72 offspring were assigned to 54 parents (3 of which were hatchery-origin), for an assignment rate of **2.3%**. This assignment rate is similar to what was observed for Hogan odd-year (huge parental escapement in 2013 relative to sampling rate). 
3. Family size varied from 0-2 for females and 0-4 for males.  
4. RRS for females = 0.13 (0.01-0.59). RRS for males = 0.34 (0.05-1.10). Hatchery females produce significantly fewer offspring than natural-origin females, but there is not a statistically significant difference for males. 
...
**Note** that *FRANz* assigned 1054 offspring to 405 parents. 865 of these offspring have natural-origin parents and 373 offspring have hatchery-origin parents. The hatchery origin fish are from AFK12B, CCH12, and WNH12PINKB.

Most assignments were to 1 parent (*single parent-offspring duos*) - there were 183 2 parent assignments (*parent pair-offspring trios*). 
...
## Family size
Here, we calculate and plot family size for both natural and hatchery parents.  Family size ranges from 1-19. 
...
More intensive sampling in 2014 and 2016 allowed for fuller distributions of RS. 

```{r Calculate relative reproductive success including 0s even, echo=FALSE, warning=FALSE, message=FALSE}
RRS_Male_Even <- M_h_0_14_16/M_n_0_14_16
RRS_Female_Even <- F_h_0_14_16/F_n_0_14_16

RRS_tibble_0_14_16 <- tibble(RRS_Male_Even, RRS_Female_Even)
round(RRS_tibble_0_14_16, 2)
```

**RRS for males = `r round(RRS_Male_Even, 2)`. RRS for females = `r round(RRS_Female_Even, 2)`.**
...
RRS for females = 0.42 (0.35-0.50). 


RRS for males = 0.28 (0.24-0.34). 

### RS vs. field data plots

As in the odd-lineage, we'll again examine relationships between RS and two explanatory variables: parental sample date and parental length. Since we have many more offspring assignments, we can more clearly visualize these relationships.  

Natural-origin males and females both do better early in the season. This does not appear to be a result of density-dependence or fishing pressure because early-arriving natural-origin fish have higher RS than early-arriving hatchery-origin fish. 
...
Larger males are much more successful (but only natural-origin males). Again, we see at most size classes that natural-origin fish have higher RS than hatchery-origin fish. Size alone, therefore, doesn't explain differences in RS. 
...
Length and run timing are confounding, but natural-origin fish do better regardless of size and timing. In the future, we will also look at RS on fine spatial scales. 
...
```{r cross_rs_even, echo=FALSE, warning=FALSE, message=FALSE}
parents_paired_14_16_cross %>% 
  count(cross, `Fish ID`, `Fish ID.par1`) %>% 
  group_by(cross) %>% 
  summarise(rs = round(mean(n), digits = 2))  
```

The only significant difference in RRS is between HH and NN matings. RRS = 0.61 (0.35-0.99). 
...
4. RRS was significant for both males (0.28 [CI's 0.24-0.34]) and females (0.42 [CI's 0.35-0.5]). 
5. RRS was significant for HH vs. NN crosses (0.61 [CI's 0.35-0.99]).
6. Hybrids (HN and NH) had RS values that were intermediate between HH and NN crosses.
7. RS significantly varied with parent sample date (early fish do best, middle fish worst, late returning okay).Across the season, regardless of sample date, natural-origin fish had higher RS than hatchery-origin fish. 
8. RS significantly varied with parent size (large males had more offspring than smaller males).However, regardless of size, natural-origin fish do better. 
