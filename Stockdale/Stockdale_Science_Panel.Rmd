...
Following Janowitz-Koch et al. (2018), we used a negative binomial distribution model and log-linked function to evaluation associations between RS and origin, length, and sample date separately for females and males. 
...
RS.glm.F <- glm.nb(Female.even$n ~ Female.even$`Sample Date` + Female.even$`Length Mm` + Female.even$origin, link=log) 
RS.glm.F
summary(RS.glm.F)
...
RS.glm.M <- glm.nb(Male.even$n ~ Male.even$`Sample Date` + Male.even$`Length Mm` + Male.even$origin, link=log) 
RS.glm.M
summary(RS.glm.M)
