...
All parentage assigments have a posterior probability of 1, which means that no offspring assignments were split among multiple potential parents. 
...
The above table shows the *p-values* for each test and sex. Female hatchery-origin fish produced significantly fewer offspring than natural-origin females, but there is not a significant diference in RS between hatchery- and natural-origin males. 
...
We examined associations between two potential explanatory variables (sample date and body size) and RS. 
...
The above plot show RS by parent body length, separately for females (top) and males (bottom). It is difficult to draw conclusions about this relationship for hatchery-origin parents since there were so few assignments. Smaller to intermediate-sized females may have higher RS than larger females. Larger natural-origin males may have higher RS than smaller males. Again, sample sizes are small, which makes it difficult to draw robust conclusions.
...
Almost all parentage assigments have a posterior probability of 1, which means that very few offspring assignments were split among multiple potential parents. Note that we used a cutoff of 0.9 for including parent-offspring assignments in downstream analyses.
...
The above table shows the *p-values* for each test and sex. Both male and female hatchery-origin fish produced fewer offspring than their natural-origin counterparts. 
...
### Plots of RS vs. field data
As in the odd-lineage above, we examined associations between two potential explanatory variables (sample date and body size) and RS. Since we have many more offspring assignments, we can more clearly visualize these relationships.
...
1. Most exclusion probabilities were equal to 1, which means we can be confident in the ability of our marker set to correctly assign parents to offspring. 
...
6. RS significantly varied with parent size (large males had more offspring than smaller males). However, regardless of size, natural-origin fish had higher RS. 
