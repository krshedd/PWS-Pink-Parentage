---
title: "All Streams 2016/2018 Parentage"
...
**NOTE** the Gilmour, Erb, and Paddy data will need to be re-run post-GOD. All of the `NA`-origin parents (2016) were removed in `1_QA`, and thus not included in these analyses. The `NA`-origin offspring (2018) were retained.

## Pedigree reconstruction

We use the program *FRANz* [link](https://www.bioinf.uni-leipzig.de/Software/FRANz/About.html) to reconstruct parent-offspring relationships. Below are analyses of parentage for **all fitness streams (Hogan, Stockdale, Gilmour, Paddy, and Erb)** for the **even lineage** years **2016** (potential parents) and **2018** (potential offspring).

**NOTE** due to memory allocation limits on the UAF cluster, we had to break this into two (2) separate analyses:
  1) only Hogan 2016 parents (n = 11,522), all streams 2018 offspring  (n = 25,926), and 
  2) no Hogan 2016 parents (just Stockdale, Gilmour, Paddy, and Erb; n = 12,973), all streams 2018 offspring (n = 25,926)

Ideally, offspring will only have parentage assignments from a single stream...

*FRANz* was run by Chase secretly on the UAF cluster on 5/23/2022 using the following parameters:
FRANz.exe --Nmmax 25000 --Nfmax 25000 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 All_Streams_16_18_develop_postQA_2016_2018_Hogan_parents_only.dat

*FRANz* was run by Chase secretly on the UAF cluster on 5/24/2022 using the following parameters:
FRANz.exe --Nmmax 25000 --Nfmax 25000 --femrepro 1:2 --malerepro 1:2 --typingerror 0.006 --updatefreqs --poutformat 2 All_Streams_16_18_develop_postQA_2016_2018_no_Hogan_parents.dat
...
Since we had to do two *FRANz* runs for memory allocation reasons, we need to combine the `parentage.csv` outputs before we proceed.
```{r}
parentage_16_18_hogan_parents <-
  readr::read_csv(
    "~/../Desktop/Local_PWS_pinks/Franz/All_Streams_16_18_develop/2016_2018_HoganParentsOnly/results/parentage.csv"
  ) %>%  # read_csv("../../Franz/All_Streams_16_18_develop/2016_2018_HoganParentsOnly/results/parentage.csv") 
  dplyr::mutate(run = "only_hogan")

parentage_16_18_no_hogan_parents <-
  readr::read_csv(
    "~/../Desktop/Local_PWS_pinks/Franz/All_Streams_16_18_develop/2016_2018_NoHoganParents/results/parentage.csv"
  ) %>%  # read_csv("../../Franz/All_Streams_16_18_develop/2016_2018_NoHoganParents/results/parentage.csv")
  dplyr::mutate(run = "no_hogan")
...
base::rm(parentage_16_18_hogan_parents, parentage_16_18_no_hogan_parents)
```

Import paired *OceanAK* data and `riverdist` stream location data.
```{r data_import, echo=FALSE, warning=FALSE, message=FALSE}
paired_16_18 <-
  readr::read_csv(
    "../Franz/All_Streams_16_18_develop_postQA_OceanAK_paired_2016_2018_HOGAN_STOCK_GILMOUR_PADDY_ERB.csv"
  )
stream_specimens_riverdist <-
  readr::read_csv(
    "~/../Desktop/Local_PWS_pinks/GIS/stream_specimens_riverdist_all_streams_2013_2020.csv"
  )  # read_csv("../GIS/R/all_streams/stream_specimens_riverdist_all_streams_2013_2020.csv")
```
...
parentage_16_18  %>% 
  ...
  ggplot2::facet_grid(rows = dplyr::vars(run))
```
...
(
  parentage_16_18_filter <- parentage_16_18 %>%
    ...
    dplyr::select(Offspring, `Parent 1`, `Parent 2`, run) %>%
    tidyr::pivot_longer(
      names_to = "Parent",
      values_to = "Parent_ID",
      cols = -c(Offspring, run)
    ) %>%
    dplyr::filter(!is.na(Parent_ID))
)
```

Since we are combining runs ("only Hogan parents" and "no Hogan parents"), we need to verify that Offspring were only assigned parents in 1 of those two runs (i.e. not both!). Going to check two different ways to make extra, super-duper sure :).
```{r}
tmp_only_hogan <- parentage_16_18_filter %>% 
  dplyr::filter(run == "only_hogan") %>% 
  dplyr::pull(Offspring)

tmp_no_hogan <- parentage_16_18_filter %>% 
  dplyr::filter(run == "no_hogan") %>% 
  dplyr::pull(Offspring)

base::intersect(tmp_only_hogan, tmp_no_hogan)

base::rm(tmp_no_hogan, tmp_only_hogan)

parentage_16_18_filter %>% 
  dplyr::filter(Parent == "Parent 1") %>% 
  dplyr::count(Offspring, run) %>% 
  dplyr::count(Offspring) %>% 
...
(
  paired_16_18_filter <- paired_16_18_filter %>%
...
readr::write_csv(paired_16_18_filter, "all_streams_paired_16_18_filter.csv")
```

Clean up environment to save memory.
```{r}
base::rm(paired_16_18, stream_specimens_riverdist, parentage_16_18)
```

## Sample sizes

  * Quick look at sample sizes for potential parents (2016)

**NOTE** Gilmour, Erb, and Paddy were all affected by GOD. We *dropped* the `NA` origin parents (2016) in `1_QA`
...
```
  ...
  * Quick look at sample sizes for potential offspring (2018)

**NOTE** Gilmour, Erb, and Paddy were all affected by GOD. We *kept* the `NA` origin offspring (2018) in `1_QA`
```{r sample_sizes_offspring, echo=FALSE, warning=FALSE, message=FALSE}
paired_16_18_filter %>% 
  dplyr::filter(year == 2018) %>%  # filter for offspring year
  dplyr::count(stream)
```

Which ones were involved in GOD?
```{r}
paired_16_18_filter %>% 
  dplyr::filter(year == 2018) %>%  # filter for offspring year
  dplyr::count(stream, origin) %>% 
...
**NOTE** Since we already dropped the `NA` parents (2016) for Gilmour, Erb, and Paddy, they do not show up as grey columns here in these plots.
...
sample_size_plot(indata = paired_16_18_filter, title = "All Streams 2016-2018")
```

**NOTE** `NA` origin fish in 2018 (offspring) for Erb, Gilmour, and Paddy are due to the GOD event.
...
(
  parents_paired_16_18 <- parentage_16_18_filter %>%
    dplyr::left_join(paired_16_18_filter, by = c("Offspring" = "franz_id")) %>%
    dplyr::left_join(
      paired_16_18_filter,
...
readr::write_csv(x = parents_paired_16_18,
                 file = "all_streams_parents_paired_16_18.csv")
```
...
```{r}
n_off <- paired_16_18_filter %>%
  dplyr::filter(year == 2018) %>%  # filter for offspring year
  dplyr::count(stream)

n_off_assigned <- parents_paired_16_18 %>%
  dplyr::filter(parent == "Parent 1") %>% 
...
**Results:** Most offspring were only assigned to 1 parent (dyads), but there were a decent amount assigned to 2 parents (triads). Offspring assignment rates ranged from a low of **12.2%** in Paddy to a high of **34.8%** in Hogan. Interestingly, Stockdale and Gilmour (on Montague Island) mostly receive strays from AFK, Hogan receives a mix of AFK (most common), CCH, WNH, and SGH, and Paddy and Erb receive most of their strays from WNH, followed by AFK, CCH, and a few SGH.
...
(
  spawners_and_offspring_16_18 <-
    dplyr::bind_rows(spawners_16, offspring_18) %>%
    dplyr::arrange(stream, origin) %>%
    tidyr::pivot_wider(names_from = generation, values_from = n)
)

rm(spawners_16, offspring_18)  # keep it clean!
...
**Results:** All streams, except for Gilmour and Paddy, showed a significant difference in the proportions of offspring assigned to hatchery- and natural-origin parents relative to the proportions of potential parents sampled, indicating an under-representation of offspring assigning to hatchery-origin parents. 
...
(
  paired_16_18_filter_parents <- paired_16_18_filter %>%
    ...
    dplyr::left_join(
      x = .,
      y = dplyr::count(parents_paired_16_18, parent_id),
...
write_csv(x = paired_16_18_filter_parents, file = "all_streams_paired_16_18_filter_parents.csv")
```
...
**Results:** There are *only* significant differences in RRS for females in Erb, and *only* in Erb, Hogan, and Paddy for males. 
...
**Results:** There are significant differences in RRS for Erb, Hogan, and Stockdale^*^ when we ignore sex-specific differences. 
...
**Results:** Trends vary by stream, but there is not a ton of separation in RS between hatchery- and natural-origin fish regardless of parent sample date.
...
RS_by_location_plots <-
  sapply(unique(paired_16_18_filter_parents$stream), function(x)
  {
    paired_16_18_filter_parents %>%
      filter(stream == x) %>%
...
**Results:** Trends vary by stream, but in general, RS is higher near the intertidal for Erb, Paddy, and Stockdale. Not so much for Hogan and Gilmour.

#### *RS by intertidal*

View how RS varies by parent sample location within the stream (intertidal or upstream).

**NOTE** the y-axis varies among streams to facilitate visualizing patterns, labels indicate mean values.
...
paired_16_18_filter_parents %>%
  ...
  ggplot2::facet_grid(stream ~ sex, scales = "free_y") +
  ggplot2::labs(title = "Reproductive Success by Parental Location (Intertidal) and Sex",
                fill = "Parent: Origin",
...
View how RS varies by pre-spawning state within the stream (TRUE or FALSE).

**NOTE** the y-axis varies among streams to facilitate visualizing patterns, labels indicate mean values.
```{r RS_by_prespawn, echo=FALSE, warning=FALSE, message=FALSE, fig.height=10}
# Females and males separated
paired_16_18_filter_parents %>%
  dplyr::filter(!is.na(pre_spawn)) %>%
  ggplot2::ggplot(ggplot2::aes(x = pre_spawn, y = n, fill = origin)) +
  ggplot2::geom_hline(yintercept = 0, color = "grey50") +
  ggplot2::geom_boxplot() +
  ggplot2::stat_summary(
    ggplot2::aes(label = round(..y.., 2)),
  ...
  ggplot2::facet_grid(stream ~ sex, scales = "free_y") +
  ggplot2::labs(title = "Reproductive Success by Pre-spawn Status and Sex",
                fill = "Parent: Origin",
                x = "Pre-spawn Mortality?",
                y = "Reproductive Success") +
  ggplot2::theme_bw() +
  # ggplot2::theme(panel.grid.minor = element_blank()) +
  ggplot2::scale_fill_manual(values = cbPalette2)
...
**Results:** Trends vary by stream, but in general, RS is higher for fish that were not marked as pre-spawn mortality.

#### *RS by partial-spawn*

View how RS varies by partial-spawning state within the stream (TRUE or FALSE).

**NOTE** the y-axis varies among streams to facilitate visualizing patterns, labels indicate mean values.
```{r RS_by_partialspawn, echo=FALSE, warning=FALSE, message=FALSE, fig.height=10}
# Females and males separated
paired_16_18_filter_parents %>%
  dplyr::filter(!is.na(partial_spawn)) %>%
  ggplot2::ggplot(ggplot2::aes(x = partial_spawn, y = n, fill = origin)) +
  ggplot2::geom_hline(yintercept = 0, color = "grey50") +
  ggplot2::geom_boxplot() +
  ggplot2::stat_summary(
    ggplot2::aes(label = round(..y.., 2)),
  ...
  ggplot2::facet_grid(stream ~ sex, scales = "free_y") +
  ggplot2::labs(title = "Reproductive Success by Partial-spawn Status and Sex",
                fill = "Parent: Origin",
                x = "Partial-spawn Mortality?",
                y = "Reproductive Success") +
  ggplot2::theme_bw() +
  # ggplot2::theme(panel.grid.minor = element_blank()) +
  ggplot2::scale_fill_manual(values = cbPalette2)
...
**Results:** Trends vary by stream, but in general, RS is higher for fish that were not marked as partial-spawn mortality.
...
**NOTE** the y-axis varies among streams to facilitate visualizing patterns, labels indicate mean values.
```{r RS_by_preyed_upon, echo=FALSE, warning=FALSE, message=FALSE, fig.height=10}
# Females and males separated
paired_16_18_filter_parents %>%
  dplyr::filter(!is.na(preyed_upon)) %>%
  ggplot2::ggplot(ggplot2::aes(x = preyed_upon, y = n, fill = origin)) +
  ggplot2::geom_hline(yintercept = 0, color = "grey50") +
  ggplot2::geom_boxplot() +
  ggplot2::stat_summary(
    ggplot2::aes(label = round(..y.., 2)),
  ...
  ggplot2::facet_grid(stream ~ sex, scales = "free_y") +
  ggplot2::labs(title = "Reproductive Success by Preyed-upon Status and Sex",
                fill = "Parent: Origin",
                x = "Preyed-upon Mortality?",
                y = "Reproductive Success") +
  ggplot2::theme_bw() +
  # ggplot2::theme(panel.grid.minor = element_blank()) +
  ggplot2::scale_fill_manual(values = cbPalette2)
...
**Results:** Trends vary by stream, but in general, RS is higher for fish that were not marked as preyed-upon mortality.
...
(
  parents_paired_16_18_cross <- parents_paired_16_18 %>%
    dplyr::filter(offspring_id %in% triads_16_18) %>%
...
readr::write_csv(x = parents_paired_16_18_cross, file = "all_streams_parents_paired_16_18_cross.csv")
```

Verify that parents were sampled in the same stream.
```{r parents_from_same_stream}
all(parents_paired_16_18_cross$stream_dam == parents_paired_16_18_cross$stream_sire)
```
...
readr::write_csv(x = paired_16_18_filter_cross, file = "all_streams_paired_16_18_filter_cross.csv")
```

**Results:** We have **617** unique matings documented among the five fitness streams for 2016.

Calculate mean RS by stream and cross type
```{r calculate_RS_cross, echo=FALSE, warning=FALSE, message=FALSE}
# Mean RS, including zeroes
rs_16_18_cross <- paired_16_18_filter_cross %>%
...
**Results:** RRS values by cross type are a bit all over the place, but looking at the sample sizes below, that isn't terribly surprising given the pretty sparse sample sizes by cross type.

#### Samples sizes

How many unique matings occurred by cross type and stream?
```{r n_matings by cross}
paired_16_18_filter_cross %>%
  dplyr::count(stream, cross_type, name = "n_mating") %>%
  tidyr::pivot_wider(names_from = cross_type, values_from =  n_mating)
```

**Results:** Sample sizes of families (matings) by cross type are fairly small, especially for non-NN crosses.
...
**Results:** RRS estimates are not necessarily < 1 for all streams, although sample sizes for non-NN cross types are pretty low.
...
**Results:** None of these RRS results are significant, low sample size of families + high variance in family size.
...
dam_promiscuity <- parents_paired_16_18_cross %>% 
...
sire_promiscuitiy <- parents_paired_16_18_cross %>% 
...
Neat to see that sibling groups do not all tend to stray together, typically just 1 or 2 fish from a given parent end up straying. Keep in mind, that these aren't "stray rates" since sampling proportion varies across streams (and thus the probability of detection!).
...
2. 6,412 out of 25,926 offspring were assigned to 2,925 distinct parents, for an overall assignment rate of 28.6%.
4. RRS point estimates ranged from 0.30-1.01 for males and 0.45-1.31 for females. RRS estimates were statistically significant (95% CI did not contain 1) for all combinations, except Stockdale and Gilmour for males, and Gilmour and Paddy for females.  
5. All four crosses were represented (HH, NH, HN, and NN), but cross type RS relative to the NN were not significant, largely due to small sample sizes of families.  

```{r}
save.image("2_Parentage_All_Streams_2016_2018_develop_20220527.RData")
```
