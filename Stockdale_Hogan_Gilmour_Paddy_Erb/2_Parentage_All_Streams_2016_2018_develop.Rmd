...
```{r}
parentage_14_16_hogan_parents <- readr::read_csv("~/../Desktop/Local_PWS_pinks/Franz/All_Streams_16_18_develop/2016_2018_HoganParentsOnly/results/parentage.csv")  # read_csv("../../Franz/All_Streams_16_18_develop/2016_2018_HoganParentsOnly/results/parentage.csv") 
```

Combine
```{r}
(
  parentage_14_16_hogan_parents_filter <- parentage_14_16_hogan_parents %>%
...
parentage_14_16_hogan_parents_filter %>% 
  dplyr::mutate(stream_off = stringr::str_sub(string = Offspring, start = 2, end = 2)) %>% 
  count(stream_off)
```

Import paired *OceanAK* data and `riverdist` stream location data.
```{r data_import, echo=FALSE, warning=FALSE, message=FALSE}
paired_14_16 <- readr::read_csv("../Franz/All_Streams_16_18_develop_postQA_OceanAK_paired_2016_2018_HOGAN_STOCK_GILMOUR_PADDY_ERB.csv")
